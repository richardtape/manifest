...
**1604 tests with no Docker, and 194 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier's one skipped test is GONE**: it was the production gate's positive control, and P6a's Task 14 un-skipped it by building the last blocking item it was waiting for. *(P6a sitting 10 took the first from 1598 to 1604 and the second from 192 to 194, 2026-09-20: §13's *Integrity of the gate* made falsifiable — the registry's refusal now asserted against a push the REAL registry accepts, the laptop-image rule for production and for the rehearsal, and the approval record's append-only property. Each earlier sitting's movement is in its plan record.)* See
...
| **Executing P6a — the current job** | **P5c IS EXECUTED — all 14 tasks in nine sittings, 2026-09-18/19, 107 findings — and PHASE 1c IS COMPLETE** ([P5c, the clients](docs/superpowers/plans/2026-09-18-p5c-the-clients.md)). **P6a — the first production launch — IS WRITTEN: [19 tasks in eleven agreed sittings](docs/superpowers/plans/2026-09-19-p6a-first-production-launch.md) (2026-09-19), from [the P6 brief](docs/superpowers/plans/2026-09-19-p6-brief.md). IT IS EXECUTING: the measurements moved no task boundary, so the eleven-sitting split stands; **§12's INTERNAL/PUBLIC LISTENER SPLIT IS NOW REAL** — the edge runs two servers in one container, `srv0` on `127.0.0.2` for staging, sandbox, the console and the IdP, and `srv1` on `127.0.0.3` for the production zone and nothing else, so §21's honest divergence 2 no longer describes this machine; **since 2026-09-20 §13's LAUNCH CHECKLIST IS THE THING THAT GATES PRODUCTION** — one evaluation the deploy route calls and the read renders, with both of the old unconditional refusals gone, so a production deploy is now refused for a reason a person can act on and an administrator recording what UBC IAM and the Privacy Office said visibly shortens the list; **and §20's STEP-UP RE-AUTHENTICATION IS REAL, walked end to end against the live Manifest IdP** — a person who has not re-proved themselves in the last ten minutes is refused `403 STEP_UP_REQUIRED` with the remedy in the answer, `GET /auth/step-up` makes the IdP prompt again even for somebody already signed in to it, and the same request then succeeds; **and since the same day `release:approve` HAS A CALLER — its first in this platform's life** — an administrator approves a release for production behind step-up, the approval binds the build's immutable image digest and stores the exact diff shown at decision time, and a summary the language model cannot produce is recorded as ABSENT rather than blocking the approval; **and since 2026-09-20 AN APPLICATION IS IN PRODUCTION** — `journey-app` answers on `127.0.0.3` as the instance the deploy started, with every one of §13's six blocking items met the way a person meets them, including **D21's rehearsal: Manifest deploys the candidate to its production hostname, registers its Service Provider with production values and completes a real CWL sign-in, and the item is met by that MEASUREMENT rather than by a tick.** **And since the same day THE CONSOLE DRIVES THE WHOLE FIRST LAUNCH** — an administrator records what UBC said, runs the rehearsal and reads its evidence, is asked to confirm it is them at the IdP, approves, and deploys to production, **clicked end to end by a person** — and it found that no client can show the diff an approval records until after the decision, which is Rich's question now.' [ORIENTATION §7e](docs/superpowers/ORIENTATION.md) is the hand-off; **the plan's own sittings table says how far it has got, and this row deliberately states no sitting number.** P6 was split into P6a and P6b on D9's own two clauses. |
