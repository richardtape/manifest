...
**831 tests with no Docker, and 167 more that need a daemon** — nothing skipped, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block. See
...
| **Executing P4c — the current job** | [`docs/superpowers/plans/2026-09-15-p4c-zero-downtime-redeploys.md`](docs/superpowers/plans/2026-09-15-p4c-zero-downtime-redeploys.md) — zero-downtime redeploys, 11 tasks in **eight agreed sittings, one per session** — **sittings 1 to 7 are done (2026-09-15 and 2026-09-16; 13 findings, 4, 7, 6, 9, 10 and 9)**. The first built `make demo-redeploy` and watched it fail three times; the second put §11's redeploy contract in code with its eleven-test continuity block; the third made the **driver** redeploy without interrupting anybody (a hostname taken over from one live instance to another under a request every 25 ms with zero 502s and zero wildcard answers, where the old order records seven empty 502s); the fourth made **the driver contract green on Docker — 25 of 25, nothing skipped**; the fifth built **§6's `Route` record — specified since P2 and never built — and the retirer**, the control plane's first background work; and **the sixth gave all of it a caller**: `deployRelease` runs everything that changes what serves inside an advisory lock per environment, schedules the retire, and leaves the previous release serving when a new one never becomes ready — and `recoverAtBoot` puts the routes back, ends the deploys a restart interrupted and finishes the drains it cut short, before the server listens; and **the seventh put the blueprint's sessions in the app's own Mongo**, so a redeploy signs nobody out and `make demo-redeploy` is **21 of 21 green for the first time**. **Five of the plan's own negative controls turned out to be unable to fail; every one was measured and replaced.** **Sitting 8 is Task 11, the acceptance, alone and last** — two more runs, one from a `make reset` machine, and nine controls watched red. **Not the whole plan in one session**, and Rich checks in at each boundary. Its brief, with the measurements behind it, is [`2026-09-15-p4c-brief.md`](docs/superpowers/plans/2026-09-15-p4c-brief.md). Read [P4b](docs/superpowers/plans/2026-09-07-p4b-ai-events-streaming-incidents.md)'s *What executing this plan found* first — **all ten sittings are done, 2026-09-15, 140 findings** — and sitting 10 especially: findings 181 and 189 are two of P4c's inputs |
