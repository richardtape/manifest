...
**1723 tests with no Docker, and 198 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier's one skipped test is GONE**: it was the production gate's positive control, and P6a's Task 14 un-skipped it by building the last blocking item it was waiting for. *(P6b sitting 6 took the first from 1690 to 1723 and left the second at 198, all passing, 2026-09-23: the stored preview an approval copies, and the screen that shows it first. Each earlier sitting's movement is in its plan record.)* See
