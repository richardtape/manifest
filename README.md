...
**1496 tests with no Docker, and 185 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier now has exactly ONE skipped test**: the production gate's positive control, which needs the approval and the rehearsal that P6a's Tasks 10 and 14 build, and which names them. *(P6a sitting 6 took the first from 1456 to 1496 in two new files when §20's step-up re-authentication landed; the Docker tier was owed, run and unmoved at 185 — which is a coverage statement, because no Docker test makes a member call over HTTP or drives `/auth/step-up`, 2026-09-20.)* See
