...
**1456 tests with no Docker, and 185 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier now has exactly ONE skipped test**: the production gate's positive control, which needs the approval and the rehearsal that P6a's Tasks 10 and 14 build, and which names them. *(P6a sitting 5 took the first from 1449 to 1456 when §13's checklist became the thing that gates production, with no new file; the Docker tier was owed, run and unmoved, 2026-09-20.)* See
