...
**1605 tests with no Docker, and 194 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier's one skipped test is GONE**: it was the production gate's positive control, and P6a's Task 14 un-skipped it by building the last blocking item it was waiting for. *(P6a sitting 11 took the first from 1604 to 1605, 2026-09-22: a test that an existing route found on the WRONG listener is moved there, which its acceptance's own negative control found by staying green. Each earlier sitting's movement is in its plan record.)* See
