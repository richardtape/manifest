...
**832 tests with no Docker, and 167 more that need a daemon** — nothing skipped, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block. See
...
**P4a, P4b and P4c are finished — 2026-09-09, 2026-09-15 and 2026-09-16. Writing P5a — the first of P5's three plans — is the current work.**
...
| **Writing P5a — the current job** | **[The P5 brief](docs/superpowers/plans/2026-09-16-p5-brief.md) first** — it compares 1c with the code and estimates 30–40 tasks; Rich has since split P5 into **P5a (the contract), P5b (delegated tokens) and P5c (the clients)**, settled all five decisions and had them applied to the spec — **writing P5a is the job, and [ORIENTATION §7e](docs/superpowers/ORIENTATION.md) is its complete task description** — then [ORIENTATION §7e](docs/superpowers/ORIENTATION.md) and the roadmap's P5 section: the published contract, `manifest-mock`, the generated client and the reference console. **No plan is waiting to be executed**: [P4c](docs/superpowers/plans/2026-09-15-p4c-zero-downtime-redeploys.md) — zero-downtime redeploys, 11 tasks in eight agreed sittings — **finished 2026-09-16 with 70 findings**. Its last sitting ran the acceptance green three times, once from a `make reset` machine, and found that **four of its nine negative controls could not fail in it** — each is now watched red where it can be; read its sitting-8 record before trusting a green `make demo-redeploy` to prove any of them. Its brief, with the measurements behind it, is [`2026-09-15-p4c-brief.md`](docs/superpowers/plans/2026-09-15-p4c-brief.md) |
| Seeing the platform actually work | `make demo` (an app, from a bare repository to a URL), **`make demo-identity`** (a real CWL sign-in whose note nobody else can see), **`make demo-ai`** (the same app answering a question from the asker's own notes, charged to them) and **`make demo-redeploy`** (the same app redeployed twice and failed once while a signed-in student keeps asking, with nothing interrupted), after `make up` and starting the control plane. [`RUNBOOK.md`](docs/superpowers/RUNBOOK.md) has all four, step by step |
