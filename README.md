...
**19 checks / 0 failed** and `make verify` **55 / 0**, and both were green **with the
network off** when P1 was executed. *(P6a sitting 10 added one verify check, 2026-09-20: the
registry's token realm must be the control plane's own route, for the service it mints —
every earlier check passed whatever realm the registry named. Each earlier sitting's count is
in its plan record.)* `https://console.manifest.internal/` returned the same hostname and
...
**1742 tests with no Docker, and 200 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier's one skipped test is GONE**: it was the production gate's positive control, and P6a's Task 14 un-skipped it by building the last blocking item it was waiting for. *(Re-measured at the close of the D5 plan's sitting 1, 2026-09-24, which changed no code: 1742, unmoved; the Docker tier was not owed and its 200 is the F10 sitting's. Each earlier sitting's movement is in its plan record.)* See
...
| **The next job — executing sitting 2 (Tasks 2 and 3) of D5's GitHub source driver plan ([written 2026-09-24](docs/superpowers/plans/2026-09-24-d5-github-source-driver.md); its sitting 1, the measurements, ran the same day — [`spikes/d5-baseline/`](docs/superpowers/spikes/d5-baseline/README.md))** | **P6a — the first production launch — IS EXECUTED: [all 19 tasks in the eleven agreed sittings](docs/superpowers/plans/2026-09-19-p6a-first-production-launch.md), 2026-09-19 → 2026-09-22**, from [the P6 brief](docs/superpowers/plans/2026-09-19-p6-brief.md). **An application now reaches production with every one of §13's blocking items honestly met**: on §12's PUBLIC listener (`127.0.0.3`, a second Caddy server staging cannot reach), after an administrator records what UBC IAM and the Privacy Office said, Manifest's rehearsal completes a real CWL sign-in on the production hostname, and an administrator re-proves themselves at the IdP and approves the exact image digest — which the deploy verifies before starting anything. **`make demo-production` proves it headlessly**, green from a `make reset` machine and step 11 of the offline acceptance, **and a person has clicked the whole launch through the console.** Its acceptance found and fixed a live fail-open: a production route kept whatever listener already held it. **Rich decided on 2026-09-22 that an administrator sees the approval's diff before deciding — a stored preview the approval binds — in P6b.** **P6b, subsequent releases, IS EXECUTED** ([`plans/2026-09-22-p6b-subsequent-releases.md`](docs/superpowers/plans/2026-09-22-p6b-subsequent-releases.md), written 2026-09-22, executed 2026-09-22 → 2026-09-23 in seven sittings): self-serve production redeploys, re-escalation to an administrator when a release changes a sensitive field, the stored preview an approval binds, and the IAM change request. **Its sitting 1, the measurements, ran on 2026-09-22** — every premise true, and one defect nobody knew: **a changed `egress.allow` never reached a running environment's proxy**, now the plan's Task 5a. **Its sitting 2 ran on 2026-09-23**: an agent's token can never approve a release or record UBC's decisions — refused centrally with a code of its own, `TOKEN_PERSON_ONLY` — and the sensitive-field comparison now reads what production actually runs, against the last release an administrator did not withdraw. **Its sittings 3 and 4 ran the same day**: an app's launch is recorded once, by the deploy that makes it true, and a launched app is never rehearsed; and **once launched, a release goes to production with no administrator — through the route a person uses — unless it changes a sensitive field**, when it is refused `409 RELEASE_REESCALATED` until an administrator approves it; production deploys only the release serving staging. **Its sitting 5 ran the same day**: an IAM change request is recorded as the registration's own state, so what UBC registered changes only when UBC registers it — adding a CWL attribute waits for UBC, removing one waits only for an administrator — and every approval now records a security note for each sensitive change, the code reviewer's verdict and the plain statement that nothing reviews a self-serve release. **Its sitting 6 ran the same day**: an administrator now reads a STORED preview of exactly what an approval will record — before deciding, and still the same after the step-up round trip — and the approval names it and copies it, so the model's summary is written once and is what was read. **Its sitting 7 — the acceptance — ran the same day: `make demo-releases` drives a launched app's next release three ways through nothing but the generated client, green three times (the third from a `make reset` machine), and a person clicked it** — including the path where an administrator's preview goes stale because another release was approved meanwhile. **What comes next is stated in [ORIENTATION §7e](docs/superpowers/ORIENTATION.md)**, the hand-off — writing D5's GitHub source driver plan, Rich's order — and deliberately not here; and Rich answered the plan's three questions the same day (ORIENTATION §8, *Decided*): seven sittings. P6 was split into P6a and P6b on D9's own two clauses. |
