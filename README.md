...
**1544 tests with no Docker, and 185 more that need a daemon** — the Docker tier skips nothing, since P4c Task 5 gave the Docker driver the fixtures for the driver contract's continuity block, and **the unit tier still has exactly ONE skipped test**: the production gate's positive control, which needs the approval and the rehearsal that P6a's Tasks 10 and 14 build. **Task 10 is now done and it is still skipped**, because the rehearsal is Task 14's, and the test names them both. *(P6a sitting 7 took the first from 1496 to 1544 in two new files when §13's approval landed — `release:approve`'s first caller in this platform's life; the Docker tier was owed, run and unmoved at 185, which is again a coverage statement: not one Docker test drives an approval, 2026-09-20.)* See
