...
*Approved in substance, then applied only after Rich read the exact wording below and said yes — the same afternoon they were raised.*

**Why they exist.** On 2026-09-24 Rich switched the chat model to `qwen3.5:4b`, a thinking model, run with thinking OFF, and then asked for the two things below. The config, the tests and the measurements are committed (`be7042e`, `36c7e92`); the D5 plan's sitting 1 record, *After the sitting*, and `spikes/d5-baseline/` have the numbers. **No shared HTML page restates §7's catalogue or §21's thinking-model sentence** (checked: no `default-chat`, `max_classification`, `logical model` or `thinking` in `specs/*.html`), so none moves.

**A — §7 *Logical model names* and *Classification gates model routing (D17)*: the `-reasoning` names.** In *Logical model names*, after its first paragraph (*"…no redeploys, no faculty involvement."*), add:
...
In *Classification gates model routing (D17)*, the catalogue block becomes:
...
**B — §21 *What offline AI does and does not prove*: "must stream content", not "must be non-thinking".** Its first sentence's *"A 7–8B model"* becomes *"A 4–8B model"*. The paragraph beginning *"**The local chat model must be a non-thinking model**"* is replaced by:
...
3. **§20 *Git driver* bullet 1** — the one installation token that cannot be scoped to a repository: the one that creates it. That token carries administration alone, is used for that one call and is never kept. **This action is withdrawn if Task 6's real conformance leg shows otherwise.**
   **→ THE RUN SHOWED OTHERWISE, AND RICH CHOSE THE CORRECTION — APPLIED 2026-09-24.** Task 6's real leg (sitting 3,
   C7s) measured a token scoped to ANOTHER repository creating one: GitHub does not confine creation to a token's
   repositories. Offered (a) revert, (b) correct, (c) keep; **Rich chose (b)**, and §20's bullet 1 now reads:
   *"installation tokens are short-lived and scoped per repository — though GitHub does not confine creation to a
   token's repositories: any token holding the administration permission can create one in the organisation, so
   the token that creates a repository carries that permission alone, is used for that one call, and is never
...
Written in [P6b's *Spec actions*](./2026-09-22-p6b-subsequent-releases.md). **One. Rich chose the plan's Question 2 option (a) on 2026-09-22 with this wording shown beside it, which approves it in substance; **Rich said to apply it on 2026-09-24, and it is APPLIED**, in the exact wording he was shown**: §13 D9.2's *"A change to `auth.attributes` additionally requires an IAM change request to reach `active`"* now reads *"a change … **that adds an attribute UBC IAM has not registered**"*, with a sentence saying a removal re-escalates like any sensitive change and does not wait on IAM. `manifest-decisions.html`'s D9 and D16 cards were swept with it the same day — D9's now says *"changes something sensitive"* rather than *"starts asking for something new"*, which a removal is not. **The spec and the code now agree**: from 2026-09-23 to 2026-09-24 the spec's sentence was stricter than what P6b built. **Not proposed, deliberately:** R4(d) (Rich kept §13 untouched on 2026-09-19), the new columns and table (§6 lists key fields, and P6a's `rehearsals` set the precedent), and the plan's Decisions 8 and 16, which make true what §13 and §9 already say.
...
| **P6–P11** | 2 | **SEVEN plans since 2026-09-19, listed below** — P6 split into P6a and P6b on D9's own two clauses — **P6a IS EXECUTED (2026-09-22) and P6b IS EXECUTED (2026-09-23, all twelve tasks in Rich's seven sittings; its acceptance, `make demo-releases`, passed three times and was clicked by a person) — D5's GitHub source driver, next in Rich's order, IS WRITTEN (2026-09-24, [15 tasks in eight sittings](./2026-09-24-d5-github-source-driver.md)) and the next job is EXECUTING its sitting 1** — the P6a row below has the record, and this cell deliberately names no sitting (it named *"sitting 8"* for the whole of sitting 9 and into sitting 10 — found by P6a sitting 10's sweep). P6a IS BRIEFED** ([`2026-09-19-p6-brief.md`](./2026-09-19-p6-brief.md)), which records the three decisions Rich made that day and sizes the whole of P6 at 18–24 tasks | P6a: an app reaches production with every blocking item honestly met |
...
| **P6b** | *(Findings counts for every sitting are in the defect-rate table below — this row states none.)* **Subsequent releases** — ✅ **EXECUTED 2026-09-22 → 2026-09-23 — [all twelve tasks](./2026-09-22-p6b-subsequent-releases.md) (eleven and Task 5a) in Rich's seven sittings.** **F10 — the console's *Sign out* leaving the IdP session alive — was FIXED AFTER THE PLAN, in its own sitting (2026-09-24, `0aa1824`, at Rich's word)**, with the defect fixing it found: the SLO route accepted UNSIGNED logout messages. The findings are in the defect-rate table's row after P6b's total. **Sitting 7 — Task 11, the acceptance — RAN 2026-09-23** (`1e02844`, `6fe5279`): **`make demo-releases` passed three times** — fresh, re-use, and from an `echo reset | make reset` machine — driving a launched app's next release three ways through nothing but `@manifest/contract`: a sensitive change refused `RELEASE_REESCALATED` until an administrator approved it from a stored preview, its egress change then in force in production's proxy; a code-only release to production with no administrator under a loop that saw only the app; and §9's IAM change request refusing a build until UBC registered the attribute. **All nine controls fired**, three of them where the plan's own prediction was wrong or unreachable; **Rich clicked every row plus the stale path, never clicked before.** It found no defect in P6b's code; it found the model's summary inventing an administrator's verdict, the console's *Sign out* leaving the IdP session alive, and four console rough edges — all inherited by the next plan through P6b's *What this plan does not build*. Offline-acceptance step 12; a `ci-acceptance` step. **Sitting 1 — Task 1, the measurements — RAN 2026-09-22** ([`spikes/p6b-baseline/`](../spikes/p6b-baseline/README.md)): **all six premises measured true, all five controls fired, and it found a seventh defect the plan did not know — the egress proxy never re-renders its allowlist**, so a changed `egress.allow` never reaches a running environment (an added host refused, a REMOVED host still reachable). That adds **Task 5a** to sitting 3 (the plan's Decision 20) and corrects Task 11's leg A; **Rich's seven sittings stand.** **Sitting 2 — Tasks 2 and 3 — RAN 2026-09-23** (`5f323ef`, `9372f06`, `88fd228`): **the person-only class is one central rule** — `assertCapability` refuses a token asking for `release:approve` or `launch:record` as `403 TOKEN_PERSON_ONLY` with no pending action, the mint refuses both, and `@manifest/contract` is `1.1.0`; **and the sensitive diff is one rule over what production runs** — `isSensitiveDiff` now sees a raised production override, the re-escalation baseline is each release's LATEST decision, and a release freezes its BUILD's spec and refuses another project's build. Every one of its controls fired; its findings are mostly about the plan's controls, two of which left a guard with no witness until this sitting gave it one. **Rich confirmed on 2026-09-23 that Task 5a stays in sitting 3.** **Sitting 3 — Tasks 4, 5 and 5a — RAN 2026-09-23** (`b861294`, `ca97c2a`, `bb9d1ce`, and `a7948fd` for a Docker-tier test): **an app's launch is recorded once by the deploy that makes it true** (`projects.launched_at`, migration 0021) and a launched app is not rehearsed (`REHEARSAL_LAUNCHED`); **`deployRelease` reads ONE rule** — self-serve unless a sensitive field changed since the last approved release, fail closed with no baseline, never a rejected release; and **the egress proxy follows the release it serves**, proved through the platform on `launch-app` in both directions. The vulnerability database was refreshed at the open (Rich's yes). Four of the plan's control predictions were wrong, and the owed Docker tier found a P4c test being rate-limited by the edge it tests — fixed. **Sitting 4 — Task 6, alone — RAN 2026-09-23** (`ea76321`, and `6329275` for the test file's own hygiene): **D9.2 is reachable through the ROUTE** — a launched app's owner deploys a non-sensitive release to production with no administrator, a sensitive one is refused `409 RELEASE_REESCALATED` carrying a checklist byte-identical to the read and deploys once approved, a rejection is final, and a release that is not the one serving staging is refused `409 RELEASE_NOT_STAGED` (`[M8]`, closed). It found and fixed two defects outside the plan — **a staging whose only deploy FAILED offered the failed release as the launch candidate**, and the file's tests left a retire pass racing the next test's `TRUNCATE` (`deadlock detected` in 2 of 11 runs) — and found the plan's control (f) wrong about its mechanism: a field left out of a representation is STRIPPED from both answers, so byte-identity and `tsc` cannot see it. **Sitting 5 — Tasks 7 and 8 — RAN 2026-09-23** (`7edb819`, `bd2a00a`, `3ae1cfd`), both tasks without stopping after Task 7: **§9's IAM change request is the registration's own `change_requested` state** (migration 0022) — once UBC has registered the SP, what it registered changes only on a record that reaches `active`, so `[M9]`'s overwrite is refused and its consequence (an unregistered attribute BUILDING) watched; **one statement of *covers*** makes the first launch compare the recorded ACS and SLO (`[M10]`) and a launched app's `iam-registration` the LIVE check — an added attribute waits for UBC, a removal re-escalates and waits for nobody, a lapsed registration stops everything; and **R4(d)** — the approval snapshot names its baseline, the sensitive fields, a deterministic security note for each and D33's coverage limit, the reviewer is asked before the model, and `code-review` reads the newest verdict. Its controls found the plan's Task 8 control (a) not `tsc`-only: every success body is parsed, so a missing note is a `500` — after the decision is written. **Sitting 6 — Tasks 9 and 10 — RAN 2026-09-23** (`549dda4`, `600d9f0`, `e89c1c5`), in one sitting so the console's Approve button was never broken across a session boundary: **the stored preview** — an administrator takes a preview (migration 0023, thirty minutes, no step-up), approve and reject must NAME it (`400 APPROVAL_PREVIEW_REQUIRED`, `404` for another release's, `409 APPROVAL_PREVIEW_EXPIRED` / `APPROVAL_PREVIEW_STALE`), and the record COPIES its snapshot, so the model is asked once, when the preview is taken — measured with a model that answers differently on every call; **and the approvals screen shows the preview BEFORE the decision**, through the component the record uses, with its id in the URL so the step-up returns to the same preview. Its first suite run found a pre-existing defect — **the rehearsal compared Postgres's clock with the host's**, red 8 then 0 — fixed with a deterministic test; and clicking found **the mock accepting a decision that named no preview**, which it now refuses. **Rich answered its three questions on 2026-09-22**: **SEVEN sittings** (the lean split; eight was recommended); **removing a CWL attribute does not wait for IAM, adding one does** (which approves the plan's one spec action, on §13 D9.2's sentence, in substance — **not yet applied**); and **a sensitive change never returns an approved PIA to `draft` automatically**. **Writing it found six premises false in the code**, each now a Task 1 measurement: `isSensitiveDiff` cannot see a raised production override; the baseline counts a release approved and then rejected; a release freezes the project's NEWEST spec rather than its build's, and accepts another project's build; a rehearsal after launch puts an unapproved release on the live public listener; the gate evaluates the candidate and deploys whatever release is named; and recording an IAM change request overwrites what UBC registered. **Its design**: "launched" is stored (`projects.launched_at`, written once by the first healthy launch deploy); one rule, `approvalRequirementFor`, read by the checklist and by `deployRelease`, compares the release's frozen PRODUCTION config with the last APPROVED release; the gate answers `RELEASE_REESCALATED` or `RELEASE_NOT_STAGED` beside P6a's code; the stored preview binds facts and never the model's words; the IAM change request is the registration's own `change_requested` state; the person-only class is one central rule with its own code (`TOKEN_PERSON_ONLY`); and the contract goes to `1.1.0`, covering P6a's seven additions too. **Acceptance:** `make demo-releases`. *Its inputs, as recorded before it was written:* **It inherited Rich's 2026-09-22 decisions — build the PERSON-ONLY class (`release:approve`, `launch:record`: no token may hold them, no pending action grants them), and §13 names no number of sensitive fields — and, from P6a's execution (2026-09-22):** (1) **Rich's decision — the administrator sees the diff BEFORE deciding, as a STORED preview the approval binds** (the approve request names the preview; the platform refuses if what it would store now differs); (2) **an approval is bound to a RELEASE in code and to a DIGEST in §13**, and the builder is reproducible, so an identical rebuild is a new release needing a new approval — decide which one re-escalation reads; (3) the `code-review` checklist item never reads the reviewer's verdict, which the first real reviewer will need; (4) `make demo-production` re-uses `launch-app` and so is P6b's natural starting state. | D9.2: self-serve production redeploys, sensitive-diff re-escalation (`isSensitiveDiff` gets its first caller), the `diff_snapshot` and its **security-aware** AI summary carrying the reviewer's verdict, and the `auth.attributes` → IAM change-request path (§9). **Demo:** a self-serve redeploy, then a sensitive change refused until approved. **D5's GitHub source driver is placed immediately after this plan** — see below for why the ordering is a security argument rather than a dependency |
...
### Tracked tooling items — small, not a plan of their own

Work on how the platform is TESTED rather than on what it enforces, kept here for the same reason as the table above:
so it is not lost between plans. **A plan that touches the named test helpers should fold the item in and delete its
row.** It is a separate table because the one above is for security rules the spec describes and nothing enforces,
and its columns do not fit a test's speed.

| Item | Measured | Touches | Needs | Why it is not done yet |
|---|---|---|---|---|
| **The Docker tier's speed** — `pnpm test:docker` takes ~26 min because its 31 files run one at a time over one database, edge, IdP and registry, and **four files are 60% of it**. Ranked candidates: **build each fixture image once per run and share it** (~3–5 min, estimated); **cut what `boot` and `releases/production` spend 384 s of setup on**, once it is measured (up to ~3 min); shorter never-ready and denial-probe timeouts (under a minute together); and — **Rich's call** — owing the full tier less often. **No test is recommended for deletion.** Full write-up: [`2026-09-23-docker-tier-speed-brief.md`](./2026-09-23-docker-tier-speed-brief.md) | P6b sitting 5's run, 2026-09-23: 198 tests, 1575 s; per file, split into setup and tests | `runtime/docker/testing.ts`, `releases/testing.ts`, the tier's global setup, and the four slowest files | **Its first step is a measurement**: time each phase of `boot`'s and `production`'s setup and count the builds per run — the two obvious explanations for `boot` (its compiles, a fixed drain) both measured false | Raised by Rich on 2026-09-23 as a question, and written up at his request; not scheduled. Candidates 1–4 fold into any plan that touches the Docker test helpers; candidate 5 is a policy decision |
...
### D5's driver 2 — the GitHub source driver. PLACED AFTER P6b, 2026-09-19 — **WRITTEN 2026-09-24: [`2026-09-24-d5-github-source-driver.md`](./2026-09-24-d5-github-source-driver.md), 15 tasks in Rich's eight sittings. SITTING 1 — THE MEASUREMENTS — SITTING 2 — THE BUILD PATH AND THE KEY'S CUSTODY — SITTING 3 — THE FAKE, ITS CONTAINER AND THE CONFORMANCE RUN — AND SITTING 4 — THE DRIVER AND ITS WIRING — RAN 2026-09-24** *(which sitting is next: the plan's sittings table, and ORIENTATION §7e)*
...
- Git over HTTP needs only core git's `--stateless-rpc`; Alpine's git 2.49.1 has no `git-http-backend`, and none is needed.
- **A token handed to git in argv or in the URL leaks into the `SourceError` message that goes on the wire**; only `GIT_CONFIG_*` in the environment does not.
- **The build's secret gate misses GitHub's stateless `ghs_APPID_JWT` installation token** (rolled out from 2026-04-27), which this plan puts into circulation.
- A platform container reaches `127.0.0.1:7100` through `host.docker.internal`, and is refused `403` at the edge, so webhooks take the edge's own route and the edge does not change.
- **A free organisation cannot protect a private repository's `main`** (GitHub's own description), and **an installation token cannot create a repository on a personal account** — so `Manifest (local dev)` must live on a free ORGANISATION.
...
**SITTING 1 — Task 1, the measurements, alone and first — RAN 2026-09-24** ([`spikes/d5-baseline/`](../spikes/d5-baseline/README.md); the count is in the defect-rate table). **Every premise above held**, most of them exactly — and driving them found what the plan did not know: **after ONE rewrite of GitHub's `main`, every later non-forced fetch reports the same refusal, so a normal push is never an update** — never scanned for secrets, validated or reported — and the driver's `headCommit` would answer the frozen `main` as current for ever. The fix (a second, forced refspec into `refs/manifest/upstream/*`) is measured and corrects Tasks 7, 9 and 11. **Task 13's check withholds 7 summaries in 10 under Task 13's own prompt, and none of them states a verdict** (backticks round a hostname; *"previously blocked"* describing egress) — over the plan's own line. **Rich decided it the same day: the summary becomes STRUCTURED OUTPUT** — the model fills a JSON schema with one sentence per change and no field a verdict could go in (measured 10 of 10 in shape, 0 decision words), and Task 13 is rewritten for it, with Decision 22 making a schema the rule for every platform model call whose answer is read. **He also switched the chat model to `qwen3.5:4b`**, a thinking model run with thinking off — Ollama's `think: false` pinned in the LiteLLM mapping, which a request cannot override, without which it streams no content at all — and added `default-chat-reasoning` and `default-chat-onprem-reasoning`, the same model with thinking on. Its two spec actions are listed above, approved and APPLIED the same day. F9 itself reproduced (3 in 10, twice). **The contract first moves in Task 2, not Task 8**, which takes the `1.2.0` bump. The rest: Fastify is 5.12.3; a bare `compose down` exits 0 with the profiled fake still running (so `make down` and `make reset` name the profile); two of GitHub's schemas do not compile in Ajv without a `nullable` normaliser; an inherited `GIT_TRACE_CURL` file can hold a token. **No sitting boundary moved; Rich's eight stand.**

**SITTING 2 — Tasks 2 and 3, the build path behind `SourceDriver` and the App key's custody — RAN 2026-09-24** (`5758330`, `ef464a3`; the count is in the defect-rate table). **The roadmap's finding 2 is closed by the compiler**: `RepoRef` carries no path, so the builder and the code reviewer ask `SourceDriver.localGitDir` for the commit, and `startBuild` refuses one the repository lacks with `409 SOURCE_COMMIT_NOT_FOUND` rather than a build that fails at `git archive`. `describeSourceDriver` is D5's contract, eleven cases; driver 1 passes it and driver 2 must pass it unchanged. **§20's *same custody class* is one rule with two callers** — a control plane handed a `644` master key refuses to boot, measured — and `make up` mints the fake App's four credentials beside the master key. The contract is `1.2.0`. What executing found: the plan's driver left `SOURCE_FOREIGN_REPO` registered and unthrowable (retired); **registering an error code moves `openapi.json`** (the `ErrorCode` enum is the registry — sitting 1's F8 in a second shape); two pieces of the plan's code let a raw `TypeError` or `node:crypto` error escape where a coded refusal belongs; its mint script contradicted its own Step 5; and the approval preview can now answer a code its operation does not list, reachable only after a force-push on driver 1, which Task 12 closes. **No sitting boundary moved.**

**SITTING 3 — Tasks 4, 5 and 6, the fake, its container and the conformance run — RAN 2026-09-24** (`038cdf4`, `f10f895`, `a874895`; the count is in the defect-rate table). **Rich's decision is built**: `@manifest/github-fake` answers App JWTs, stateless installation tokens scoped to repositories and permissions, an organisation's repositories and git over HTTP; every JSON answer it gives is held to GitHub's OWN schemas; it runs as `manifest-github-fake:local` behind `--profile github` on `127.0.0.1:7110` (`make seed` builds it; `make down` and `make reset` name the profile), and `make doctor` and `make verify` grew to 20 and 57. **The real leg ran, at Rich's yes** — `Manifest (local dev)`, App 5068172, both repositories deleted — and `golden.json` now holds GitHub's MEASURED answers: three differed from the documentation and the fake was corrected (a token's permissions answer adds `metadata: read`; a read-only push is refused *"Write access to repository not granted."*; and creation needs `administration: write` alone). **That last one met Spec action 3's condition: a token SCOPED to another repository CREATED a repository** — offered with three options, and **Rich chose (b) the same evening: §20's bullet 1 is corrected** (the section above). What else executing found: the plan's code had a `tsc`-only error and a `500` where a `401` belonged; GitHub's schema does not require `visibility`, so one of the plan's controls could not show what it claimed; the fake's base is Alpine 3.24.1, so its git is 2.54.0, not 2.49.1; and `[M19](f)` has no step in the conformance script (owed to sitting 8). **No sitting boundary moved.**

**SITTING 4 — Tasks 7 and 8, the GitHub driver and its wiring — RAN 2026-09-24** (`ac96f7b`, `f51bd54`, and the fix `c47bdf0`; the count is in the defect-rate table). **D5's driver 2 exists and boots**: `createGithubSourceDriver` keeps a mirror at driver 1's path, fetched with two refspecs so one rewrite of GitHub's `main` no longer freezes it, holds installation tokens per repository and purpose in memory and hands them to git only through its environment, creates repositories private or deletes them, never adopts one, and answers `503 SOURCE_UNREACHABLE` offline while a mirrored commit still builds — **Task 2's contract suite passes unchanged against the fake**. `MANIFEST_SOURCE_DRIVER=github` boots it; `source_repositories` (migration 0024) records every project's provider, and `repositoryOf` refuses another driver's project `409 SOURCE_PROVIDER_MISMATCH` — **measured: without it, a GitHub-mode control plane BUILT a driver-1 project from its bare repository**, because both drivers keep repositories at the same root. A real BuildKit build ran from the mirror of a repository in the fake's image, and again with the fake stopped, to the same digest — and through the edge on a booted driver-2 control plane. **One defect of the sitting's own, found by measurement and fixed in its own commit: concurrent reads of one project after a push failed five times in six** until `sync` ran one fetch at a time per mirror. **No task boundary moved**; nothing of Rich's was needed.
...
| P6b Tasks 4, 5 and 5a (sitting 3, an app has launched, D9.2's second half, and the egress proxy that follows its release) | 3 | 17 | 5.7 |
| P6b Task 6 (sitting 4, the gate for a launched app) | 1 | 14 | 14.0 |
| P6b Tasks 7–8 (sitting 5, the IAM change request and the security-aware summary) | 2 | 13 | 6.5 |
| P6b Tasks 9–10 (sitting 6, the stored preview and the screen that reads it first) | 2 | 14 | 7.0 |
...
| D5 driver 2 Tasks 2–3 (sitting 2, the build path behind `SourceDriver` and the App key's custody) | 2 | 20 | 10.0 |
| D5 driver 2 Tasks 4–6 (sitting 3, the fake, its container and the conformance run — real leg included) | 3 | 17 | 5.7 |
