...
**Applied 2026-09-14, with Rich's approval — every row except the `MONGODB_DB_NAME` note**, which is history rather than design and stays recorded here. The §21 row went in reworded as a requirement, because `ssp_ro` has since been created, and §1's `docker-simple-saml` row was rewritten with the §9 change so the spec does not contradict itself. The table below is the original proposal, kept as the record. P3's own six were applied on 2026-09-07 with his approval, before this plan was written.
