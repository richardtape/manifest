...
**Status: ✅ FULLY EXECUTED AND GREEN — all 19 tasks, 2026-09-07.** `pnpm test`
**381**, `pnpm test:docker` **89**, `make doctor` **16 / 0**, `make verify` **34 / 0**.
`make demo` takes the fixture app from a bare repository to
`https://fixture-app.staging.manifest.internal` — and does it again from a dropped
database, a deleted repository root and an emptied registry. **Executing it found 82
defects across five sessions**, recorded per session in *What executing this plan
found*; read Sessions 4 and 5 first, since between them they establish that no build
and then no deploy in this platform had ever actually succeeded.

**Status when written: complete — 19 tasks, self-reviewed, and reconciled against a running P2.** No step in this plan stands in for a spike result. The two controls S1 left open were settled **before** it was written; see the next section. The self-review ran on 2026-09-04 and found seven defects, all fixed — *What the self-review caught*, near the end, records them.
...
Expected: 5 unit tests and **5** Docker tests green. The fourth Docker test is the one
that distinguishes this design from a shared allowlist: `example.com` is reachable
because **this app** declared it.
...
Expected: **FAIL** on **three** tests — both ALLOW tests and the DENY test. Restore it.

> **Corrected 2026-09-06, by running it.** This step predicted one failure, with
> *"every other test stays green"*. `FilterDefaultDeny No` does not merely stop
> denying: it **inverts the filter file into a denylist**, so `^manifest-verdaccio$`
> becomes a block rule and the platform baseline is refused too. Three failures is
> the correct signal; one would mean the ALLOW paths were not being exercised.
...
> **minted registry JWT** rather than the build credential (the daemon cannot reach
> the token realm — measured), and the image reference it is given is the one the
> **daemon** uses, not the in-network one.
...
### Session 2 — Tasks 5-8 (2026-09-06).

| # | Task | Defect | Measured against |
|---|---|---|---|
| 11 | 5 | The generated tinyproxy config omitted `DefaultErrorFile`, while the comment three lines above it said the proxy **exits after every denial** without one. | Measured on the 1.11.2 this image ships: 3 denials, 0 restarts, in three config variants — it does **not** reproduce. P1 measured it on **1.11.0** and its own proxy shows `RestartCount=0` after 16 h. Added anyway (one line, and the failure it guards is racy), with the version qualification the plan lacked. |
...
| # | Task | Defect | Measured against |
|---|---|---|---|
| 31 | 10 | **The builder never started.** `withEphemeralBuilder` overrides the entrypoint so the generated TOML can be written from inside the container — and the image's own `ENTRYPOINT ["rootlesskit","buildkitd"]` goes with it. The plan's `exec buildkitd …` drops rootlesskit. | Ran it: container **exits 1**, and all five Docker tests fail — including *"is rootless and NOT privileged"*, whose `ps \| rootlesskit` assertion never gets to run. Fixed with `exec rootlesskit buildkitd`, plus `builderCommand()` and a **fast-tier** test, so the next person does not need Docker to catch it. |
| 32 | 10 | **Nothing waited for buildkitd to answer.** `fn` was called as soon as the container had *started* — defect 17's shape, two tasks later. | With rootlesskit removed the failure surfaced as `POST /containers/…/exec failed (409): container is not running`, which names neither the config nor the cause. `awaitBuilderReady` now fails **fast** on an exited container with `BUILDER_NOT_READY`, the exit code and the daemon's last ten log lines. |
| 33 | 10 | **Step 9's premise is wrong, and the step was weaker than it needed to be.** It says a malformed `buildkitd.toml` is *"logged as a warning and otherwise ignored"*, so "the builder came up" proves nothing. | Measured on v0.32.2: `toml: expected character ]` / `failed to parse config`, container **exits 1**. So starting IS evidence the file parsed — and better, `buildctl debug workers -v` **echoes the GC policy back**. A default daemon reports **four** rules (512MB / 10GB / 100GB); ours reports **one**, `All: true` at `4.294967296GB`. Now a permanent Docker-tier readback. Control: `maxUsedSpace: '8GB'` fails it. |
| 34 | 10 | **The Docker test's token could not build anything.** `mintFor` minted one repository — the push target. A `registrytoken` in a docker config is presented verbatim for every scope, with no per-scope negotiation with the realm, so it carries no pull scope for the base image. | Measured: `FROM manifest-registry:5000/base/alpine:3.22` → **`pull access denied … insufficient_scope`**. `mint-token.mjs` now takes N repositories (backwards compatible — `doctor` and `verify` pass one). |
| 35 | 10 | `runBuildxBuild` leaked one temp `DOCKER_CONFIG` directory **per build**, each holding a live push token. Same class as defect 30, and as P2's 944. | Read the code path; there is no `finally`. Added one. |
| 36 | 10 | The buildkitd TOML hardcoded `manifest-registry:5000` while Task 15 passes `options.registryHost` to buildx separately — two copies of one value, and the failure if they diverge is an HTTPS handshake against a plain-HTTP registry, which reads as a certificate problem. | `withEphemeralBuilder` takes the host as an optional fifth argument defaulting to `DEFAULT_REGISTRY_INTERNAL_HOST`; a unit test proves the TOML follows it. **Task 15 should pass `options.registryHost`.** |
| 37 | 11 | **`.gitignore` SILENTLY SWALLOWED THE WHOLE MODULE.** Line 10 was `build/`, which matches `packages/control-plane/src/build/` — §5's build module. | `git add` refused it; `git check-ignore -v` names `.gitignore:10:build/`. Had `-f` been used out of habit the module would have been green here and **absent from every clone** — P1's open second-machine gap, and the identical shape to defect 29's blanket `*.key`. Anchored to `/build/` and `packages/*/build/`. |
| 38 | 11 | **`assembleContext` copied a file called `Dockerfile`, and no blueprint has one.** `blueprints/fixture-node/` ships `Dockerfile.tmpl`, named by `blueprint.yaml`'s `dockerfile:` field and carrying `{{BASE_IMAGE}}` and `{{RUN_AS_UID}}`. | `copyFile` would `ENOENT`; copying the template verbatim leaves `FROM {{BASE_IMAGE}}` for BuildKit to reject with an invalid-reference error that mentions no blueprint. Now rendered from the descriptor, with an unresolved placeholder refused **by name** (`BLUEPRINT_TEMPLATE_UNRESOLVED`). The tests use the real blueprint rather than a fixture with a shape this repository does not have. |
| 39 | 11 | **There was no `.npmrc` anywhere in the repository**, so the file the D13 control asserts the blueprint overwrites did not exist — and `npm ci` would have gone to the public registry. S1's silently-wrong build, arriving by a different route. | `grep -r npmrc` found only plan text. Added `blueprints/fixture-node/.npmrc` pointing at the mirror. The D13 control now asserts a real substitution; inverting the copy order fails it with `expected 'FROM attacker/image…' not to contain 'attacker'`. |
| 40 | 12 | **A SECURITY GATE THAT COULD NEVER FIRE.** The database age is read from `descriptor.db.built`. On grype v0.118.0 / schema v6 that key does not exist — it is `descriptor.db.**status**.built`. | Measured: `descriptor.db.built` → `undefined` → `databaseAgeDays` **Infinity** → every scan `stale` → and a stale scan **warns rather than blocks**, so the gate is off for ever while every test stays green. The Docker test now asserts the age is **finite**; control: restoring the plan's path turns it red. |
| 41 | 12 | **Blocking on Critical+High would have stopped every deploy on day one.** | Measured: `alpine:3.22` against a database built the same morning — **4 Critical, 14 High**, every one an `apk` package (`libcrypto3`, `libssl3`), every one fixed only by a newer base image. The base image is platform-owned and digest-pinned by the blueprint (D13), so no rebuild of an app clears one. §20 already says this and puts the remedy (fleet-wide rebuild) in Phase 4+. OS-package findings are now **recorded and reported, never blocked on**; the app's own dependencies still block. Control: removing the exclusion turns the real alpine scan red. |
| 42 | 12 | **The scanner cannot read the image from the registry at all**, which is what the plan has it do. | Measured, with and without a bearer token: `invalid realm in www-authenticate: realm host "127.0.0.1" is a private or link-local address`. go-containerregistry — what both Syft and Grype use — refuses it, and the realm **has** to be a host address because the buildx client on the host is what exchanges the token (Task 9). Mounting the Docker socket is the other obvious answer and is precisely what §12 forbids. So the daemon exports the image (`GET /images/…/get`) and the scanner gets a read-only bind of one tar with **`NetworkMode: 'none'`** — measured working, and offline by construction. |
| 43 | 12 | `--fail-on none` is not a value grype accepts. | `invalid application config: bad --fail-on severity value 'none'` — the accepted values are the five severities. **Every scan would have failed.** The flag is omitted entirely; grype does not fail on findings by default and `assessScan` decides what blocks. |
| 44 | 12 | `doctor`'s age check read `/db/*/metadata.json`, which the v6 database does not have. | `ls /db/6` → `import.json`, `last_update_check`, `vulnerability.db`. A perfectly good database reported as **absent**, telling the developer to run `make seed`. It now asks the tool: `grype db status -o json`. Also `date -j -f` without `-u` reads a `Z` timestamp as local time and shifts the age by the offset. |
| — | 12 | (trap recorded, so nobody spends a morning on it) **`X-Registry-Auth` must be PADDED base64.** | Node's `'base64url'` omits the `=` padding; the daemon's decoder rejects it, **silently falls back to anonymous**, and the error blames the token realm: `failed to fetch anonymous token … dial tcp 127.0.0.1:7100: connection refused`. The identical request with padded base64 pulls in under a second. `registryAuthHeader()` in `engine.ts` is the only place that builds it. |
...
### Session 5 — Tasks 16-19 and the close-out (2026-09-07). 21 defects.

***`make demo` had never run, and running it is what this session was.*** Session 4
established that no BUILD had ever succeeded. Session 5 established that **no DEPLOY
had ever succeeded through the control plane** — the Docker tier had been green
throughout, because every test in it constructs its own inputs and hands the driver
values the production path rebuilds by hand. Seven of the defects below are exactly
that shape: *the test built the value correctly and the running system re-derived it
wrongly.* Task 16 produced none; Tasks 17 produced sixteen.

**Task 16 was clean, and the reason is worth keeping**: it replaced an existing
check rather than adding one, and P2's own test — untouched — is what proved the
replacement was load-bearing. The negative control reddens *both* tests.

| # | Task | Defect | Measured against |
|---|---|---|---|
| 62 | 17 | The plan puts the fixture app's entry at `src/index.js`. The blueprint's `CMD ["node", "server.js"]` is not templated and `blueprint.yaml` has no entrypoint field. | Read before writing: the build would have succeeded and the container died on `Cannot find module '/app/server.js'` — a green build and a dead app. Entry is `server.js` at the tree root, and the fixture says so in a comment because the coupling is invisible from the app's side. |
| 63 | 17 | **The plan's `boots` counter rises on every REQUEST**, so *"its data survives a stop and a start"* passes whether or not the stop and start happened. | It discriminates a wiped volume and nothing else. Rewritten to be written **once per process start**: `after.boots === before.boots + 1` is then only satisfiable if the app really restarted AND the previous run's row survived. |
| 64 | 17 | **`npm install --package-lock-only --registry http://127.0.0.1:7108/`**, which the plan prescribes, writes `resolved` URLs pointing at the HOST-published mirror port — a host that does not exist inside the builder. | npm's default `replace-registry-host=npmjs` rewrites the tarball host **only when it is literally registry.npmjs.org**. So the plan's lockfile could not build, and worse, D13 was only in force for lockfiles that happen to name npmjs — a real app pointed at a company proxy would have gone straight there. Lockfile regenerated as a real app's is, and the blueprint `.npmrc` gained `replace-registry-host=always`. |
| 65 | 17 | **`waitForReady` and `edgeProbe` — the whole of Task 14 — had NO call site.** Nothing outside their own tests ever called them. | `grep` across `src/`: one export line and nothing else. Task 14's own text says §11's `starting -> healthy` "should mean" reachable at its hostname; nothing made it so. Found by the round trip: `stopInstance` then `ensureInstance` returned a handle while the app was still connecting, and the next request through the edge got Caddy's **502 with an empty body**. Now waited on inside `ensureInstance`, with `INSTANCE_NOT_REACHABLE` when it times out. |
| 66 | 17 | **`buildImage` IGNORED its own `blueprintRef` argument**, building everything from one fixed directory — which `src/index.ts` set to `config.blueprintsRoot`, the directory *holding* the blueprints. | `BLUEPRINT_DESCRIPTOR_MISSING: no blueprint.yaml in .../blueprints`. No build through the control plane could ever have worked. Invisible to the Docker tier, whose driver is constructed with a single blueprint's directory. P2's registry already exposed `pathOf(ref)`, described there as *"the builder needs it for the Dockerfile"*. With one blueprint the ignored argument is harmless; the moment P4 adds `node-ts-mongo@1` it silently builds every project with the wrong Dockerfile and base image. |
| 67 | 17 | **A FAILED EXPORT REPORTED SUCCESS.** `assembleContext` ran `git archive … \| tar -x`, and a pipeline's exit status is the LAST command's. | Measured: `git --git-dir=file://… archive HEAD` exits **128**; `sh -c 'git … \| tar …'` exits **0**. So every way `git archive` can fail produced an EMPTY context and reported success, `SOURCE_EXPORT_FAILED` could not fire, and the failure surfaced as whichever §12 gate noticed first — *"package-lock.json is missing"* for a repository whose HEAD plainly has one, which sends the reader to the app. Two processes now, with a test for each failure mode. |
| 68 | 17 | The delivery route handed the driver **`file://${reposRoot}/${slug}.git`** where `git --git-dir=` needs a filesystem path. | `fatal: not a git repository`, swallowed by 67. Every test passed: they all construct a path themselves. The field was even named `repoUrl`. Now `deps.source.repositoryFor(slug).path` — D5's interface gained the lookup it was missing, which is why the route was hand-building a URL at all. |
| 69 | 17 | **`deployRelease` re-derived the image repository as `local/<slug>`**, and an unqualified name is DOCKER HUB to the daemon. | `failed to resolve reference "docker.io/local/fixture-app@sha256:…": 401 Unauthorized`. `buildImage` returns `127.0.0.1:7107/local/<slug>`; the deploy rebuilt a different string. §13 binds an approval to a digest, and a digest is only half an image reference — so the other half is now RECORDED on the build row rather than reconstructed. Task 16's `isLocallyBuilt` matches on the path precisely for this. |
| 70 | 17 | **NOTHING INJECTED `PORT`.** The platform picks the port — it is what the container HEALTHCHECK probes and what Caddy uses as the upstream — and the app was never told. | A manifest declaring `runtime.port: 8080`; the app logged `"port":3000` and the deploy timed out. It had worked only for apps that happen to hardcode the blueprint's `default_port`. `PORT`, `MANIFEST_ENV`, `MANIFEST_PROJECT_SLUG` and `MANIFEST_APP_URL` are now applied AFTER the app's own env, so a declared `PORT` cannot break its own routing. §8's full contract stays P4's. |
| 71 | 17 | **EVERY APP'S HEALTH CHECK WAS BROKEN BY THE EGRESS PROXY.** D18 puts `http_proxy` in the container; BusyBox wget honours it and **supports `NO_PROXY` not at all**, so the probe of the app's OWN loopback went to tinyproxy. | `wget: server returned error: HTTP/1.1 403 Filtered`, a growing `FailingStreak`, and §11 recording a perfectly healthy deploy as **failed**. Measured inside a running container, BusyBox v1.37.0: plain wget exits 1, `wget -Y off` exits 0. Loosening the proxy allowlist was rejected — an app reaching itself is not egress. **Nothing had ever asserted `healthy === true` against the real driver**; the contract suite checks `state`. It does now. |
| 72 | 17 | **`startBuild` threw the build error away** — literally `void error`. A failed build was a row saying `failed`, with a `logsRef` naming a log store that does not exist. | §14 makes a failure a row so it can be SEEN, and the row said only "no". The only way to learn why a build failed was to re-run it outside the platform. An `error` column now carries code, message and hint; it named every subsequent defect in this list within one run. |
| 73 | 17 | **An unexpected 500 left NO TRACE ANYWHERE.** The error handler called `request.log.error`, and the server is built `Fastify({ logger: false })` — under which that method exists, accepts the call and writes nothing. | The identical trap Session 4 recorded for `app.log.info` on the boot line, in the one place it matters most: a deploy answered `INTERNAL` with nothing in the process output, nothing in the database and nothing on the wire. `console.error` with the stack, for the OPERATOR; the client envelope stays opaque (D23.7). |
| 74 | 17 | `BuildGateError`'s message said *"build blocked by 1 mandatory gate finding(s)"* and did not name the finding. | A build failure that does not say what to change is one the author has to reproduce elsewhere to understand. The findings are already written for them and already redacted. |
| 75 | 17 | **No route could re-validate `manifest.yaml`.** Project creation was the only caller of `validateSpec`, so a project's spec was fixed for its whole life. | An agent could push a manifest declaring a database and the platform would never read it — which is what `make demo` needs. `POST /projects/:id/spec` added. It also gives **`isSensitiveDiff` (D9) its first call site outside its own unit test**: reported, deliberately not enforced, because the escalation it feeds is P6's and a gate with nothing behind it is a control on paper. |
| 76 | 17 | Creating a project whose slug is taken answered **500 INTERNAL**. | The unique violation reached `toErrorResponse` as an unknown error. A `ProjectError` now puts it in the 409 family. Found only because defect 73's fix made the trace visible. |
| 77 | 17 | The round-trip suite and `make demo` both used the slug **`fixture-app`**, and a slug plus a kind IS the deployment — hostname, route id, network, service. | The suite's `afterAll` removed the demo's route and destroyed its database with `deleteData: true`; a healthy app went `unhealthy` a minute later and the suite failed `INSTANCE_NOT_REACHABLE` against a route the demo owned. The suite is `fixture-rt`. |
| 78 | 18 | **S6's first run failed on its two POSITIVE controls**, because the probe forced `--dns 10.89.0.53` on `bridge` containers too — and that address lives on the platform network. | Both controls returned **exit 6**, "couldn't resolve host", indistinguishable from the denial they exist to disprove. Had the same mistake been made on the denial side, the matrix would have printed twelve confident greens measuring nothing. This is the failure mode the whole task is about, arriving inside the task itself. |
| 79 | 17 | **The plan's offline control could not have run.** `mint-token.mjs` mints `['pull','push']`, and a registry DELETE needs the `delete` action. | **401**. The image would have stayed, the build would have succeeded, and "offline is proven" would have been the conclusion — which is precisely what the plan's own two warnings about this control were guarding against, one warning short. `--actions=` added. |
| 80 | 17 | **`registry garbage-collect` on a RUNNING registry corrupts it**, and the plan's procedure runs it live. | Upstream requires read-only or stopped. Run live it deleted the manifest blob while leaving the tag and revision links, and every later `docker push` of the same content reported success **with the correct digest** while the registry answered 404. A restart before re-mirroring clears it. Recorded in the runbook. |
| 81 | 17 | The plan says the `Accept` headers are required. They must also be **separate `-H` flags**, and the mirrored base is an **OCI** manifest. | Comma-joined, this `registry:2` answers 404 `OCI manifest found, but accept header does not support OCI manifests` — which reads exactly like "the image is missing", and did, for two rounds of chasing a restore that had actually worked. |
| 82 | 19 | **`make verify`'s four builder checks had not run since Task 10** — gated on `docker inspect manifest-buildkitd`, a compose service behind a profile nothing starts. *(Inherited: named at the end of Session 4.)* | Fixed by DELETING the compose service rather than starting it. A second, permanently-running definition of the builder drifts from the ephemeral one the driver creates, and verify would then assert the wrong object's properties. Verify keeps the topology check (S1's open `--internal` question, asserted every run); the builder's own properties live in `builder.docker.test.ts`, against the container the driver really creates. |
| 83 | 19 | **`make doctor` failed during the platform's own documented flow.** §21 puts the control plane on the HOST; doctor's port check reads published CONTAINER ports, so 7100 was "CLAIMED BY SOMETHING ELSE" whenever the control plane was up. | `make up` -> start the control plane -> `make demo` -> `make doctor` -> FAIL. It now identifies the port by ASKING it — the D23.7 envelope on `/auth/me` is this application answering, where "node on 7100" would be a guess. Watched failing against a foreign listener on the same port. |

**Controls watched failing, so nobody re-checks them.** Removing
`assertPromotable`'s capability test reddens the new promotion test **and P2's
untouched one**, which is what proves Task 16 replaced P2's guard rather than
shadowing it. Restoring the `git archive | tar` pipeline reddens both export-failure
tests. Pointing a deploy at a port nothing listens on reddens the new readiness gate
with `INSTANCE_NOT_REACHABLE` — a container that is up, healthy and routed, with only
the upstream port wrong. A foreign listener on 7100 reddens doctor's port check even
with the control-plane exemption in place. And the S6 matrix's positive controls
reddened on their own, first time, for a real reason.

**The offline claim, controlled.** Deleting the mirrored `base/node` makes the build
fail `failed to resolve source metadata for manifest-registry:5000/base/node@sha256:1ef15d33…: not found`
— and it does **not** fall back to Docker Hub, because §12 leaves the builder no route
off its network. `bash infra/seed/mirror-images.sh` restores it with no network at
all, to the exact digest the blueprint pins.

**`make reset` proven, not assumed.** After a reset: `mf-` containers=0 networks=0
volumes=0; `docker-simple-saml-saml-idp-1`, `qdrant-local-dev`, `mongodb` and
`mongo-express` all still present; `manifest-caddy-data`, the npm mirror cache and
the Grype database all kept. Then `make up && make demo` took a **dropped database, a
deleted repository root and an emptied registry** back to a healthy app at its URL.
...
|---|---|---|---|
| §21, divergence 8 | *"Workload containers can reach the developer's own machine… the host remains reachable"* | Narrow to: the **app** networks are `--internal` and have no route to the host gateway; the divergence now covers **platform and builder containers only**. | Task 4 makes every app network internal, so §12's east-west denials are topology rather than policy. **MEASURED 2026-09-07 by Task 18, so the deferral is lifted and this is now ready to apply:** from an app network both `host.docker.internal:27017` and `:6333` fail at `curl` exit **6** — the name never resolves — while `:6333` answers exit **0** from a bridge container, so the ports are genuinely open and it is the app network that has no route. See `spikes/S6-findings.md`. |
| §12, *isolation* | `isolationLevel` is an open question | Record the measured answer for **staging and production apps: `container`** — shared kernel, no gVisor, no Kata — and state explicitly that the **sandbox** question stays open until S5. | Measured by Task 18, and only for what was tested: those probes ran against a *staging* app, and §11 gives sandboxes `exec`, a wider egress baseline and session-scoped AI keys that do not exist yet. Answering the sandbox question on the strength of a staging app is the class of claim this project has been burned by twice. |
...
| §16, *Security regression* | *(the tier)* | Add the S6 matrix's positive controls as part of the tier: each denial is paired with the same probe succeeding from a bridge network — **and an unpairable control must be reported as unpaired rather than omitted**. | A matrix of denials with no positive control looks identical to a matrix where the probe tool is missing. Task 18's first run produced exactly that, on the control side, where it was loud; probe 7's control additionally needs the internet, which the rest of P3 does not, so the tier says out loud when the pairing could not be made. |
