...
### Sitting 8 — 2026-09-18 — Task 12, `make demo-token` — 16 findings
...
| F12 | **The post-sweep §7e re-read found a step count with two values.** §7e said the RUNBOOK's *`make demo-token`* section *lists* the demo's **nine** steps; the RUNBOOK numbers **seven** grouped items. A cold agent counting them would find the hand-off wrong about the thing it was pointing at. Both now say the run prints nine and that the RUNBOOK groups them into seven. Found by opening the list and counting it, which is §6's *count what you summarised* — re-reading the sentence would not have. |
| F13 | **And a claim about this plan's own record that was wrong at BOTH ends.** §7e as first written said this plan *"has now found one \[a control that could not fail\] in every sitting but the first two"*. **Sitting 2's F14 found one** — the TRUNCATE control the statement's CASCADE makes unfailable — so the lower end was wrong; and **sitting 8's is a different shape**, a control that fails but does not fail informatively (F6), so the upper end was wrong too. Sitting 7's own F2 had already said *sixth consecutive*, counting from sitting 2, which is the value that should have been checked against. §7e now says *from the SECOND onwards*, names no count, and points at this record as the only statement of it — the structural fix F13 and F14 of the last sitting prescribe, applied to a claim written in the same breath as them. |
| F14 | **And a THIRD restated count, pre-existing and stale for two sittings.** ORIENTATION §2's P5b paragraph said *"and **six sittings are done**"* while the same file's opening box said EIGHT — sitting 7 moved the box and left the paragraph, so the file disagreed with itself for two sittings. Same class as sitting 7's F13 and F14, and the reason those were fixed structurally rather than by correcting the number: **correcting a value leaves the second copy, and the second copy is what drifts.** The paragraph now names no count and points at the box. *The general form, for the next sitting: after fixing one restated number, `grep` for the PHRASE — not for the number — because the stale copy carries a different number by definition.* **Doing exactly that then found a FOURTH**: `README.md` enumerated P5b's sittings in TWO paragraphs, and the one that is not the P5b row still ended *“Sitting 8 — Task 12's `make demo-token` — is the current work”*. That paragraph has stopped enumerating them. The roadmap's *Order of operations* also matched the grep and was correct — its *“Sittings 1 to 7”* is P4c's, not P5b's. |
| F15 | **THREE MORE from the post-sweep check, and one of them DISARMED THE TRAP §7e exists to arm.** The state table said *"`student` (`stu000001`) HAS signed in on this database"* — `users` in fact held only `ins000001` and `opr000001`, because the GATES' `pnpm test` ran after the last `make demo-token` and truncated it, and the closing `make demo-journey` signs in only the instructor and the operator. So the hand-off told the next agent not to worry about the one thing §7e names as most likely to cost them the sitting. The same truncation falsified two more rows: the database holds `journey-app` **and nothing else** (`token-app` has a running, serving container and no project rows), and only `journey-app` has a `Route` row, so `token-app`'s live Caddy route would not survive an edge restart. **The generalisation: a state table written from what the sitting DID is wrong whenever a gate ran afterwards. `pnpm test` is the last thing most sittings run and it truncates — so write the state table from a QUERY at close, never from the sitting's own narrative.** |
| F16 | **A number with no metric reads as a contradiction.** The hand-off said `docker images` **132 → 46**; `docker images -q | wc -l` answers **53** on the same machine, because it and `docker system df` count dangling and intermediate images differently — and the figure was stale by one anyway, since the fixture app's own image went after it was written. A cold agent checking it would conclude the hand-off was wrong rather than that two commands mean different things. **Name the command beside the number**, which §2's gate box already does for the four gates and nothing did for this one. |
...
### Sitting 9 — 2026-09-18 — Task 13, the acceptance — 11 findings
...
| F9 | **THE POST-SWEEP CHECK, FIRST OF THREE: a claim about §8 that counting refuted.** This sitting's own §7e told P5c's author that *"two of §8's open questions are P5c's to make concrete"*. **§8 has ONE** that P5c forces — an app route's `stream_close_delay`, which is Rich's; the rest are an init flag, a base-image bump, an apk mirror, SimpleSAMLphp's superuser, `passport-ubcshib`, a LiteLLM bug and the external track, none of which P5c makes concrete. The error came from conflating §10's *Decisions P5c must make* — which does list two — with §8's list, **and the sweeper, §10's other one, is not in §8 at all**, because it is the plan's decision rather than Rich's. Found by opening §8 and counting its bullets, which is §6's *count what you summarised*; re-reading the sentence would not have. |
| F10 | **SECOND: *"whose first paragraph is that §17's Phase 1c row is NOT P5c's scope"* — and it is not the first paragraph.** §10's opening paragraph is about §2 and §4 of the brief being STALE; the Phase 1c statement is three paragraphs later, under the subsection *The one thing to get right first*. **The claim was written by sitting 8, in Task 13's own Step 6, and this sitting propagated it into FIVE documents** — ORIENTATION §7e and §3, `README.md`, `CLAUDE.md` and the roadmap's P5c row — before the check opened §10 and read it. A cold agent told to read "the first paragraph" would read the staleness warning, conclude the hand-off was wrong about the thing it was pointing at, and lose confidence in the rest. All five now name the SUBSECTION. **It was found by grepping the PHRASE, not the fact** — sitting 8's F14 rule, and the fifth copy (§3's, which says *"the first paragraph is the one that saves the plan"*) would have survived a grep for any of the words that differ between them. |
| F11 | **THIRD, and the smallest: this table's own findings ran F1–F6, F8, F7.** F8 was inserted above F7 rather than after it. Nothing depends on the order, which is exactly why nobody would notice — and a reader counting findings to check the header's *8* against the table is the reader who trips. Counted, and swapped. |
