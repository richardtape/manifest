...
| §11, *Execution model*, and §13's deploy | nothing on redeploy continuity | **Decided 2026-09-14 by Rich; the wording is to be written with its plan:** a redeploy does not interrupt the app. The new instance is ready before the edge route moves, the move is atomic, the old instance drains, and only then are its AI key revoked and its container retired. | Measured the same day: every redeploy 502s the whole app while the new container starts (finding 74), and a key revoked at mint time fails the old instance's AI calls. Rich: not acceptable. |
