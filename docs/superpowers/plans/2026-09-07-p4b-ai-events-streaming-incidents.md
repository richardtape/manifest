...
**ALL TEN SITTINGS ARE COMPLETE — P4b IS EXECUTED, 2026-09-15. P4c, zero-downtime redeploys, HAS SINCE BEEN WRITTEN (2026-09-15) AND IS BEING EXECUTED — sittings 1 to 5 of eight are done, and sitting 6 (Tasks 8-9) is next. This plan is finished; go to [`2026-09-15-p4c-zero-downtime-redeploys.md`](./2026-09-15-p4c-zero-downtime-redeploys.md), and to ORIENTATION §7d-3 for the hand-off.** Sitting 10 (Task 16, 2026-09-15) produced **22 findings**. §16's proof app answers a question from the asker's own notes, charged to `sha256(puid ‖ project ‖ environment)`, and `make demo-ai` proves it — the event stream, a streamed answer, a 768-dimension embedding, and LiteLLM's own spend log — green from a `make reset` machine, twice. Its first run failed at step 9 on its own guess, and the measurement behind the fix found that **the blueprint's `embed()` charged every embedding to nobody** (finding 177, fixed). Negative control (e) found that **an app whose gateway vanishes under a pooled connection makes a student wait 611 seconds** (181, named for P4c and the blueprint). The external-track trigger has fired and was raised with Rich, not started. Its record is the last entry in *What executing this plan found*. Sitting 9 (Tasks 14–15, 2026-09-15) produced **18 findings**. `WS /projects/:projectId/events` exists and is authorized in a route hook BEFORE the upgrade — **the plan's code authorized inside `wsHandler`, after the upgrade, and could not pass its own test** — and every build, deploy, Incident and AI key rotation reaches it through one `publishEvent`, which records the row and publishes it as stored; P4a's two SSO events go through it too. `audit.events` gained `clock_timestamp()` and a database CHECK on its closed set of nine types (migrations 0007 and 0008), and a deploy now opens the app's secret set before it mints anything (finding 162). **It wrote a note at the top of Task 16 — read it first.** Sitting 8 (Task 13, 2026-09-15) produced **20 findings**. It settled finding 133 first — a Mongo service is now healthy only once it enforces authentication, with the race forced in the Docker tier by a 20 s init script — and then built §14's `Incident`, after finding that **the plan's caller position was unreachable on the Docker driver**: its readiness refusal threw with no handle, so an app that crashed as it started produced no record and left its row in `provisioning`. A refusal now carries its handle and is recorded as a failed instance with an Incident, on both drivers, and a failed deploy is a `200`. **It wrote a note at the top of Task 14 — read it first; one item in it corrects Task 14's own code.** Sitting 7 (Tasks 11–12, 2026-09-14) produced **16 findings**: a build's output reaches `audit.build_logs` line by line while it runs, redacted, and redaction has its heuristic half — and **migration 0005 applied without its grant**, because a `$(ls …)` captured an aliased listing, and was replayed from the file as it ships. Two of its 27 negative controls came out green, and each forced a test that goes red. Its Docker tier ended 130 of 131 on a Mongo readiness race (finding 133), which sitting 8 settled. Sitting 6 (Task 10, 2026-09-14) produced **14 findings**: `node-ts-mongo@1` provides AI, its component is proved on the wire against the real toolkit, and **seed's npm warm turned out to be inert on any machine whose npm cache already held a package** — fixed in `infra/seed/seed.sh`, and caught only by `make verify`. It wrote one note at the top of Task 11, and **a pre-flight read the same evening wrote eighteen corrections at the tops of Tasks 11 and 12 — Task 12 as printed fails two of its own tests and would redact every image digest in a build log.** Sitting 5 (Tasks 8–9, 2026-09-14) produced **11 findings**, built Rich's key decision and both §7 decisions, and made the decision its hand-off named — a release that declares models, deployed with AI switched off, is refused with `RELEASE_AI_DISABLED`. It wrote four corrections at the top of Task 10, all resolved in sitting 6, and one at the top of Task 15, which is still to be read before Task 15. Before it, a pre-flight read of Tasks 8 and 9 wrote **fifteen corrections and a note** at their tops, all resolved. **Rich then decided, the same day, that no live AI call may fail because a deploy revoked its key, and that redeploys must become zero-downtime for the whole app in a plan of their own** — the interim Task 9 builds is the *DECIDED* block at the top of Task 9. **He also settled two §7 spec actions that change Task 6's and Task 7's committed code** — the *SPEC DECISIONS* block, also at the top of Task 9. Sitting 4 (Tasks 6–7, 2026-09-14) produced **12 findings** and made the decision its hand-off named — AI is on unless `MANIFEST_AI_ENABLED=0`, and on means a boot with no LiteLLM master key is refused; read its entry in *What executing this plan found* before Task 9, which inherits that decision and the two bound services it has to wire. Sitting 1 (Tasks 1–2,
...
| 8 ✅ | 13 | §14's `Incident` — the failure shaped as a repair prompt. **Alone**: it consumes Task 12's redactor plus `Driver.logs` and `Driver.status`, and it can only be proved against containers that have really failed, which is slow Docker-tier work | ✅ **done 2026-09-15, 20 findings** — finding 133 settled first and made deterministic; the plan's caller position turned out to be unreachable on the Docker driver, whose readiness refusal now carries its handle and is recorded as a failed instance; 17 negative controls |
| 9 ✅ | 14–15 | `WS /projects/:projectId/events`, then the call sites that make it carry anything. **Never split these.** Task 15's own title is *"the task that makes the stream carry anything"* — a stream with no publisher is the call-site defect again, and it would sit green for a whole session | ✅ **done 2026-09-15, 18 findings** — the stream authorized in a route hook before the upgrade, because the plan's code authorized after it and could not pass its own test; nine call sites through one `publishEvent`; the event types closed by a database CHECK; 25 negative controls |
| 10 ✅ | 16 | P4b's acceptance: the proof app answers a question, and §16's proof app is complete. **Alone, for the reason P4a's Task 15 was alone** — the first end-to-end run is where this project's worst defects have always been. P4a's passed at the first attempt and still produced eight defects, every one of them from refusing to believe it. **It also fires the external-track trigger, which is Rich's to act on** | ✅ **done 2026-09-15, 22 findings** — `make demo-ai` green from a `make reset` machine, twice; its first run failed at step 9 on its own guess, and the measurement behind the fix found every embedding charged to nobody (177, fixed in the blueprint); negative control (e) waits 611 s (181, named); 12 negative controls; the external-track trigger fired and was raised with Rich. **P4b is finished** |
...
**A NOTE FROM SITTING 8 (2026-09-15), before Step 1.** Not a pre-flight read of Tasks 14 and 15 — facts sitting 8 established, measured, that both tasks' text predates. Read them with sitting 8's entry in *What executing this plan found*.

- **Task 14's `authorizeStream` names a capability that does not exist.** It calls `assertCapability(deps.db, actor, projectId, 'read')`, and `Capability` is a union whose read member is `'project:read'` (`projects/authz.ts:8`) — what every project, build, build-log, environment and incident route passes. `tsc` refuses `'read'`; Vitest, which strips types, would run it (finding 153).
- **The authorization contract gained a route** — `GET /environments/:environmentId/incidents`, five actor rows, so `authz-contract.test.ts` is 96 tests. `WS /projects/:projectId/events` adds its own rows beside it; the drift guard compares registered routes, not test counts.
- **D23.2's stream carries incidents, and the Incident has exactly ONE call site**: the `if (!healthy)` branch of `deployRelease` in `releases/release.ts`, straight after the row records `failed` and its handle. It now covers BOTH ways a deploy fails to become ready — the health wait, and the Docker driver's readiness refusal, which throws `InstanceNotReadyError` carrying the handle and is caught and recorded rather than rethrown (finding 140). So Task 15's publish for a failed deploy belongs there, beside `captureIncident`, reusing the redactor built on the line above it — `makeRedactor(await deps.appSecrets.secretValues(db, …))` — not a second one. Task 15's *Files* block names `src/observability/incidents.ts`: publishing from inside `captureIncident` would put the bus into `observability/`'s capture signature, and publishing at the call site does not. That is Task 15's call to make and record.
- **Any OTHER failure to start still throws and records no Incident** — there is no process and no log. A `deploy.failed` event for that route, if Task 15 wants one, goes in the `else` of the same `catch`, which already discards a minted AI key before it rethrows.
- **`POST /environments/:environmentId/deploy` answers `200` with `state: 'failed'` for a Docker readiness refusal**, where it answered `500 INTERNAL` (finding 146). Nothing may read a `200` from that route as success: `make demo` and `make demo-identity` now assert `healthy`, and a stream frame for a deploy must carry the state, not only the fact that the call returned.
- **`InstanceStatus.exitCode` exists** — the Docker driver fills it once a container has exited — so a faculty-legible `human_message` for a failed deploy can say *"it stopped with exit code 3"* without parsing anything (finding 141). §14's own example, *"Your app couldn't start — it's asking for a database it hasn't declared"*, needs more than an exit code, and the Incident's log tail is where that sentence's evidence is.
- **`AppSecretResolver.secretValues(db, scope)` exists** — values, not a `Map` (sitting 1, divergence 5) — for any redactor `releases/` builds.
- **Task 15's *Test* block names `runtime/docker/roundtrip.docker.test.ts`**, whose readiness-refusal test now asserts `InstanceNotReadyError`, its handle and its check text.
...
**EXECUTED 2026-09-15, in sitting 10.** The record is sitting 10's entry in *What executing this plan found*, and it supersedes the printed steps below wherever the two differ. The differences that matter: the route is `POST /api/ask`, taking the PUID from `req.user.user` (finding 173); it uses both declared models, embedding the question beside the asker's own notes to choose one as context before streaming the answer; the proof app's own `identity.js` is deleted and `endUserId` comes from the blueprint; the blueprint's `embed()` now takes and charges the PUID (177); the spend check reads `/spend/logs/v2` by `end_user` (175, 176); `make demo-identity` and `make demo-ai` share `scripts/lib/proof-app.sh`; and negative control (e) hangs for ten minutes rather than failing (181).

**A NOTE FROM SITTING 9 (2026-09-15), before Step 1.** Not a pre-flight read of Task 16 — facts sitting 9 established, measured, that this task's text predates. Read them with sitting 9's entry in *What executing this plan found*.

- **The stream exists, and a shell cannot speak it.** Step 3.3 watches `WS /projects/:projectId/events` with a background subscriber, and `curl` does not do WebSocket. A plain GET answers `426` with `Upgrade: websocket` — a stranger `404`, nobody `401` — which is all `curl` can show. The subscriber has to be a program: `ws@8.21.3` is a devDependency of `packages/control-plane` and is what `api/events.test.ts` uses; Node 24's global `WebSocket` is undici's, and **whether it will send the Manifest session cookie is unmeasured** — measure it before building on it. Authorization is the session cookie from Step 1, checked in a route hook before the upgrade.
- **What the stream carries, in order, for the proof app's first deploy:** `build.started`, one `log` frame per build line, `build.succeeded`; then `sso.registered` (the app declares CWL), `instance.healthy`, and — because it declares models — `ai.key_rotated`, once the key is committed. A deploy that fails to become healthy is `instance.failed` then `incident.opened`, naming the Incident. Every `event` frame is an `audit.events` row published as stored; `log` frames are never rows. An instance event's `machine_detail` carries its `state`, because a deploy is a `200` whether it worked or not (sitting 8).
- **A late subscriber still sees the events, and not the log.** A connection is replayed the newest 50 events, then gets a `control` frame of type `manifest.stream.ready`, then live frames. Log lines are not replayed — `GET /builds/:buildId/logs` has them. So Step 3.7 can connect after the deploy and assert the event types from the replay; asserting `log` frames needs a subscriber connected before the build.
- **The bus is in-process.** A frame reaches only sockets on the control-plane process that published it: a subscriber whose control plane restarts mid-demo gets the replay and nothing live.
- **`ai.key_rotated` never carries the key** — its row is asserted to contain neither the key nor `[REDACTED]`. Spend attribution (Step 3.6) is LiteLLM's `/spend/logs`, not the stream.
- **There is no event for an app's failed AI call.** `EVENT_TYPES` is closed — nine types, enforced by the database CHECK `events_type_known` — and the platform never sees an app's calls to LiteLLM. Step 7 (e)'s *"the platform reports AI_BACKEND_UNAVAILABLE"* can only be a claim about the app's own response, not about an Event. A new type is a migration plus `observability/events.test.ts`'s drift test.
- **A deploy now opens the app's whole secret set before it mints a key** (finding 162). A set that cannot be opened — a row sealed under another master key — refuses the deploy before anything starts, which is new for an app with no sign-on. `make reset` drops the database and keeps `infra/secrets/master.key`, so Step 5's run from nothing is consistent.
- **Two new migrations**: `0007_stormy_demogoblin` (`audit.events.created_at` is `clock_timestamp()`) and `0008_amused_rachel_grey` (the CHECK). Step 5's `make reset` needs `db:migrate` before the control plane starts, as always.
- **Rich's decisions of 2026-09-15 are in the spec, and none changes this task's scope.** Step 1's `per_user_monthly_usd` is declared and, per §10, **validated but not enforced** in Phase 1 — Step 1's comment is right, and no step should try to prove a per-student cap; §14 now describes the stream as built, so Step 3.7's event assertions match the spec; and §10's agent-key row binds from Phase 3.
- **Signatures moved:** `startBuild(db, driver, bus, input)`, `DeployDeps.bus`, `registerServiceProvider(db, pool, keys, bus, input)` and `createSsoRegistrar(pool, keys, entityBase, certPath, bus)`; `ServerDeps.bus`, and `api/testing.ts`'s `testDeps` carries a real bus.
...
### Sitting 8, Task 13 — §14's `Incident`, and first the Mongo readiness race (2026-09-15). 20 findings, a caller position the Docker driver never reached, and a race made to happen on demand.

**Both are done, in two commits** — finding 133's fix (`7e624e3`), then Task 13 (`3d4b99e`). A Mongo service is handed over only once it enforces authentication, and its check runs every second while it starts and every thirty after. A deploy that never becomes healthy — by the health wait, or by the Docker driver's readiness refusal, which now throws `InstanceNotReadyError` carrying the handle — is recorded as a `failed` instance and captured as §14's `Incident`: the exit reason (from the new `InstanceStatus.exitCode`), the last 200 log lines, the failing check and the diff since the last healthy release, every field redacted at capture with the app's own stored secret set, stored append-only in `audit.incidents` (migration `0006_abandoned_morbius`), and served with its repair prompt at `GET /environments/:environmentId/incidents`. `describeDiff` describes every leaf of the frozen `ResolvedConfig`. **The six items in sitting 7's note at the top of Task 13 are resolved**: the migration is drizzle-kit's `0006`, its grant appended through a glob and counted before migrating; the fixture is a local `withEnvironment`; the redaction canaries are values no heuristic matches; the refusals use `expectSqlState`; `audit.incidents` is in both `TABLES` lists; and the one-file loop is `pnpm exec vitest run --project unit …`. Versions: Docker Engine 29.7.2 (the client pinned to API v1.44), `mongodb/mongodb-community-server:7.0.28-ubi8` (`sha256:56d07a02…`) with mongosh 2.6.0, Postgres 16, drizzle-kit 0.31.10, Node 24.12.0, macOS 26.6.2; LiteLLM 1.98.0 untouched.

**Finding 133, measured before a line changed.** On throwaway containers — all named `m133-`, all removed — with a 20 s script in `/docker-entrypoint-initdb.d` and an authenticated insert looping from another container as the truth: the insert failed until T+21.8 s and succeeded from T+22.3 s; the catalogue's loopback `ping` passed from T+1.4 s; an **unauthenticated** `listDatabases` was allowed (`ok=1`) through init and refused with `code 13 Unauthorized` from T+22.0 s; and a `ping` of the container's own IPv4 address failed through init and passed from T+22.0 s. Then the chosen check as a real `HEALTHCHECK` with `StartInterval`: it exited 1 through init and 0 at T+22.3 s, 240 ms after the final `mongod` began listening, and Docker called the container healthy at T+22.9 s — after the insert's first success at T+22.4 s. A Mongo started with no credentials stayed `starting`, every check exiting 1. Then test-first: the new Docker-tier test went red against the `ping` in 6.3 s (`expected 1 to be +0` — handed over while the init script still held the unauthenticated `mongod`), and green in 22.9 s after the fix.

**What is proved, and at which tier.** *Unit, no Docker:* `describeDiff` — eight tests, among them a guard that reads every leaf off a real resolved config; `captureIncident` — the last 200 of 300 lines from a source that ignores `tail`, every field redacted with a canary only the exact-match half can catch, the last HEALTHY release rather than the previous one, the same-release and code-only cases, the never-healthy sentence, unreadable state and logs recorded as sentences carrying the error's code and never its message, a refusal for an instance with no handle, and the list newest first; `audit.incidents` refusing UPDATE, DELETE and TRUNCATE (`42501`) and a delete of its instance (`23503`); `deployRelease`'s three failure routes — the health wait (an Incident naming the check, the log read by the handle the row records), the readiness refusal (recorded, not thrown, not parked) and any other failure (thrown, nothing recorded) — plus the deploy's own hex `SESSION_SECRET` redacted from a driver's log and a minted AI key discarded on a refusal; the route, which hides an environment's incidents behind 404; and the contract's five new rows. *Docker tier:* the finding-133 race, forced; and a real app — the fixture app's lockfile with an entry point that prints 252 lines, one of them its own `SESSION_SECRET`, and exits 3 — built through §12's gates, deployed through `deployRelease` and the real driver, refused by readiness, and recorded as `failed` with an Incident: `the process exited with code 3`, lines 53 to 252, the secret as `[REDACTED]`, the readiness check with its reason, and the never-healthy sentence. `roundtrip.docker.test.ts`'s refusal test now asserts the typed error, its handle and its check. **Not proved:** an Incident on the stream (Task 15's); a crash LOOP (Phase 4's, in *What this plan does not build*); and `make demo` was not re-run — see the decisions.

**Decisions made here, so nobody re-derives them.**

| Decision | Why, and what was rejected |
|---|---|
| **A service is ready when an unauthenticated `listDatabases` is REFUSED with code 13** | It asserts the property that matters — authentication enforced — and fails closed for a Mongo with no authentication, measured (139). *Rejected:* a `ping` of the container's own IPv4 address, which discriminated as well on 2026-09-15 (137) but asserts only that the final `mongod` is bound, and which sitting 7 once saw never go healthy; and an authenticated `ping`, because the user exists before the init phase ends — by the entrypoint's order, read rather than measured |
| **Every second while starting (`StartInterval`), every 30 s after, 3 retries, a 120 s start period** | Finding 134. The start period outlasts `waitForServiceHealthy`'s 90 s, so a slow start is never marked `unhealthy` while it is being waited on. *Rejected:* keeping the one-second check, the load that widened 133 |
| **Every `ensureServiceContainer` path waits for health** | 136 |
| **`createServiceContainer` is exported** | The only way to place an init script in a container created exactly as production creates it, before its first start. *Rejected:* a test-only hook parameter in production code, and a derived image, which would test a different image |
| **`InstanceNotReadyError` in `runtime/driver.ts`, carrying the handle and the check** | 140. *Rejected:* re-deriving the container name in `releases/` — a second producer of a driver's naming, P3 Session 5's shape; and making `ensureInstance` return an unready handle, which reopens the 502-on-wake defect that readiness was moved inside `ensureInstance` to close |
| **A readiness refusal is a RECORDED failure — the row `failed`, an Incident — returned, not thrown** | §14, and `startBuild`'s precedent: a failure is a row. Both drivers now agree; the fake already returned `failed`. The cost is 146. Any other failure to start still throws and records nothing — no process, no log |
| **The Incident reads its handle, release and environment from the instance ROW** | One source instead of three arguments that must agree, so it is captured after the row records `failed` and its handle |
| **`describeDiff` compares the frozen `ResolvedConfig`, every leaf, behind a drift guard** | 144. Every leaf because an Incident that says nothing changed when something did is the lie §14's diff exists to prevent. Environment variables by NAME, never by value |
| **The diff names the commit** | 145 |
| **"The last healthy release" is the release of the most recent `healthy` instance row in the environment, by `last_seen_at`** | Redeploys leave earlier rows `healthy` (RUNBOOK's leaked-container gap), so the most recent is what "last" means |
| **A state or log the driver cannot give becomes a sentence carrying the error's code; a failed INSERT reaches the caller** | An Incident that says why it is incomplete is worth more than none; a code, never a message, because a message can carry what the app printed. The row already says `failed`, so a failed insert leaves the instance recorded as what it is |
| **The redactor comes from `AppSecretResolver.secretValues`, which returns values** | 142 — and a `Map` is iterable as pairs and matches nothing (sitting 1, divergence 5) |
| **`GET /environments/:environmentId/incidents` returns the 20 newest, each with its `prompt`** | The prompt is §14's purpose, so it is what an agent should be handed; twenty because each carries up to 200 lines |
| **No spec action** | §11 lists the `Driver` methods and fixes neither `InstanceStatus`'s fields nor how `ensureInstance` fails; P4c owns any §11 wording about readiness |
| **`make demo` was not re-run** | Its database predates the fix (150), so it could not exercise finding 133 on this machine — `roundtrip.docker.test.ts` creates a fresh service, which does — and a run would leave a new app container and route to unpick. The two scripts' changed lines were instead extracted from the scripts and run against a healthy, a failed and an error response, after `bash -n` |

| # | Finding | Measured against |
|---|---|---|
| 135 | **The Mongo image has no `/docker-entrypoint-initdb.d`**, and the archive endpoint refuses a path that does not exist. The test uploads at `/` with the directory in the entry's name, which extraction creates — and asserts the script's own last line in the container log, because an init directory the entrypoint never read would leave no slow init and the test green | `ls -ldn /docker-entrypoint-initdb.d` in the image: *No such file or directory*; control 133 (c) |
| 136 | **`ensureServiceContainer` returned the moment it asked an EXISTING stopped container to start.** The health wait was reached only by a container the same call had created, so a service woken after a stop or a Docker restart was handed to an app while `mongod` was still coming up | `services.ts` before; control 133 (b) RED |
| 137 | **Sitting 7's "own-IP ping never went healthy in 200 s" did not reproduce** — on 2026-09-15 it discriminated exactly as the chosen check did. Unexplained either way, and no longer on the path | Measurement 1 |
| 138 | **Docker 29.7.2 honours `StartInterval` at API v1.44**: checks 1.2 s apart until the first success, then 30 s apart. The new cadence idles at 0.62% CPU against 30.07% and 28.76% for the two demo databases on the one-second check (finding 134) | Measurement 2's `Health.Log`; `docker stats` |
| 139 | **A Mongo started with no authentication never reports healthy under the new check** — it stayed `starting`, every check exiting 1, while its unauthenticated `listDatabases` answered `ok: 1` | Measurement 2 |
| 140 | **The plan's caller position was unreachable on the Docker driver.** Step 5 captures where `next === 'failed'`, and `DockerDriver.ensureInstance` refuses an instance that never answers at its hostname by THROWING — `EngineError('INSTANCE_NOT_REACHABLE')`, with no handle — so the commonest real failure, an app that crashes as it starts, went through `deployRelease`'s `catch`, produced no Incident and left its row in `provisioning`, a state nothing moves. The printed Step 5 was reachable only by the fake driver | The Docker driver's readiness throw and `roundtrip.docker.test.ts`'s refusal test; control (e) RED in both tiers — in Docker the deploy throws |
| 141 | **`InstanceStatus` carried no exit code**, so §14's "exit reason" had no producer; the Docker driver read `State.ExitCode` and dropped it | Control (f) RED: `the platform reports the instance as …` |
| 142 | **Step 5's `redactor` had no source.** `deployRelease` holds no key material by design, and nothing it held could give it the app's secret set | `release.ts`; control (c2) RED |
| 143 | **`waitForHealth` returned a bare boolean, so the check could not say whether the driver gave up or the wait ran out** — and "did not report healthy within 10 s" was false for a driver that reported `failed` at once | The first draft of the check text, against the fake driver |
| 144 | **The plan's `describeDiff(ManifestSpec)` would have read `app_specs.parsed` back** — sitting 1's divergence 4, the second source of truth P4a deleted | Control (g) RED on the drift guard |
| 145 | **The plan's diff omitted the code.** A release is a build and a config, and the commit is the likelier cause of a failure | Design |
| 146 | **A failed deploy is now a `200` on both drivers** — for a Docker readiness refusal it was a `500 INTERNAL` — **and `make demo` and `make demo-identity` only checked that a state came back**, so a failed demo deploy would have run on to step 9 and blamed the Caddyfile wildcard for an app that crashed. Both now assert `healthy` and name the incidents route | The scripts; their changed lines run against three scripted responses |
| 147 | **The first expectation written for the AI-key refusal test was wrong**: a fake that creates the instance healthy and then refuses it reads back `health passed` at the Incident's status read. The refused instance is now created failing | First run: `expected [ 'mint', 'instance', …(2) ] to deeply equal [ … ]` |
| 148 | **Task 9's "DISCARDS the minted key" test pinned an exact event list that Task 13 lengthens** — the Incident reads the status once more, after the discard. Updated deliberately; the order — the discard before anything is written about the failure — is what the new list asserts | `releases.test.ts` |
| 149 | **A measurement script piped `mongosh`'s exit code through `tail`**, so its exit column read 0 for every check; only the printed result was evidence, which is why the second measurement ran the check as a real `HEALTHCHECK` | Measurement 1's log: `exit=0 [LISTED ok=1]` |
| 150 | **Service containers created before the fix keep the old check** — nothing recreates an existing service. Both demo databases on this machine still run the one-second `ping`, at 54.5% and 52.0% CPU when inspected | `docker inspect` `.Config.Healthcheck`: the `ping`, `interval=1s startInterval=0s`; RUNBOOK's *Known gaps* |
| 151 | **`createServiceContainer` does not refuse a 404 from `/containers/create`** — the defect `ensureInstanceContainer` fixed as `INSTANCE_IMAGE_MISSING` — so a service whose image is not in the daemon waits 90 s and reports `last status: none`. Named, not fixed | Reading `services.ts` |
| 152 | **Both control runs' working-tree hashes overlapped edits to other files**, and each reported *unchanged*; the per-file hashes, checked after every restore, are the proof | The runs' timing against this session's own edits |
| 153 | **Task 14's `authorizeStream` calls `assertCapability(…, 'read')`, and no such capability exists** — the read member of `Capability` is `'project:read'`. Written into the note at the top of Task 14 | `projects/authz.ts:8` |
| 154 | **The Docker-tier Incident test passed at its first attempt, in 27.7 s** — which is why its three Docker controls were run rather than assumed | Controls (a′), (e) and (f), all RED |

**Negative controls — 17, every file mutation applied by a script that refused unless its pattern matched exactly once, restored byte for byte and hash-checked after each run; the two database controls restored and read back.**

| Control | Result |
|---|---|
| 133 (a) the loopback `ping` restored | **RED** — `expected 1 to be +0`, the insert refused at hand-over |
| 133 (b) no wait when the container already existed | **RED** — the same line |
| 133 (c) the init script uploaded where the entrypoint never reads | **RED** — at the log assertion, `expected [ …(285) ] to include 'manifest slow init: done'` |
| 13 (a) the head of the log kept, not the tail | **RED** — `expected 'line 1' to be 'line 101'` |
| 13 (a′) no tail requested and the head kept, in Docker | **RED** — `expected 'boot line 1' to be 'boot line 53'`. The head alone, with the tail still requested, would be GREEN there — Docker's own `tail=200` holds the property — so the unit tier is where that half is proved |
| 13 (b) the "healthy" filter removed | **RED** ×2 — the previous failed release named, and a never-healthy app compared with a failure |
| 13 (c1) `log_tail` stored unredacted | **RED** — `not to contain 'hunter2'` |
| 13 (c2) the app's secret set empty | **RED** — `SESSION_SECRET=a2945…` stored |
| 13 (d) an empty repair prompt | **RED** ×2 — the prompt test, and the route test |
| 13 (e) the readiness refusal not caught, unit | **RED** ×2 — the recorded-refusal test and the AI-key discard test |
| 13 (e) the same, Docker | **RED** — the deploy threw |
| 13 (f) no exit code from the Docker driver | **RED** — `the platform reports the instance as …` |
| 13 (g) a resolved-config leaf left undescribed | **RED** — `[ …(17) ]` against `[ …(18) ]` |
| 13 (h) services compared in order | **RED** — a reordering reported as a change |
| 13 (j) the refusal's check dropped | **RED** — `expected [ '' ]` |
| 13 (k) `GRANT UPDATE, DELETE, TRUNCATE` to `manifest_app` | **RED** ×3; `REVOKE` → ACL back to `manifest_app=ar/manifest` |
| 13 (l) the foreign key made `ON DELETE CASCADE` | **RED**; RESTRICT restored → `confdeltype = r` |

**Checked, and unchanged:** the four HTML pages — none mentions incidents, and `manifest-phases.html`'s "the AI half is what remains" stays true until Task 16; and `docs/external-track.md`, whose trigger is Task 16. **Found stale in the sweep and corrected:** the roadmap's P4 plan-table row still said *"10 executed by 2026-09-14 (sittings 1–6 of ten)"* — two sittings behind.

**Gates at the end:** `pnpm test` **703** (64 files, twice), `pnpm test:docker` **133 of 133** (25 files, 490 s — the first full tier since finding 133 was measured; sitting 7's ended 130 of 131), lint / typecheck / `format:check` clean, `make doctor` **18/0** and `make verify` **47/0**, both after the Docker tier. **The machine at the end** differs from its start-of-session snapshot in the clock, free disk (151 → 147 GiB), container uptimes — `manifest-caddy`, which `routes.docker.test.ts` restarts by design — another application's UDP socket on 443, and HEAD. **The image list is identical**: the Docker tier's images were removed by every name each carried — `local/blueprint-ntm`, `local/chem-labs`, `local/fixture-s6` with `local/fixture-rt`, `local/saml-unsigned` with `local/saml-probe`, and this sitting's `local/incident-probe` — and each second name of a shared digest took a second pass, exactly as ORIENTATION §4 warns. `make verify`'s per-app count is unchanged at 8 containers, 9 networks and 6 volumes, and no `m133-`, `svctest`, `slowinit` or `incident-probe` resource is left. **One change that is not this sitting's:** the `credit-card-helper` compose project's two containers were started together at 16:06:09 UTC and stopped together at 16:24:53, restart policy `no`. No command this session ran names that project and neither time matches one of its commands, so it is recorded here, not undone. **Sitting 9 — Tasks 14 and 15, the event stream and the call sites that make it carry anything — is next; a note from this sitting is at the top of Task 14.**

### Sitting 9, Tasks 14 and 15 — the event stream, and the call sites that make it carry anything (2026-09-15). 18 findings, a stream whose printed code could not pass its own test, and nine event types closed by the database.

**Both are done, in two commits** — Task 14 (`13d28f9`), then Task 15 (`28df4f3`) — with a third, `5149663`, that corrects a comment Task 15 left describing the old read position. `WS /projects/:projectId/events` is D23.2's stream: authorized by `project:read` in a route hook that runs before `@fastify/websocket` upgrades, so a stranger gets an HTTP `404` and no socket, and a member's plain GET gets `426` with `Upgrade: websocket` — which is what the authorization contract suite covers as five actors. A connection subscribes, is replayed the newest 50 `audit.events` rows oldest first, gets a `manifest.stream.ready` frame, and then the frames that arrived meanwhile, minus any the replay carried; past 1 MiB buffered it is closed with 1013 and unsubscribed. Then Task 15: `publishEvent` records a row and publishes it as stored, and it is the only way an event is written — three sites in `startBuild` (`build.started`, `build.succeeded`, `build.failed`), four in `deployRelease` (`instance.healthy`, `instance.failed`, `incident.opened`, `ai.key_rotated`) and P4a's two in `sso/registration.ts`. A build's log lines reach the stream as they are written, redacted, as `log` frames that never become rows. `audit.events.created_at` is `clock_timestamp()` (migration `0007_stormy_demogoblin`) and its `type` is closed by the CHECK `events_type_known` (`0008_amused_rachel_grey`). Sitting 8's note at the top of Task 14 and sitting 5's correction at the top of Task 15 are resolved. Versions: `@fastify/websocket` 11.3.0 with `ws` 8.21.3 and `@types/ws` 8.18.1 — each the latest published on the day, pinned exact, the closure M6 measured script-free — Fastify 5.12.3, Postgres 16, drizzle-kit 0.31.10, Node 24.12.0, pnpm 11.24.0, Docker Engine 29.7.2, macOS 26.6.2; LiteLLM 1.98.0 untouched.

**What is proved, and at which tier.** *Unit, with Postgres:* the bus — per-project delivery, a throwing subscriber that neither stops the others nor goes unreported, per-subscription counts; the replay — the newest `limit` oldest first, and events of one transaction in the order written; the route, against a real listening server — `426` with its header; a stranger's and nobody's upgrade refused as HTTP `404` and `401`, with no frame and no subscription; replay, boundary, then live, to a collaborator; a frame published while the replay is held behind a table lock, delivered once and after the boundary; another project's frames not delivered; a slow socket closed with 1013 before half of 20,000 frames were sent, and unsubscribed; a closed socket unsubscribed; the contract suite's five rows. `publishEvent` — the row as stored and redacted, and nothing when the record is refused; the closed set — the CHECK read back out of Postgres equal to `EVENT_TYPES`, and a raw INSERT of an unknown type refused `23514`. `startBuild` — started, log, succeeded in that order and recorded; a failed build's sentence free of `exit`, `npm` and the reason, with the reason redacted in `machine_detail`; log frames equal to the stored lines. `deployRelease` — `instance.healthy` with its state; `instance.failed` then `incident.opened` naming the Incident; nothing for an instance that never started; `ai.key_rotated` only after the commit, carrying neither the key nor `[REDACTED]`, and none on a discard or a failed commit; and a secret set that cannot be opened refused before anything is minted or started. Through the API, a failed build, a good build, a healthy deploy and a failed deploy recorded as seven rows that are exactly the seven event frames. *Docker tier:* the boot entry point with the plugin and the bus (`boot.docker.test.ts`); a real SSO registration's frames equal to its rows; a real crashing container's deploy streaming `instance.failed` then `incident.opened`. **Not proved:** a WebSocket client other than `ws` — no browser, no shell — and the stream through the edge; both are Task 16's.

**Decisions made here, so nobody re-derives them.**

| Decision | Why, and what was rejected |
|---|---|
| **Authorization is a `preValidation` hook on the route, and `wsHandler` reads it again** | 155. The hook runs for a plain GET and for an upgrade, before the plugin upgrades, so a stranger never holds a socket and the two halves cannot disagree. The second read is defence in depth: control (a3) shows it is unobservable while the hook stands, and (a) and (a2) show what each layer catches. *Rejected:* the plan's check inside `wsHandler` — after the upgrade — and a shared function called from both halves, which is still after the upgrade on the socket's side |
| **The route is declared in full, `handler` plus `wsHandler`, without `websocket: true`** | 156 |
| **`426` carries `Upgrade: websocket` and the D23.7 envelope, code `EVENTS_UPGRADE_REQUIRED`** | RFC 9110 requires the header on a 426. `authz-contract.ts`'s `Expectation` gained `426` rather than folding it into `pass`, which means "below 400" |
| **After the upgrade, a refusal closes with 4404, a failure to read with 1011, and a slow socket with 1013** | An application code lets a client tell "you may not" from "I broke"; 1013 is RFC 6455's Try Again Later, and the reconnect is replayed |
| **What arrives during the replay is held, and the hold is capped by the same 1 MiB** | A flood during a slow replay read would otherwise grow the hold without bound — the leak the socket cap closes |
| **`EventBus.listenerCount`** | A socket that closed and stayed subscribed is a leak nothing else shows. *Rejected:* a fixed wait for the subscription in the ordering test |
| **One producer per frame shape: `eventFrame`, `logFrame`, `readyFrame`** | The shape a client switches on is written once |
| **`created_at` is `clock_timestamp()`, and the replay breaks a microsecond tie on `id`** | 157 |
| **`publishEvent` lives in `bus.ts`** | 161 |
| **The CHECK's list is written out in `db/schema.ts`, not generated from `EVENT_TYPES`** | Two independent reads of one rule, compared by a test that reads the constraint back out of Postgres. The cost, stated in 0008's own comment: adding a type is a migration that drops and re-adds the constraint |
| **`incident.opened` and `ai.key_rotated` are published at the call site in `deployRelease`** | 159. `captureIncident` stays a store function, and `ai/` never holds the bus; the deploy holds it and narrates. The cost: a second caller of `commitAppKey` would have to publish its own event. Sitting 8's note left the Incident's placement to this task |
| **`instance.failed` is published before the Incident is captured, and `incident.opened` after** | The row already says failed; the Incident is announced once it exists, by id |
| **P4a's two SSO events go through `publishEvent`** | 160 |
| **No event for a discarded AI key** | It never became the app's key, and the failed instance has its own event; a failed discard stays on stderr, where sitting 5 put it. Sitting 5's correction left this call to Task 15 |
| **`build.succeeded` is published outside the `try`** | A failure to record the success must not re-mark a built image as a failed build. **Not proved by a test** — nothing in the unit tier can fail only that insert — and named here for that reason |
| **The build-log writer redacts at `write` and hands that line to `onLine`; the store's insert redacts again** | The frame is the line that will be stored, and `appendBuildLog` keeps its own guarantee for any other caller; redaction is idempotent |
| **One redactor per deploy, from the app's secret set read before the mint; `ai.key_rotated`'s adds the committed key** | 162 and 163 |
| **The stream authorizes at connect, not per frame** | Named in *What this plan does not build*: a per-frame check is a database read per frame |

| # | Finding | Measured against |
|---|---|---|
| 155 | **Task 14's code authorizes inside `wsHandler`, which runs AFTER the upgrade, so its own test cannot pass.** `@fastify/websocket` 11.3.0 answers `upgrade` by routing the request through Fastify — `fastify.routing(rawRequest, rawResponse)`, with the response bound to the socket — so the route's hooks run before the wrapped handler calls `wss.handleUpgrade`, and a hook that throws writes an ordinary HTTP status to the socket, which the plugin's `onResponse` hook then destroys. Inside `wsHandler` a refusal can only be a close code on a socket the stranger already holds; the plan's test expects `404` | The plugin's `index.js`; control (a): the upgrade-refusal test and the contract's stranger and anonymous rows RED |
| 156 | **Task 14's declaration — `websocket: true` beside `handler` and `wsHandler` — makes the plugin use `handler` as the socket handler and answer every plain GET `404`.** Its `onRoute` hook, when `websocket` is set, takes `wsHandler = routeOptions.handler` and replaces the handler with `reply.code(404).send()` | The plugin's `index.js`; control (i): 9 RED |
| 157 | **`audit.events.created_at` defaulted to `now()`, the TRANSACTION's start**, so every event one transaction wrote carried the same instant and a replay ordered by it came back in whatever order the index held | The distinct-timestamp test over three events in one transaction; control (g): both replay tests RED under `now()` |
| 158 | **Task 15's "a database `CHECK` enforces it" named a constraint that did not exist** — migration 0004 carries only `events_human_message_present` | `pg_constraint` before 0008; controls (f) and (g) |
| 159 | **Task 15's grep puts `incident.opened` in `observability/incidents.ts` and `ai.key_rotated` in `ai/keys.ts`**, which would put the bus into `captureIncident`'s and `commitAppKey`'s signatures. Both are published in `deployRelease`, so the acceptance grep is nine `publishEvent(` lines — `build.ts` 3, `release.ts` 4, `registration.ts` 2 — with no bare `recordEvent(` outside `observability/`, and `bus.publish` in exactly `publishEvent` and the build-log path | The grep, run |
| 160 | **Task 15 left P4a's two SSO events on `recordEvent`**: a site that records without publishing, whose events the replay would then serve only on reconnect — while §9's ACS-change alert is exactly the kind of subscriber D23.2 describes | Control (i), Docker tier: the registration test RED |
| 161 | **The plan's `publishEvent` in `observability/events.ts` would make `events.ts` and `bus.ts` import each other** — it needs `eventFrame`, and `bus.ts` needs `Event` and `recordEvent` | Reading the two modules |
| 162 | **Every deploy now opens the app's whole secret set, and Task 15 first read it after the instance was recorded** — so a set that cannot be opened threw with the row `healthy` and a minted AI key neither committed nor discarded. Found because a `releases.test.ts` fixture sealed its service password under a second master keypair: `SECRET_UNWRAP_FAILED` from `secretValuesFor`, a mismatch nothing had read until a healthy deploy did. The read is now before the mint, and the fixture uses one keypair, as `src/index.ts` binds it | The failure's stack; control (j): the new refusal test RED |
| 163 | **`ai.key_rotated`'s "no `sk-`" assertion could not fail once the redactor held the committed key** — a key put into `machine_detail` comes out as `[REDACTED]` and passes. The test also asserts no `[REDACTED]`, which nothing this event legitimately carries produces | Controls (b) and (b2) RED, (b3) GREEN as predicted |
| 164 | **Nothing said a streamed log line is the redacted line**: a writer that published raw text would have passed every test, because the store redacts again. A test now compares the frames with the stored lines | Control (e): RED |
| 165 | **Task 14's backpressure test pauses the client and never resumes it** — and a paused client never reads the close frame queued behind the data it has not read, so it cannot see 1013. The test resumes after the flood | Control (k): RED |
| 166 | **Task 14's ordering test cannot make its own control fail cleanly** — the plan says replay-before-subscribe "flakes". Made deterministic: an admin connection holds `audit.events` in `ACCESS EXCLUSIVE` while an event is recorded (invisible until COMMIT) and published inside the window beside a frame never recorded, and nothing may be sent until the lock goes | Controls (b), (b2) and (b3): each RED |
| 167 | **This sitting's own collaborator was a stranger.** The test added `bio_student` before the user row existed; `POST …/members` answered `400 MEMBER_USER_NOT_FOUND`, unasserted; and the test timed out at Vitest's 5 s with no message, because its own wait was also 5 s. The add is asserted now and the wait is 3 s. It is the shape `authz-contract.ts` warns of, caught only because it failed rather than passed | The first run |
| 168 | **Two control runs were not controls at first**: Task 14 (e)'s mutation did not compile (`no tests`), and Task 15 (e)'s pattern matched twice — `appendBuildLog` has the identical line — which the runner refused. Both re-ran with a mutation that compiles and a pattern that matches once | The runs |
| 169 | **Installing `@fastify/websocket` re-resolved `ubc-genai-toolkit-llm@0.7.0` against its optional `ws` peer** — `0.7.0(ws@8.21.3)` in `pnpm-lock.yaml`; no version moved | `git diff pnpm-lock.yaml` |
| 170 | **`@fastify/websocket` wraps EVERY route's handler, not only the stream's** — its `onRoute` hook replaces each one, so an upgrade to a plain route can be closed | Its `index.js`; the unit suite unchanged at 721 once the plugin was registered |
| 171 | **Task 14's Step 3 and Task 15's Step 2 run `pnpm test -- <filter>`, which does not filter** (ORIENTATION §4) | The one-file loop used instead: `pnpm exec vitest run --project unit src/api/events` |
| 172 | **`deploy-sso.docker.test.ts` pinned a CWL deploy's audit events to exactly `['sso.registered']`**, and Task 15 records the build and the instance too — sitting 8's warning that exact-list tests move when call sites land, in a file only the Docker tier runs, so the unit tier's 735 could not see it. It now asserts the whole ordered list, which also pins the registration between the build and the healthy instance | The first full Docker tier: 132 of 133, `expected [ 'build.started', …(3) ] to deeply equal [ 'sso.registered' ]` |

**Negative controls — 25, every file mutation applied by a script that refused unless its pattern matched exactly once, restored byte for byte and hash-checked after every run; both database controls restored and read back.**

| Control | Result |
|---|---|
| 14 (a) the authorization hook removed, `wsHandler`'s read kept | **RED** ×3 — the upgrade-refusal test, and the contract's stranger and anonymous rows |
| 14 (a2) the hook and the second read both removed | **RED** — the upgrade-refusal test |
| 14 (a3) only the second read removed | **GREEN, as predicted** — unobservable while the hook stands |
| 14 (b) the replay read before subscribing | **RED** — the ordering test |
| 14 (b2) no de-duplication of held frames | **RED** — the ordering test |
| 14 (b3) frames sent during the replay instead of held | **RED** — the ordering test |
| 14 (c) no `bufferedAmount` cap | **RED** — the backpressure test |
| 14 (d) no unsubscribe on close | **RED** |
| 14 (e) the bus swallows a throwing subscriber silently | **RED** — after an invalid first run (168) |
| 14 (f) the replay returned newest first | **RED** ×2 |
| 14 (g) `created_at` back to `now()` in the database | **RED** ×2; `clock_timestamp()` restored and read back |
| 14 (i) the plan's `websocket: true` declaration | **RED** ×9 |
| 14 (k) the plan's `'read'` capability | **`tsc` RED** (TS2345), and 9 runtime failures |
| 15 (a) `instance.healthy` written with a bare `recordEvent` | **RED** ×4, the counting test through the API among them |
| 15 (b) the key in `ai.key_rotated`'s `machine_detail` | **RED** (163) |
| 15 (b2) the same, and the redactor without the key | **RED** |
| 15 (b3) only the redactor without the key | **GREEN, as predicted** |
| 15 (c) `build.failed`'s sentence set to the raw reason | **RED** |
| 15 (d) the build log not published | **RED** ×3 |
| 15 (e) a log frame unredacted, the store still redacting | **RED** — after a refused first run (168) |
| 15 (f) the database CHECK dropped | **RED** ×2; re-added and read back |
| 15 (g) a type added to `EVENT_TYPES` with no migration | **RED** |
| 15 (i) P4a's SSO events left on `recordEvent`, in the Docker tier | **RED** |
| 15 (j) the secret set read after the instance is recorded | **RED** (162) |
| 15 (k) Task 14's backpressure test without its resume | **RED** (165) |

**Checked, and unchanged:** the four HTML pages — none mentions incidents, a WebSocket or an event stream, and `manifest-phases.html`'s "the AI half is what remains" stays true until Task 16; and `docs/external-track.md`, whose trigger is Task 16. **Added to:** `RUNBOOK.md`, a section on watching a project's event stream; and ORIENTATION §4, six durable facts about the plugin, `now()`, `ws` and the secret-set read.

**Gates at the end:** `pnpm test` **735** (66 files, twice), `pnpm test:docker` **133 of 133** (25 files, 464 s) — the first full run was 132 of 133 in 472 s, on finding 172, which was corrected and re-run alone before the second — lint / typecheck / `format:check` clean, `make doctor` **18/0** and `make verify` **47/0**, both after the Docker tier. **The machine at the end** differs from its start-of-session snapshot in the clock, container uptimes — `manifest-caddy`, which `routes.docker.test.ts` restarts by design — another application's UDP socket on 443, and HEAD, with this record's six documents uncommitted when it was taken. **The image list is identical to the pre-tier list after both runs**: the tier's images were removed by every name each carried — `local/blueprint-ntm`, `local/chem-labs`, `local/fixture-s6` with `local/fixture-rt`, `local/saml-unsigned` with `local/saml-probe`, and `local/incident-probe`. LiteLLM, checked after each full run: `p4b-probe-user` holds **0 keys** and there are **0** `mf-` users. New in the lockfile: `@fastify/websocket`, `ws` and `@types/ws`, with `duplexify` and its closure. **Sitting 10 — Task 16, P4b's acceptance, alone — is next; a note from this sitting is at the top of Task 16.**

### Sitting 10, Task 16 — the proof app answers a question, and P4b is finished (2026-09-15). 22 findings, an embedding charged to nobody, and a gateway that makes a student wait ten minutes.

**Task 16 is done, in one commit (`78f65f9`)**, and with it P4b. §16's proof app is complete: `POST /api/ask` answers a signed-in person's question with one of THEIR OWN notes as context — the question embedded beside their newest twenty notes with `default-embed`, the closest chosen, the answer streamed from `default-chat` — through the key the platform minted for that app, confined to three routes and two models, and charged to `sha256(puid ‖ project ‖ environment)`. `make demo-ai` proves it by `curl` and a stream subscriber against the running platform, in nine steps: it subscribes to `WS /projects/:projectId/events` before anything is built and asserts the live half carried `build.started → build.succeeded → sso.registered → instance.healthy → ai.key_rotated` and exactly the build's stored log lines, in order; signs a student and an instructor in and writes each a note — the instructor's written to be the closer match; asserts each answer is non-empty, streamed, chose its context through a **768**-dimension embedding, and used the asker's own note and never the other's — the instructor's question naming the student in its body every run; and reads LiteLLM's own spend log for a chat row AND an embedding row per person, each on their namespaced identifier through this app's LiteLLM user, and nothing on a bare PUID hash or a raw PUID. **It passed from a `make reset` machine, twice**, with `make demo-identity` before each — the two scripts now share `scripts/lib/proof-app.sh` — and `make demo` restored fixture-app after. The external-track trigger has fired and was raised with Rich, not started. Versions: Node 24.12.0 on the host and 22.23.2 in the app image; `ubc-genai-toolkit-llm` 0.7.0 over `openai` 4.104.0; LiteLLM 1.98.0 (`sha256:20b5044b`); Ollama 0.34.0 with `ministral-3:latest` and `nomic-embed-text`; Docker Engine 29.7.2; macOS 26.6.2.

**The first run failed at step 9, on this sitting's own guess** — and the measurement that fixed the guess found the finding worth the sitting: every embedding the proof app made for a student was charged to nobody (177).

**What is proved, and at which tier.** *Unit:* the blueprint's `embed()` sends the namespaced `user` and refuses a call that names nobody before anything is sent (`ai-component.test.ts`, on the wire); the parsed-call guard in `blueprints.test.ts` now requires `user: endUserId(…)` on a toolkit `.embed(` as well as a chat call; the end-user formula is pinned against its one producer and the proof app carries no second copy — no `identity.js`, no hashing anywhere in its source (`proof-app-identity.test.ts`). *End to end, `make demo-ai`:* everything in the paragraph above. *Docker tier:* `node-ts-mongo.docker.test.ts` builds the changed skeleton through §12's gates. **Not proved:** offline — Rich's to run, and step 7 of `scripts/offline-acceptance.sh` is new; a browser as the stream client; the answer's content, which is printed and never asserted; a per-student cap, which §10 validates and does not enforce in Phase 1; and any bound on how long a question waits when the gateway vanishes (181).

**Decisions made here, so nobody re-derives them.**

| Decision | Why, and what was rejected |
|---|---|
| **`POST /api/ask` under `/api/`, behind `signedIn`, the PUID from `req.user.user`** | The app's own conventions (173). The body's `puid` is ignored, and the demo sends one naming the other person every run. *Rejected:* the plan's `/ask` and `requireSignIn`, which do not exist |
| **Both declared models do work: embed the question beside the asker's newest 20 notes in ONE call, pick by cosine similarity, stream the answer with that note** | Decision 14 requires asserting 768 from an embedding, and the plan's Step 2 never embedded while its Step 1 declared `default-embed` — a model declared for nothing, which a PIA reviewer would rightly ask about (191). It also makes the AI half prove what the notes half proves: a model is handed nothing its asker could not already read. *Rejected:* the plan's `ask()` alone; a separate route that embeds for no purpose |
| **The proof app calls `configureAi()` unconditionally** | An app that needs its AI half should fail its first boot without it, with the Incident naming the fix (186). *Rejected:* the skeleton's `AI_ENABLED` guard, right for a blueprint that serves apps with no models and wrong for this one — it would start healthy and fail at the first question |
| **`fixtures/proof-app/identity.js` DELETED; `endUserId` imported from the blueprint** | Sitting 1's reconciliation item 2: two producers of the string every note is stored under and every question is charged to. *Rejected:* a re-export shim, a second file that cannot be imported outside the assembled tree |
| **`embed(texts, puid)` — the PUID REQUIRED** | 177. An embedding made for a person is spend on their behalf. *Rejected:* an optional PUID, which fails open silently — the spend simply has no person on it |
| **The proof app classifies a failed AI call by HTTP status and the SDK's two fixed no-answer messages, and logs only the error's class and status** | 185: the toolkit's `details.type` is `'Error'` for every failure. The two messages are the SDK's constants, never text from the gateway, which for a refused key carries the key's hash. *Rejected:* matching any gateway text; putting the mapping in the blueprint now — named in *What this plan does not build* with the deadline below |
| **No deadline on an AI call** | 181. A correct one belongs in the blueprint, needs stream semantics and a decision about the abandoned request, which keeps its socket until the SDK gives up. *Rejected:* a `Promise.race` in the proof app, which would hide the defect from every other generated app |
| **`scripts/lib/proof-app.sh` assembles and deploys the proof app for both demos, copying the WHOLE fixture over the skeleton** | P4a defect 80's shape one level up: two scripts that disagree about what the proof app is. *Rejected:* a second copy in `demo-ai.sh`; a list of the fixture's files, which a new file silently escapes |
| **The stream subscriber is `scripts/lib/event-stream.mjs`, dependency-free on Node's global `WebSocket`** | 179. *Rejected:* `ws`, which does not resolve from `scripts/` and would be a new prerequisite (C1) |
| **Only the stream's LIVE half is asserted, and its log frames must equal the stored build log line for line, inside the build's two events** | The replay carries earlier deploys, and a count passes a writer that published the wrong lines. Control (g) shows the replay is refused as evidence |
| **The spend check reads `/spend/logs/v2?end_user=` with `LITELLM_MASTER_KEY` from `.env`, requires a chat and an embedding row per person, and ties each row to the project through its LiteLLM `user`** | 175, 176. An operator's check on an operator's machine. *Rejected:* the plan's `user_id` filter (the app, not the person) and the key's metadata (absent from spend rows) |
| **The answer's content is printed, never asserted; the context line says whether its note was written this run** | A model's wording is not the platform's contract; which note it was given is. 180 |
| **`demo-identity.sh` loses its step 9** | The `/api/ai` stub it asserted is gone (174); the third half is `make demo-ai`'s |
| **Step 5's from-nothing run moved `.manifest/repos` aside, reversibly** | 190. `make reset` leaves it, so "from nothing" otherwise reuses the old repository through a `POST /projects` that fails after committing its row. *Rejected:* changing `make reset` in this task — named instead |
| **No spec action** | §10, §14 and §16 describe what was built, as amended on 2026-09-15 |

| # | Finding | Measured against |
|---|---|---|
| 173 | **Task 16's Step 2 code cannot run against the proof app**: `requireSignIn` does not exist — the guard is `signedIn` — and the PUID is `req.user.user.ubcEduCwlPuid`, not `req.user.ubcEduCwlPuid` | `fixtures/proof-app/server.js` |
| 174 | **`demo-identity.sh`'s step 9 asserted the `/api/ai` 501 stub Task 16 replaces**, and the task's *Files* block does not list the script — P4a's acceptance would have gone red on the first run after this one | The script |
| 175 | **Step 3's `GET /spend/logs?user_id=…` filters on the wrong identity**: `user_id` is the LiteLLM user the key was minted under — `mf-<projectId>-<env>`, the APP — and the person is each row's `end_user`, which `/spend/logs/v2` filters on | LiteLLM 1.98.0's OpenAPI document; the rows |
| 176 | **A spend row carries none of the key's `metadata`**, so `manifest_project` cannot be read back from spend — S3 Evidence 2's round trip is `/key/info`. **The first run of `make demo-ai` failed at step 9 on exactly that guess** | Every field of a real row, searched for `manifest` |
| 177 | **THE BLUEPRINT'S `embed()` CHARGED EVERY EMBEDDING TO NOBODY.** `embed(texts)` sent no `user`, so the first run's two `default-embed` rows had an EMPTY `end_user` while their chat rows carried the person — spend on a student's behalf outside §10's attribution, in every app generated from `node-ts-mongo@1`. The wire test pinned the body without a `user` and the parsed-call guard checked `user` on chat calls only | The spend log; fixed in `skeleton/ai/llm.js`, `AGENTS.md`, both tests; controls (j) and (f) |
| 178 | **`POST /projects` commits the project row and then fails** when the slug's bare repository survives on disk — after every `pnpm test` and every `make reset`, neither of which removes `.manifest/repos` — answering `SOURCE_GIT_FAILED` with git's raw stderr, host path included. The demos print `reusing project` for a project created a second earlier. P2's `createProject` and `createRepository` are not atomic. Named, not fixed | 0 rows before the POST, 1 after, the envelope read |
| 179 | **Node 24.12.0's global `WebSocket` sends a `cookie` header passed in `{ headers }`** — sitting 9's note left it unmeasured — and reports a refused upgrade only as `error` and close `1006` | A local server printing the upgrade request |
| 180 | **An earlier run's note stood in as this run's context** on the second run — equally similar — while the assertion was right: P4a defect 74's shape. The demo now says which | Run 2's output |
| 181 | **NEGATIVE CONTROL (e) DOES NOT FAIL: A QUESTION WAITS 611 SECONDS.** With `manifest-litellm` detached from the app network, the app's pooled socket — `ESTABLISHED`, 1,149 bytes in its send queue, retransmitting to an address nobody holds — waits out the OpenAI SDK's 600 s timeout, and `ubc-genai-toolkit-llm` 0.7.0 constructs its client with no `timeout` or `maxRetries` and exposes neither; neither the app nor the edge bounds it. With no pooled socket the same failure answers in 16.5 s; a refused connection in 1.2 s. Recreating the gateway container would do this to every AI app at once. Named, not fixed — *What this plan does not build* | `/proc/net/tcp` and `getent` inside the container while it hung; control (e′) and (d) |
| 182 | **The unit controls' first run did not run**: the command was passed as `$VT`, which zsh does not word-split — exit 127, caught by the harness printing it | The harness output |
| 183 | **`pnpm test` orphans live AI keys**: it truncates the control plane's tables and touches nothing in LiteLLM, so a key for a project whose row was gone stayed live, held by a container still running. `make reset` clears them — LiteLLM's database is in the volume it destroys: **0 `mf-` users and 0 keys** after it | `/user/list` and `/key/list` before and after |
| 184 | **A pattern-count loop ran with every argument empty** — `set -- $pair` in zsh, 182's trap one command later | The loop's `ENOENT` |
| 185 | **This sitting's first error mapping could never see a dead connection**: it classified on `details.type === 'APIConnectionError'`, and the toolkit reports `'Error'` for every SDK failure — the SDK's error classes set no `name`. A connection failure is `code` 500 with the SDK's message `'Connection error.'`. Control (e) therefore answered `AI_UNAVAILABLE`; fixed, and (e′) and (d) answer `AI_BACKEND_UNAVAILABLE` | A toolkit client in the app's container pointed at an unresolvable host |
| 186 | **Control (a) names `LLM_PROVIDER`, not the plan's `LLM_API_KEY`** — the first variable `configureAi()` reads — and it fails at first boot only because the proof app configures AI unconditionally; the skeleton's guard would have started it healthy | The stored Incident: exit 1, readiness 502 after 75 attempts, the sentence naming `ai.models`, "never been healthy in staging" |
| 187 | **Control (e′)'s first run could not sign in after `docker restart`**: its health wait was `curl -sS … && break`, which exits 0 on a 502, so it stopped waiting at once. The app was healthy 38 s later | The edge's 200 and 302 read afterwards |
| 188 | **Step 9's failure text blamed a missing EMBEDDING row when control (h) removed the CHAT row.** It now counts both | Control (h)'s output |
| 189 | **Every control redeploy leaves a container** — seven proof-app app containers attached to its network by the end of the batch (RUNBOOK's leaked-container gap; P4c) | `docker network inspect` |
| 190 | **`make reset` does not remove `.manifest/repos`**, so a run from a reset machine is not from nothing unless the repositories are moved — and without the move it runs through 178 | `ls .manifest/repos` after the reset; the from-nothing run printed `project`, not `reusing project`, once they were moved |
| 191 | **Task 16 contradicts itself on the embedding model**: Step 1 declares `default-embed`, Decision 14 asserts 768 from an embedding, and Step 2's code never embeds | The plan |
| 192 | **A cleanup near-miss, and two Docker facts.** Removing the leaked proof-app containers, `docker ps --filter 'name=^mf-proof-app-staging-' --filter 'name=-app$'` also listed the fixture-app container and the proof app's DATABASE and egress — repeated `name` filters are OR'd. The removal was a no-op only because zsh passed the list as one argument and `docker rm -f` exits 0 for a name that does not exist, so it printed "removed" too. Redone by explicit name, read off the edge's upstreams; ORIENTATION §4 has both facts | All nine containers listed Up afterwards |
| 193 | **This sitting's own "200 through the edge" could not fail.** After the Docker tier, `curl -o /dev/null -w '%{http_code}'` answered 200 for both demo hostnames — and the body was the edge's wildcard page, `manifest OK host=proof-app.staging.manifest.internal`, because `routes.docker.test.ts` restarts `manifest-caddy` and a restart drops every runtime route. The edge held no route to either demo, and one stale route to a chem-labs container the tier had already removed. Neither demo is reachable after a Docker-tier run until it is redeployed; the stopped control plane would re-apply routes only at boot, and the tier truncated the rows it would read | The response body; Caddy's `/config/` listing one `@id`, `mf-chem-labs-staging-manifest-internal` |
| 194 | **RUNBOOK's leaked-container workaround leaves a private key behind.** `docker rm -f -v` removes a container's anonymous volumes, and each app instance's `/manifest` files volume is NAMED — `mf-<slug>-<env>-<release>-app-files` — so removing the three leaked proof-app containers left three volumes, attached to nothing, each holding that instance's copy of the SP private key and the IdP certificate. Removed by name; the workaround now removes them too. `make reset`'s `name=^mf-` volume sweep does remove them | The end-of-sitting snapshot's volume list, and `ls -ln` inside each volume before it was removed |

**Negative controls — 12, every file mutation applied by a script that refused unless its pattern matched exactly once, restored byte for byte and hash-checked after every run; three runs did not run at first and are 182, 184 and 187.**

| Control | Result |
|---|---|
| (j) `embed()` sends no `user` | **RED** ×3 — the parsed-call guard, the wire body, the refusal |
| (f) `embed()` without `encoding_format: 'float'` | **RED** ×3 |
| (k) a second, bare-hash `endUserId` in the proof app | **RED** ×2 |
| (b) `data.classification: confidential` | **RED** at validation — `SPEC_MODEL_CLASSIFICATION_TOO_LOW` for `default-chat` AND `default-embed`, with the BC public-body hint |
| (a) `ai.models` removed | **RED** — the deploy `failed`, and the Incident's log tail names `LLM_PROVIDER` and `ai.models` (186) |
| (c) the PUID taken from the request body | **RED** at step 8 — the instructor, naming the student, was answered from the STUDENT's note: a leak of her data through the model, not only her spend |
| (i) the context query without `{ owner }` | **RED** at step 7 — the student was handed the instructor's note |
| (h) `askStreaming()` charges a bare PUID hash | **RED** at step 9 — only the embedding row on the student's identifier |
| (g) the subscriber connected after the deploy | **RED** at step 5 — "the live events were: (none)", with 25 replayed events correctly not counted |
| (e) the gateway detached, a pooled socket | **RED after 611.4 s** — `502 AI_UNAVAILABLE` (181, 185); before 200 in 3.8 s, reattached 200 in 8.6 s |
| (e′) the gateway unreachable, no pooled socket — paired | before **200** in 3.4 s; during **`503 AI_BACKEND_UNAVAILABLE`** in 16.5 s, a sentence, the log line carrying only class and status; after **200** in 3.3 s |
| (d) `LLM_ENDPOINT` at the control plane's loopback URL | **RED** — the container received `http://127.0.0.1:7106/v1` and answered `503 AI_BACKEND_UNAVAILABLE` in 1.2 s; its positive is the from-nothing run with the default restored |

The unmutated demo passed after the batch, and `git status` was unchanged by it.

**Run from nothing, with Rich's authorisation.** The overridden control plane stopped; `make reset` (exit 0) and `make up`; LiteLLM **0 `mf-` users, 0 keys**; `.manifest/repos` moved aside; the control plane restarted with migrations — `{"driver":"docker","port":7100,"ai":"enabled"}`; then `make demo-identity` (a new project), `make demo-ai`, `make demo-identity`, `make demo-ai`, all exit 0 — both `demo-ai` runs with five live events in order, 73 of 73 log lines, 768 dimensions, and one chat and one embedding row per person on their identifier — and LiteLLM held **1 `mf-` user and 1 key** after four AI deploys, so every commit revoked what it replaced; then `make demo`, exit 0.

**Checked, and changed:** `manifest-phases.html` — "the AI half is what remains" was true until this task — and `manifest-schematic.html`'s *Status* line; `docs/external-track.md` gains a dated line saying the start condition is met and nothing is raised. **Checked, and unchanged:** `manifest-decisions.html` (no decision changed) and `manifest-stories.html` (no hostname). **Added to:** RUNBOOK — `make demo-ai`, three *Known gaps* and step 7 of the offline acceptance; ORIENTATION §4 — six durable facts.

**Gates at the end:** `pnpm test` **736** (66 files, twice) — one more than sitting 9, the refusal of `embed()` for nobody — lint / typecheck / `format:check` clean, `pnpm test:docker` **133 of 133** (25 files, 478 s)**, `make doctor` **18/0** and `make verify` **47/0**, both after the Docker tier. **The machine at the end:** differs from its start-of-session snapshot by the reset Rich authorised and what the demos rebuilt: the five-day-old exited `mf-` app containers and their files volumes are gone; fixture-app and proof-app each have one running app container beside their recreated database and egress proxy, both unrouted since the Docker tier restarted the edge (193); the platform containers were recreated by `make reset`'s `compose down` and now show their image names rather than stale image IDs; the two new anonymous volumes are the recreated databases'; and `.manifest/repos` holds the two demos' new repositories, the old ones having been moved into this session's temporary scratchpad. **Unchanged:** the four protected containers — `docker-simple-saml-saml-idp-1` still exited — the `credit-card-helper` pair, and the image list apart from the two images the demo containers run; the sixteen others this sitting built were removed by every repo digest each carried. **Cleaned up, and recorded:** the three leaked proof-app containers (192) and their `-app-files` volumes, each holding a private key (194); and the orphaned LiteLLM user `mf-1c5c2ed2-…` with its one key, whose project row the gates had truncated (183), so LiteLLM ends as sitting 9 left it — `p4b-probe-user` and **0 keys**. The control plane is stopped, as it was found. One stale edge route, `mf-chem-labs-staging-manifest-internal`, is the Docker tier's (193). Free disk 147 → 146 GiB; HEAD `78f65f9`, with this record's nine documents uncommitted when the snapshot was taken.

**P4b is finished. P4c — zero-downtime redeploys — is next, and unwritten**; finding 181 and the leaked containers of 189 are two of its inputs.
...
**Limits inside what is built, stated so nobody mistakes them for coverage.**
...
- **The stream authorizes at connect, not per frame** (sitting 9). A member removed from the project, or a session that expires, keeps receiving the project's frames until the socket closes. A per-frame check is a database read per frame; a maximum connection lifetime would bound the window, and nothing sets one.
- **Build-log lines are streamed live and never replayed.** A connection is replayed the newest 50 events; a client that connects mid-build reads the lines it missed from `GET /builds/:buildId/logs`.
- **A deploy that fails before an instance exists streams nothing** — an image the daemon cannot pull, a container it refuses. The caller gets the error envelope, the instance row stays in `provisioning` as sitting 8 left it, and there is no `deploy.failed` type.
- **A frame published inside a transaction that is later rolled back has already gone out.** Every production call site writes on the pool; `publishEvent`'s own comment names it.
- **An app whose gateway vanishes under a kept-alive connection waits ten minutes for its answer** (sitting 10, finding 181). Measured with `manifest-litellm` detached from the proof app's network: the question hung **611.4 s**, because the OpenAI SDK's pooled socket retransmits to an address nobody holds until the SDK's 600 s timeout, and `ubc-genai-toolkit-llm` 0.7.0 builds its client with no `timeout` or `maxRetries` and exposes neither. With no pooled socket the same failure answers in 16.5 s, and a refused connection in 1.2 s. A deadline belongs in the blueprint's AI component, where every generated app inherits it, and needs a decision about stream semantics and the abandoned request; nothing in Phase 1 builds it. The realistic trigger is anything that removes the gateway from app networks — recreating `manifest-litellm` drops every attachment until each app redeploys — which is P4c's territory.
- **`POST /projects` can fail after it has committed the project row** (sitting 10, finding 178). With a bare repository already on disk for the slug — which is every developer's machine after `pnpm test` truncates the rows and after `make reset`, neither of which removes `.manifest/repos` — it answers `SOURCE_GIT_FAILED` with git's raw output, host path included, and leaves the row. Both demos recover by reusing the project. P2's `createProject` and `createRepository` are not atomic; not this plan's to fix.
- **`pnpm test` orphans live AI keys** (sitting 10, finding 183). It truncates the control plane's tables and touches nothing in LiteLLM, so the users and keys of every project it removed stay live — measured: a key for a project whose row no longer existed, confined and budgeted, held by a container still running. `make reset` does clear them, because LiteLLM's database is in the volume it destroys. Reaping is the reconciler's (Phase 4, D10).
...
**One applied 2026-09-14, with Rich's approval: the §21 LiteLLM row**, because Task 2 had already pinned the digest. **The other five are held until the tasks that implement them have run** — Tasks 6, 7, 9 and 14 — because plan-time findings have moved during execution before. **Tasks 6 and 7 ran on 2026-09-14 (sitting 4), and Rich settled both of their §7 rows the same day. Both are applied**, in one spec commit, and two of his three answers differ from what was built. The mechanics went in as proposed; an unclassified catalogue entry refuses only the model that names it, not the whole catalogue; and an omitted project budget is defaulted to the project's AI quota instead of refused. The code those need is the *SPEC DECISIONS* block at the top of Task 9. Two of them are product decisions for Rich at that point: the per-user budget §10 cannot enforce, and what streams over WebSocket. **Task 14 ran on 2026-09-15 (sitting 9), and Rich decided the last three rows the same day — all three applied as proposed, in one spec commit.** §10's end-user row is validated, not enforced, in Phase 1, with the measured reason beside it; §10's agent-key row binds from Phase 3; and §14's streaming bullet describes the stream as built and says tailing an application's own output is not v1. He chose each over the alternatives put to him: refusing the budget field, or enforcing it before Task 16; streaming application output, or narrowing the stream to D23.2's list; leaving the agent-key row, or building a TTL with no caller. What sitting 9 built for the §14 row: the stream carries D23.2's list less approval decisions (P6's) — build logs, instance state transitions and incidents — plus §9's SSO registrations and §10's key rotations, and no application stdout. The table is the original proposal, kept as the record.
