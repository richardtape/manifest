...
**READ THE FOUR BLOCKS BELOW IN ORDER, AND WHERE THEY DISAGREE THE LATER ONE WINS (2026-09-14).** The last of them, Rich's *DECIDED* block, replaces `rotateAppKey` with `mintAppKey`, `commitAppKey` and `discardAppKey`. So wherever an earlier block, the *Interfaces* list or the plan's original steps say `rotateAppKey`, read those three: mint before `ensureInstance`, commit after health passes, discard on failure. Sitting 4's empty-models refusal moves with `mintAppKey`.
