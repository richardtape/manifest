...
| **9 ← next** | **14–15** | `WS /projects/:projectId/events`, then the call sites that make it carry anything. **Never split these.** Task 15's own title is *"the task that makes the stream carry anything"* — a stream with no publisher is the call-site defect again, and it would sit green for a whole session | **No network needed. A note from sitting 8 is at the top of Task 14 — read it first, it corrects Task 14's own code; hand-off in ORIENTATION §7d-2** |
