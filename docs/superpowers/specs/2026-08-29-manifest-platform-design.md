...
| D33 | **Code review of app code gets an interface in Phase 2 and implementations later**, shipped as a null implementation whose verdict states that no review was performed. The `LaunchReadiness` item it feeds is non-blocking until a real implementation exists. | D9 accepts that the gate reviews `manifest.yaml` and not code, and containment (§20) is what makes that tolerable — but whatever reviews code eventually has to run inside the build or approval path, work offline (C1), and produce a verdict both the gate and the checklist can read. Retrofitting that path later touches the builder, the approval record and `LaunchReadiness` at once, which is D30's argument applied to a second seam. Shipping it empty is deliberate and so is its honesty: a placeholder that *appeared* to review would be another setting that reads like a control and is not, a class this project has already measured four instances of. The seam changes no risk on its own, and §20's control map says so. |
...
only changes to the sensitive fields §7 lists re-escalate. This is an accepted trade
...
- **Step-up re-authentication** for the privileged set — promoting a release to
  ...
  project membership — plus the admin-only actions of changing the model catalogue
  ...
  Approving a release is not one of them — it is a separate decision from promotion
  (§13) — and belongs to a stricter class, the *person-only* actions of D24, which no
  token can hold at all; the two admin actions are additionally restricted to
  platform admins. Keeping the lists aligned is a test, not a convention. **The list is named once and enforced
...
| Unreviewed code reaching production | **Still accepted under D9** — containment remains the control: egress default-deny, network isolation, least privilege, edge controls. A `Reviewer` seam exists from Phase 2 with **no implementation behind it** (§15, D33); static analysis and diff review are tracked hardening items, and **until one lands nothing reviews code**. |
