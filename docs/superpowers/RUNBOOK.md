...
(**doctor and verify were both UNMOVED by P6a sitting 7 on 2026-09-20 — Tasks 10–11 add three routes and no platform check — and `pnpm test` went 1496 → 1544 passed plus the same one skipped, in 114 files, up 48 and TWO new files: `releases/approval.test.ts` 16 and `releases/summary.test.ts` 5, plus +27 in `api/authz-contract.test.ts` (three routes × nine actors). `pnpm test:docker` was OWED, RUN and UNMOVED at 185 in 30, 823 s — and unmoved is the measurement there for the third sitting running, CHECKED rather than asserted: grepping all thirty `*.docker.test.ts` for the approval returns one hit and it is a test NAME. Before that, doctor and verify were both UNMOVED by P6a sitting 6 on 2026-09-20 — Tasks 8–9 add one unversioned route and a guard, and no platform check — and `pnpm test` went 1456 → 1496 passed plus the same one skipped, in 112 files, up 40 and TWO new files: `identity/step-up.test.ts` 9 and `projects/step-up-guarded.test.ts` 10, plus +8 in `api/auth.test.ts`, +9 in `api/authz-contract.test.ts` and +1 in `api/error-codes.test.ts`. `pnpm test:docker` was OWED, RUN and UNMOVED at 185 in 30, 829 s — and unmoved is the measurement there, because no `*.docker.test.ts` makes a member call over HTTP or drives `/auth/step-up`, so that tier cannot see the step-up guard at all. Before that, doctor and verify were both UNMOVED by P6a sitting 5 on 2026-09-20 — Task 7 changes a refusal and adds no platform check — and `pnpm test` went 1449 → 1456 passed PLUS THE SUITE'S FIRST SKIPPED TEST, still in 110 files and with no new file: `launch/readiness.test.ts` 8 → 15, and `api/delivery.test.ts` gained the skip, which is the production gate's positive control and names Tasks 10 and 14 as the ones that un-skip it. `pnpm test:docker` was OWED, RUN and UNMOVED at 185 in 30, 826 s — and unmoved is the measurement there, because that sitting DELETED a production refusal inside `deployRelease` and not one Docker test deploys to production. Before that, doctor and verify were both UNMOVED by P6a sitting 4 on 2026-09-20 — it adds three API routes and no platform check — and `pnpm test` went 1395 → 1449 in 110 files with it, from migration 0019's two `launch/` test files and three new routes' worth of authorization-matrix rows, while `pnpm test:docker` was OWED, RUN and UNMOVED at 185 in 30, 830 s. Before that, doctor and verify were both UNMOVED by P6a sitting 3 on 2026-09-20, which put its new port-equality assertion INSIDE `make verify`'s *the public listener answers a production name* check rather than adding a check — so the same claim is not counted twice; `pnpm test` went 1390 → 1395 and `pnpm test:docker` 180 → 185 in that sitting, up 5 each with NO new file. Before that, **moved 2026-09-19/20 by P6a sitting 2, §12's second listener** — doctor **18 → 19** (the single-alias check became a two-alias loop, and the nested-zone split gained a check asserting BOTH directions) and verify **51 → 54** (two servers, the public listener answers a production name, neither listener answers for the other's zone). **`pnpm test:docker` moved to 180 in 30 files, 848 s**, and `pnpm test` did NOT move — 1390 in 108, twice; before it **P6a sitting 1, which re-ran all four and moved none** — `pnpm test` 1390 in 108 twice over, `make doctor` 18/0, `make verify` 51/0, `pnpm test:docker` neither owed nor run; before it **P5c sitting 9, the acceptance — which moved `pnpm test` to 1390 in 108 files** when it fixed F11's single-logout defect, and ran `pnpm test:docker` TWICE (178 in 29 both times, unchanged); before it, sitting 8 moved `pnpm test` and left the other three where it found them; sitting 3 measured doctor and verify **in both of the two machine states that now exist — the reference console running on 7104, and 7104 free** — and they are unchanged since P5a sitting 11. The unit tier is **1544 passed plus 1 skipped, in 114 files** since P6a sitting 7 (Tasks 10–11, two new files), was **1496 passed plus 1 skipped in 112 files** from P6a sitting 6 (Tasks 8–9, two new files), was **1456 passed plus 1 skipped in 110 files** from P6a sitting 5 (Task 7, and no new file), was **1449 in 110 files** from P6a sitting 4 (Tasks 5–6: `launch/transitions.test.ts` 13, `launch/records.test.ts` 13, +27 in `api/authz-contract.test.ts` and +1 in `projects/privileged.test.ts`), and was **1395 in 108** since P6a sitting 3 (Task 4: four in `routing/routes.test.ts`, which exercises two distinct Caddy server names for the first time, and one in `config.test.ts` for `MANIFEST_EDGE_PUBLIC_PORT` — no new file), and was **1390 in 108** — *this sentence said 1376 in 107 for a day, contradicting the same sentence's own opening clause, and P6a sitting 1's post-sweep check found it; sitting 8's four new files were `packages/mock/src/validate.test.ts`, `packages/mock/src/server.test.ts`, `packages/console/src/api.test.ts` and `packages/console/src/coverage.test.ts`, and sitting 9 then added `api/logout.test.ts`.* `pnpm test:docker` is **185 in 30 files, 0 skipped** — **823 s at P6a sitting 7**, which owed the tier for `releases/`, `api/routes/` and `ai/` and found it unmoved; **829 s at P6a sitting 6**, which owed the tier for `identity/`, `sso/`, `projects/` and `api/routes/` and found it unmoved; **826 s at P6a sitting 5**, which owed the tier for `releases/` and `launch/` and found it unmoved; **830 s at P6a sitting 4**, which owed the tier for migration 0019 and found it UNMOVED, and 827 s at P6a sitting 3, which owed the tier and added FIVE tests to `routing/listener-split.docker.test.ts` and no file — the first here to put a REAL route on each listener; it was **180 in 30, 848 s** from P6a sitting 2, which added that file; it was **178 in 29, 795 s** from P5c sitting 8, run then because its `make doctor` repair touches `infra/lib/common.sh` and `routing/edge-source-refusal.docker.test.ts` reads that file; sittings 4 to 7 neither ran nor owed it.) ORIENTATION §2's box is the maintained copy of those; if this
