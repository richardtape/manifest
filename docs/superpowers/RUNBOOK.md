...
(**All four were re-measured at the close of P6b sitting 2, 2026-09-23: `pnpm test` **1635 passed** in 121 files (up from 1605 in 119), `make doctor` 19 — **with one warning, which is true**: the vulnerability database is past §12's seven days and `make seed` with the network refreshes it — `make verify` 55, and `pnpm test:docker` **194 in 31**, whose one red was the calendar and was fixed the same sitting.** This parenthetical states only the LATEST sitting — it had grown to 6 KB of per-sitting history before sitting 8 replaced it, and every sitting's numbers are in its own plan record, dated.) ORIENTATION §2's box is the maintained copy of those; if this
