...
(**All four were re-measured at the close of P6b sitting 3, 2026-09-23: `pnpm test` **1651 passed** in 122 files (up from 1635 in 121), `make doctor` 19 with **0 warnings** — the vulnerability database was refreshed at the sitting's open and is stale again after 2026-09-30 — `make verify` 55, and `pnpm test:docker` **198 in 31**, whose two reds were a P4c test rate-limited by the edge it tests (fixed, `a7948fd`) and an S6 probe that needs Ollama's chat model resident (recorded).** This parenthetical states only the LATEST sitting — it had grown to 6 KB of per-sitting history before sitting 8 replaced it, and every sitting's numbers are in its own plan record, dated.) ORIENTATION §2's box is the maintained copy of those; if this
