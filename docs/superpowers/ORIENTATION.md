...
- **AN ADMINISTRATOR SEES THE APPROVAL'S DIFF ONLY AFTER DECIDING** (P6a sitting 10, F15). `buildDiffSnapshot`'s only caller is the approve/reject call, so §13's *"the exact diff shown at decision time"* is shown after it; the console says so in its own words and shows the release's production configuration first. **Rich decided on 2026-09-22: a STORED preview that the approval binds, in P6b** — the approve request names the preview and the platform refuses if what it would record now differs. *(§8, Decided; P6b's Tasks 9 and 10 build it.)*
...
### 7e. Execute P6b's sitting 6 — Tasks 9 and 10 ← **START HERE**
...
naming it, and an added CWL attribute also waits for UBC IAM. **Sittings 1 to 5 are done** — the measurements
(2026-09-22), then Tasks 2 to 8 (2026-09-23). **Since sitting 5** the IAM change request is the registration's own
`change_requested` state, a launched app's `iam-registration` is the LIVE check (an added attribute waits for UBC,
a removal re-escalates and waits for nobody), and every approval snapshot carries R4(d)'s security dimension — its
baseline, the sensitive fields, a note for each, D33's coverage limit — while `code-review` reads the newest
verdict. **Your job is sitting 6: Task 9 — the stored preview (migration 0023), which approve and reject must name
— and Task 10 — the console's approvals screen, the preview BEFORE the decision, kept through the step-up.** It is
the plan's other heavy sitting, and the two go together so the console's Approve button is never broken across a
session boundary: **if it runs short, stop after Task 9, sweep, and STATE THE BROKEN BUTTON in this paragraph.**
Use `superpowers:executing-plans` and **commit on `main`, one commit per task** (§6 rule 9).
**`pnpm test:docker` IS OWED** (`releases/`) — budget ~25–35 minutes for it after the commits (1575 s in sitting 5).

**READ, IN THIS ORDER:** the plan's sittings table; **sitting 5's record** at the end of *What executing this plan
found* (F2, F6 and F7 matter here); then **Task 9 in full, starting with the block at its top — sitting 5 left SIX
things in it**; then Task 10, whose block is one paragraph and Task 1's F10.

**WHAT SITTING 5 LEFT IN TASK 9 — READ THE BLOCK, THIS IS ONLY THE LIST:**

- **The snapshot has four more facts** — `baselineReleaseId`, `sensitiveFields`, `security`, `coverage` — so
  `DiffFacts` carries the baseline, which is the fact a stale preview differs in.
- **`buildDiffSnapshot`'s order must survive the split into facts and annotations**: one baseline read, the
  reviewer BEFORE the model, and `summariseChanges`' third argument.
- **`latestReviewFor` gains `approval_previews`** as its second source; reword the no-verdict sentence to *previews*.
- **TWO fixtures approve without a preview**: `approvedProject` AND sitting 5's `launchedCwlProject`, both in
  `api/testing.ts`, which the Files list's `*.test.ts` grep does not find.
- **A decision's `500` can arrive after its row is written** (sitting 5, F7) — the preview route will do the same.
- **The mock's `APPROVAL` fixture already carries the four keys**; `ApprovalPreview`'s needs them too.

**THE THINGS MOST LIKELY TO COST YOU:**

- **FIRST: is the machine quiet? Read `uptime` before you believe a red.** A Zoom screen share (load 50–90)
  made P6b sitting 3's unit tier unmeasurable for an hour, every red a 5000 ms timeout (§4).
- **Is the vulnerability database still fresh?** It is until **2026-09-30** (§2 *Outstanding*). Past that,
  `make doctor` warns and §13's `scans` item refuses every production launch, so `make demo-production` goes red
  at step 3 — ask Rich to refresh it (network) before any step that runs the demo.
- **Your first test run is the baseline, so run the gates before touching code**: `pnpm test` twice (**1690
  in 122**), `pnpm lint`, `pnpm typecheck`, `pnpm format:check`, `make doctor` (**19 checks, 0 failed, 0
  warnings** while the database is fresh) and `make verify` (55). A different number is signal before it is
  your fault.
- **Migration 0023 must be APPLIED before the unit tier can see it** (P6b sitting 3, F4; sitting 5 did it for 0022):
  `db:migrate` with `MANIFEST_ADMIN_DATABASE_URL` exported (§3), or every insert into the new table fails.
- **Predict every control's reds by COUNT and by CODE before running it.** Sitting 5 found the plan under-counting
  five of its ten controls (F8), and one it called *"a control only `tsc` can see"* went red at runtime too,
  because **every success body is parsed against its representation** (`api/contract/route.ts:222`, F6).
- **Run a control runner under `bash`**: zsh does not word-split an unquoted `$F`, and the run answers *"No test
  files found"* with an empty summary that reads as nothing happening (sitting 5, F13).
- **The error-code scan reads COMMENTS** (sitting 4, F4): never write `new SomeError('CODE'` in prose — Task 9 adds
  three codes.
- **Task 10 has no DOM test tier**: open the screen against `manifest-mock` (RUNBOOK, *Running `manifest-mock`*;
  `MANIFEST_MOCK_ROLE=admin` shows an administrator's controls), and read the summary on screen for a literal `**`
  (Task 1's F10). **Two Vitest processes cannot run at once**, and **an unset variable in a `cp` destination writes
  into the repo root** (sitting 4, F13).

**THE MACHINE, AS SITTING 5 LEFT IT — query every one of these again; do not work from this line.**
**`make doctor` 19/0 with 0 warnings** (database built 2026-09-23, stale after 2026-09-30), **`make verify`
55/0**, **`pnpm test` 1690 in 122** (twice, quiet), `pnpm test:docker` **198 in 31, all passed** (1575 s), **23
migrations**, **the database EMPTY** (0 projects — the close's runs truncated it), **nothing listening on 7100, 7102
or 7104**, both loopback aliases on `lo0`. `launch-app` stands in production and staging on the close's fresh-demo
release — containers only: no rows hold them — with `click-launch` and journey-app's staging beside it; `make verify`
reads `mf- containers=15 networks=5 volumes=10`. **Sitting 6's demo runs fresh after a `pnpm test`, which rebuilds
what it needs.** The control plane was stopped at sitting 5's close, and it re-registers the platform's SP row at
every boot. **Seventeen stopped containers of six OTHER projects disappeared during sitting 5 by no action of its
own** (its record's *machine at close*) — if more go, it is not Manifest. **Docker Desktop may not be running when
you open** (`open -a Docker`, §4).
