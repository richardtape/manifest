...
### 7e. Execute P6b's sitting 5 — Tasks 7 and 8 ← **START HERE**
...
naming it, and an added CWL attribute also waits for UBC IAM. **Sittings 1 to 4 are done** — the measurements
(2026-09-22), then Tasks 2 to 6 (2026-09-23). **Since sitting 4 D9.2 is reachable through the ROUTE**: a launched
app's owner deploys a non-sensitive release to production with no administrator, a sensitive one is refused
`409 RELEASE_REESCALATED` carrying exactly what changed, and a release that is not the one serving staging is
refused `409 RELEASE_NOT_STAGED`. **Your job is sitting 5: Task 7 — the IAM change request (migration 0022) —
and Task 8 — R4(d), a security-aware summary carrying the reviewer's verdict.** This is the lean split's cost
(Rich, 2026-09-22): **if it runs long, stop after Task 7 and sweep** — Task 7 is where P6a hid its worst logic
hole. Use `superpowers:executing-plans` and **commit on `main`, one commit per task** (§6 rule 9).
**`pnpm test:docker` IS OWED** (`launch/`, `releases/`) — budget ~25–35 minutes for it after the commits (1407 s in sitting 4).

**READ, IN THIS ORDER:** the plan's sittings table; **sitting 4's record** at the end of *What executing this
plan found* (F2, F7 and F8 change how you write tests here); then **Task 7 in full, starting with the correction
block at its top — sitting 4 left FIVE things in it**; then Task 8, whose top block is Task 1's `[M12]`.

**WHAT SITTING 4 LEFT IN TASK 7 — READ THE BLOCK, THIS IS ONLY THE LIST:**

- **The launched branch exists** in `computeLaunchReadiness` and still calls the shared `iamItem`; Task 7 swaps
  that one call for `liveRegistrationItem`.
- **`iamItem` and `piaItem` say *"before a first production launch"*** on a launched app's checklist too (F14).
- **A representation field you forget is STRIPPED, not dropped** (F8): the byte-identical assertion stays green
  and `tsc` is silent, so each new field needs a route test that reads it by name.
- **Close every server in `api/subsequent-releases.test.ts` through `closed(ctx)`** (F7) — never
  `ctx.app.close()` alone, or a retire pass races the next test's `TRUNCATE` and reads as `deadlock detected`.
- **`candidateFor` requires a `healthy` instance** (F2).
...
- **Your first test run is the baseline, so run the gates before touching code**: `pnpm test` twice (**1662
  ...
  your fault.
- **Migration 0022 must be APPLIED before the unit tier can see it** (P6b sitting 3, F4): `db:migrate` with
  `MANIFEST_ADMIN_DATABASE_URL` exported (§3), or every insert into the table fails `42703`. And **its backfill
  line is appended by hand**, said so in a comment.
- **Predict every control's reds by COUNT and by CODE before running it.** Sitting 4 found the plan
  under-counting four of its seven controls (F10) and wrong about one's mechanism (F8); `error-codes.test.ts`'s
  *"registers nothing the source never throws"* and the console's data-layer test against the mock are
  second witnesses the plan's tables rarely name.
- **The error-code scan reads COMMENTS** (sitting 4, F4): never write `new SomeError('CODE'` in prose.
- **A production deploy needs the candidate SERVING STAGING**, and a `pnpm test` between a staging deploy and a
  production one truncates the project. **Two Vitest processes cannot run at once** (§4). **An unset variable in
  a `cp` destination writes into the repo root** (sitting 4, F13) — set `$SCRATCH` and check it.

**THE MACHINE, AS SITTING 4 LEFT IT — query every one of these again; do not work from this line.**
**`make doctor` 19/0 with 0 warnings** (database built 2026-09-23, stale after 2026-09-30), **`make verify`
55/0**, **`pnpm test` 1662 in 122** (twice, quiet), `pnpm test:docker` **198 in 31, all passed**, **22
migrations**, **the database EMPTY** (0 projects — the close's runs truncated it), **nothing listening on 7100, 7102
or 7104**, both loopback aliases on `lo0`. `launch-app` stands in production and staging on the fresh demo's
launch release — containers only: no rows hold them — with `click-launch` beside it and journey-app's staging,
which `make demo-journey` recreated; `make verify` reads `mf- containers=15 networks=5 volumes=10`. **Sitting 5's
`make demo-production` runs fresh after a `pnpm test`, which rebuilds what it needs.** The control plane was
stopped at sitting 4's open and close, and it re-registers the platform's SP row at every boot. **Docker Desktop
may not be running when you open** (`open -a Docker`, §4).
