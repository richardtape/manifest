...
`EXPECT_TESTS` is the PASSED count, so it reads 1493 and not 1494.** **`pnpm test:docker` was OWED,
...
ran on 2026-09-20 with 18 findings, and NO TASK BOUNDARY MOVED, so the eleven-sitting split
still stands.**
...
after both cleanup scripts. **`pnpm test` 1493 passed + 1 SKIPPED in 112 files**, run twice
