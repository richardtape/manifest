...
**The four numbers you will check first, measured 2026-09-16 on this machine, at the end of P5a sitting 5:**

| | |
|---|---|
| `pnpm test` (from the **repo root**) | **877 passed, 81 files**, ~50 s — the `unit` project and, since P5a Task 7, `packages` (the client and the journey, which need nothing running) — no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **168 passed, 0 SKIPPED**, 27 files, ~755 s — measured at the end of P5a sitting 3 and not re-run in sittings 4 and 5, which the plan exempts (they changed `api/` and added two packages, and no Docker test); sitting 3 added no Docker test and changed `identity/saml.docker.test.ts`'s two; sitting 2's S6 probe 15 is the 168th (the edge refuses an app the control plane, with the host as its control). The eleven skips that used to be here were the driver contract's continuity block, gone since P4c Task 5 gave the Docker driver its fixtures. Needs `make up`, and **fails rather than skips** when asked to run |
...
│                                                  SITTINGS 1–5 DONE, 6 NEXT. Its six spec
...
**P5a IS WRITTEN (2026-09-16) AND ITS SITTINGS 1 TO 5 ARE DONE, THE SAME DAY — YOUR TASK IS SITTING 6: TASKS 8 AND 9 (projects, environments, members and specs as public representations, with the two reads that did not exist; then §23's reserved labels and `GET /v1/slugs/{slug}` behind one slug function).**
...
**Read, in this order, before sitting 6:**

1. **The plan's** *How this plan is to be executed*, *Read this first*, *Decisions Rich made*, *Decisions this plan makes* (1–10 above all) and **sittings 1 to 5's entries in *What executing this plan found*** — then **Tasks 8 and 9 in full**, including the *Task 1 result* at the top of Task 9 (nothing to rename).
2. **[`plans/2026-09-16-p5-brief.md`](plans/2026-09-16-p5-brief.md)** §8 (the traps).
3. **This file's §4** — its last seven entries are sittings 3, 4 and 5's — and **§6** (how to work, and the close-out sweep every sitting owes).
...
**The state you are handed, 2026-09-16, after sitting 5.** `main`, clean. The code commits are Task 7's `97f6be4` and
`917dd5f`; everything later is documentation. The four gate numbers are §2's box — all green, `pnpm test` 877, `make verify`
50/0 and `pnpm test:docker` 168 (not re-run in sittings 4 and 5). **The proof app serves at
`https://proof-app.staging.manifest.internal/`, but with NO project rows behind it** — sitting 5's `pnpm test` truncated them,
and `make demo-identity` was deliberately not re-run, because recreating the project orphans the LiteLLM user the running
container holds, which this session could not delete (below). The next demo recreates the project from nothing. **The fixture app is not deployed** — sitting 2 removed what
`make demo` created — so `make demo` starts it from nothing. **The platform SP row's ACS is
`https://console.manifest.internal/auth/saml/callback`.** The control plane is **not** running (port 7100 is free);
README's *Running the control plane* starts it, and its boot line must name that origin. A redeploy is P4c's:
`POST /v1/environments/:environmentId/deploy` returns once the new instance serves and the old one drains in the background.
**One orphan is left for Rich: LiteLLM user `mf-9425a20b-036a-4158-8227-6096add8ca73-staging` and its one key** — sitting 3's
proof-app project, whose rows sitting 4's `pnpm test` truncated and whose container the demo replaced. (LiteLLM also holds
`mf-cd4468d1-5f71-4761-a863-6c7b53cbaf38-staging`, whose key the running proof app still uses; it becomes the next orphan the
next time a demo replaces that container.) Sitting 4's attempt to
delete it through LiteLLM's admin API was refused by the session's permission classifier as a secret-store write. It is
unused and harmless; to remove it, from the repo root: `set -a; . ./.env; set +a`, read its hashed token with
`curl -sS -H "authorization: Bearer $LITELLM_MASTER_KEY" "http://127.0.0.1:7106/user/info?user_id=mf-9425a20b-036a-4158-8227-6096add8ca73-staging"`,
then `POST /key/delete` with `{"keys":["<token>"]}` and `POST /user/delete` with `{"user_ids":["mf-9425a20b-036a-4158-8227-6096add8ca73-staging"]}`,
