...
*Last verified 2026-09-19 (P5c sitting 9, the acceptance — **no gate number moved**: sitting 9's only code change is two shell scripts, so `pnpm test:docker` was neither owed nor run. The figures below were re-measured at its baseline and again before its commits. Previously (sitting 8; **`pnpm test` MOVED — 1376 in 107 files, up 21 and four files, which is this sitting's four new test files; run twice after each of the two task commits. `pnpm test:docker` **178 in 29 files, 0 skipped**, 795 s — it was OWED and RUN, because the doctor repair touches `infra/lib/common.sh` and `routing/edge-source-refusal.docker.test.ts` reads that file. `make doctor` 18/0 and `make verify` 51/0, re-run at close — and doctor read **1 FAILED** in between, which is this sitting's F9.*)
...
- **F11 IS A LIVE DEFECT IN A SHIPPED CAPABILITY, AND IT IS RECORDED, NOT FIXED.** Signing out of
  any deployed app leaves the person on a **raw JSON `404`**: Manifest's own SP publishes
  `/auth/logout` as its Single Logout URL (`sso/entity.ts:117`) while `api/unversioned.ts:22`
  declares that path **`POST` only**, and SAML's HTTP-Redirect logout binding sends a **`GET`**.
  Measured both ways: `POST` → `204`, `GET ?SAMLRequest=` → `404`. **Manifest's own session is not
  ended by the chain.** Every gate is green through it — 1376 tests, doctor 18/0, verify 51/0,
  three clean `ci-acceptance` runs — because **no tier signs out of a deployed app in a browser**.
  It is in §8 as a decision for Rich: fix it as a hardening item now, or fold it into P6.
...
| | |
|---|---|
| The four gate numbers | §2's box, and they did **not** move this sitting: `pnpm test` **1376** in **107** files, `pnpm test:docker` **178** in **29**, `make doctor` **18/0**, `make verify` **51/0**. **`test:docker` was NOT owed and NOT run** — sitting 9 changed no Caddyfile and no app route; its only code change is two shell scripts |
...
| Identity | **`users` holds TWO rows** — `ins000001` and `stu000001` — because the acceptance signed both in. **`pnpm test` empties it**, and `POST /v1/projects/{id}/members` answers `400 MEMBER_USER_NOT_FOUND` for anybody who has never signed in **to MANIFEST** (F14: signing in to a deployed APP is a different SP with a different user store, and does not count) |
...
- **Manifest's own Single Logout endpoint answers `POST` only, and SAML's logout binding sends `GET`. Fix it as a hardening item now, or fold it into P6? — RAISED 2026-09-19 (P5c sitting 9, F11).** Signing out of ANY deployed app leaves the person on a raw JSON `404`, and **Manifest's own session is not ended by the logout chain**. `sso/entity.ts:117` publishes `https://<hostname>/auth/logout` as the SP's `sloUrl`; `api/unversioned.ts:22` declares that path `POST` only, with the reason *"The SLO URL registered beside the ACS (§9)"*. Measured both ways: `POST` → **`204`**, `GET ?SAMLRequest=…` → **`404`**. **Every gate is green through it** — 1376 tests, `doctor` 18/0, `verify` 51/0 and three clean `ci-acceptance` runs — because no tier signs out of a deployed app in a browser; only a person clicking found it. It is asked rather than fixed because the repair touches the SP contract every deployed app is registered under, and because P6 owns that area next.
