...
| `make verify` | **55 checks, 0 failed, 0 warnings** — **unmoved by all seven P6b sittings and by the F10 sitting**; up one in P6a sitting 10, three in P6a sitting 2 for §12's second listener. **Straight after `make reset` it reads ONE red — the audit grant — until the control plane has migrated the empty database** (§4). **It reads `mf- containers=6 networks=2 volumes=4` today** — only `launch-app`, re-launched by the F10 sitting's `make demo-releases` after its Docker tier truncated the tables, and with the seven networks and one volume that tier regenerated removed by `dead-app-resources.sh --apply` — and `runtime routes currently applied: 1`. **That meter counts `srv0` only**, so it never sees a production route on `srv1` (P6b sitting 1, F13) |
...
### 7e. Execute sitting 1 of the D5 GitHub source driver plan ← **START HERE**

**THE PLAN IS WRITTEN (2026-09-24, `7045a99`): [`plans/2026-09-24-d5-github-source-driver.md`](plans/2026-09-24-d5-github-source-driver.md)** —
D5's driver 2, **15 tasks in Rich's eight sittings**. Nothing is executed. **Your job is sitting 1: Task 1, the
measurements, ALONE.** It writes no feature code. Its output is `docs/superpowers/spikes/d5-baseline/`, plus an
`[M<n>]` correction block at the top of any task a measurement contradicts. Use `superpowers:executing-plans`
(or subagent-driven), and **commit on `main`**.

**READ, IN THIS ORDER:** the plan's header, its sittings table, *Decided by Rich*, *What Rich does, and when*,
*Read this first* (twenty facts, most of them measured while the plan was written, each marked *(T1: M<n>)* for
Task 1 to re-run), *Global Constraints*, and then **Task 1 in full**. The decisions (21 of them) are worth reading
before Task 1's Step 18, where you judge whether a measurement moves a task.

**ASK RICH, ONCE, AT THE START: the network, for `[M3]` (Gitea's and Forgejo's API documents) and `[M10]` (GitHub's
REST description, ~13 MB, from which the fake's schemas are extracted and committed).** If he says no, Task 1 records
both as *read 2026-09-24, see the plan*, and Task 4 does the extraction in sitting 3's network window. **Nothing
else in sitting 1 needs him**: the real GitHub App is optional and not wanted until Task 6.

**DECIDED BY RICH — DO NOT RE-ASK** (§8, *Decided*):
- the fake in a container, plus an opt-in real conformance check;
- eight sittings — **if Task 1 breaks the split, WRITE TO RICH with the measurement and a proposed re-cut, and wait. Do not re-cut it silently**;
- the order after this plan: the authoring API, then the vulnerability database;
- F10 is fixed; F9 and F11–F15 are inherited (Tasks 13 and 14).

**THINGS MOST LIKELY TO COST YOU:**
- **`[M1]` BEFORE `[M2]`.** `pnpm test` truncates the tables, and `[M1]` records `launch-app`'s state first. P6a's own `[M1]` once destroyed what its Step 2 existed to measure.
- **The tool shell is zsh, which does not word-split an unquoted `$VAR`.** A measurement script written as `G="git -c …"; $G commit` fails with *command not found*, which is how this plan's writer lost one probe. **Run multi-line probes with `bash <file>`.**
- **A probe can measure the wrong thing and read as a result.** The writer's first mirror-rewrite probe rewrote a commit the mirror had never fetched, so git correctly accepted it as a fast-forward. **Make the mirror hold the commit before the rewrite** (Task 1, Step 13).
- **`[M6]`'s pattern probe must read SEVEN patterns** from `build/gates.ts`, several of which span lines. It throws otherwise. A probe that reads no rules reports that no rule matches.
- **`[M5]`'s form C must AUTHENTICATE a real push** before *no leak* means anything, and `[M15]` needs `ministral-3` warm (the curl in Task 1, Step 14).
- **`[M18]` touches `pnpm-lock.yaml`**, and restores it from a copy in `$SCRATCH`. **`git status` must be empty of that step's paths afterwards.**
- **Do not run the Docker tier or a demo in sitting 1**: nothing under the owing paths changes.

**THE MACHINE, AS THE D5 PLAN-WRITING SESSION LEFT IT (2026-09-24) — queried at its close; query every one again:**
- **The gates were NOT run by that session**, which changed no code. The last measured numbers are the F10 sitting's: `make doctor` 19/0 with 0 warnings (the vulnerability database is stale after 2026-10-01), `make verify` 55/0, `pnpm test` 1742 in 123, `pnpm test:docker` 200 in 31, and **24 migrations**.
- The database holds **1 project (`launch-app`, launched), 4 releases, 3 approvals, 4 specs and 6 builds**. `launch-app`'s repository is a **driver-1** bare repository.
- **Nothing listens on 7100, 7102, 7104 or 7110**, and both loopback aliases are on `lo0`. `dead-app-resources.sh` reads *none dead*, and `litellm-orphans.sh` *nothing to delete*.
- **Images: `docker images -q` 192, `sort -u` 184, `127.0.0.1:7107/local/*` 145.** Ollama 0.34.4, with no model resident.
- **Its snapshot diff was uptimes and the plan file only.**
