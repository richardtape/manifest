...
**P5b IS EXECUTED** (2026-09-18, nine sittings, 116 findings; sitting 9 was Task 13, the
acceptance, with 11 findings and commits `fe174ca`, `539467f` plus its close-out). **Nothing is
...
1. **[The P5 brief's §10, *What P5c INHERITS*](plans/2026-09-16-p5-brief.md)** — written for
   you, at the end of P5b sitting 8 and extended by sitting 9. **Its subsection *The one thing
   to get right first* is the one that matters — NOT its opening paragraph, which is about the
   brief's own stale sections. That subsection says §17's Phase 1c row is NOT P5c's scope, and
   that reading it as such over-scopes the plan by about four fifths**, because P5a and P5b have
   already shipped most of what it lists.
...
4. **§8's open questions — ONE of them is yours to force, not two.** An app route's
   `stream_close_delay` is Rich's decision, open since 2026-09-16, and a console with a live log
   view is exactly what makes it concrete: the console's own site sets an hour, app routes set
   nothing, so any deploy of any app closes every app's proxied WebSocket. **The rest of §8 is
   not P5c's.** *§10's* Decisions P5c must make *lists TWO — that one and whether the
   pending-action sweeper needs a timer — but the sweeper is not in §8 at all, because it is the
   plan's to decide rather than Rich's. Checked by opening §8 and counting; this hand-off said
   "two of them" until it was.*
