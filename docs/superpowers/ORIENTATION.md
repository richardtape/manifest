...
P4a — and whose fifth, P4b, is PART-EXECUTED: sittings 1 to 8 of its ten are done. §16's proof
app signs a real person in with CWL and keeps their data theirs, and a failed deploy produces
§14's Incident. CONTINUE P4b AT TASK 14 (sitting 9: Tasks 14 and 15 — the event stream and the
call sites that make it carry anything; no network needed).** This is the single
...
| | |
|---|---|
| `pnpm test` (from the **repo root**) | **703 passed, 64 files**, ~40 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **133 tests, 25 files, all passing**, ~490 s — the first full tier since P4b finding 133, the Mongo readiness race, was fixed. Needs `make up`, and **fails rather than skips** when asked to run |
...
The immediate work is **P4b, sitting 9 — Tasks 14 and 15, the event stream and the call sites that make it carry anything; no network needed** — see §7d-2's *Sitting 9 — what you are walking into*, and read the note from sitting 8 at the top of Task 14 first. P4a's fifteen tasks are done: the IdP
...
│                                                  plan). SITTINGS 1-8 (Tasks 1-13)
│                                                  ARE DONE, 2026-09-15.
│                                                  ← CONTINUE HERE, AT TASK 14. Records ten more measured
...
## 7. What to do next — continue P4b, at Task 14
...
pin on 2026-09-09, sitting 2 ran Task 3, sitting 3 ran Tasks 4–5, sitting 4 ran Tasks 6–7, sitting 5 ran Tasks 8–9, sitting 6 ran Task 10 and sitting 7 ran Tasks 11–12, all on 2026-09-14, and sitting 8 ran Task 13 on 2026-09-15**, so continue at Task 14. **P4b runs in ten agreed sittings**,
...
— **16 tasks, written 2026-09-07. Tasks 1–2 executed 2026-09-09, Tasks 3–12 on 2026-09-14 and Task 13 on 2026-09-15; continue at Task 14.**
...
| **9 ← next** | **14–15** | the stream, and the call sites that make it carry anything — **no network; a note from sitting 8 at the top of Task 14, correcting Task 14's own code; hand-off below** |
...
is at the top of Task 7. **Sitting 3 — Tasks 4 and 5 — is done too (2026-09-14), with 11 findings.** The two to carry forward: **Task 7's `ensureAiUser` recognises an existing LiteLLM user by text Task 5's client exists to strip, so as written every redeploy of an AI app throws** — catch on status 409 — and **Task 9's `createLiteLlmClient(config.litellm)` cannot typecheck** against the shapes Task 5 fixed. Both corrections are at the top of those tasks. **Sitting 4 — Tasks 6 and 7 — is done too (2026-09-14), with 12 findings**, and it made the boot decision its hand-off named: AI is on unless `MANIFEST_AI_ENABLED=0`. **Sitting 5 — Tasks 8 and 9 — is done too (2026-09-14), with 11 findings**: the gateway joins only an AI app's network, `deployRelease` mints before the instance starts and commits after health, and an AI release deployed with AI switched off is refused with `RELEASE_AI_DISABLED`. **Sitting 6 — Task 10 — is done too (2026-09-14), with 14 findings**: `node-ts-mongo@1` provides AI and its component is proved on the wire, and seed's npm warm — inert on any machine whose npm cache already held a package — is fixed. **Sitting 7 — Tasks 11 and 12 — is done too (2026-09-14), with 16 findings**: build logs are captured line by line, redacted and stored, and redaction has its heuristic half — and its Docker tier ended 130 of 131 on a Mongo readiness race that is not its code (finding 133). **Sitting 8 — Task 13 — is done too (2026-09-15), with 20 findings**: a Mongo service is healthy only once it enforces authentication, and a deploy that fails to become ready — by either route, the Docker driver's readiness refusal included — is a recorded `failed` instance with §14's Incident. **Sitting 9 — Tasks 14 and 15 — is next, and its hand-off is immediately below.**

#### Sitting 9 — Tasks 14 and 15 — what you are walking into

**The state it was handed over in, 2026-09-15.** `main`, tree clean; last code commit `3d4b99e` (Task 13; the finding-133 fix is `7e624e3`). `pnpm test` **703** (64 files), `pnpm test:docker` **133 tests in 25 files, all passing**, `make doctor` **18/0**, `make verify` **47/0**. Migration `0006_abandoned_morbius` is applied — `audit.incidents`, with `manifest_app=ar` — so a database that has not run it fails global setup on `audit.incidents`; `db:migrate` fixes that. LiteLLM 1.98.0 (`sha256:20b5044b`) running. If your first measurements differ, find out why before you start.

**Tasks 14 and 15 stay together** — the rule that survives any re-cut, because a stream with no publisher sits green for a whole session. **No network needed.** Task 15's *Test* block includes `roundtrip.docker.test.ts`, so **the sitting ends with `pnpm test:docker`** as well as the four gates.

**What sitting 8 built** (finding 133 first, then Task 13): the Mongo catalogue's authentication-enforced health check with `StartInterval`, and `createServiceContainer`; `InstanceNotReadyError` — with `handle` and `check` — and `InstanceStatus.exitCode` in `runtime/driver.ts`, thrown and filled by the Docker driver; `deployRelease` recording a readiness refusal as a `failed` row and capturing an Incident on either failure route; `observability/incidents.ts` — `captureIncident`, `listIncidents`, `incidentPrompt`, `INCIDENT_LOG_LINES` — over migration 0006's `audit.incidents`; `describeDiff` and `DESCRIBED_PATHS` in `spec/diff.ts`; `AppSecretResolver.secretValues`; `GET /environments/:environmentId/incidents`; `destroyAppNetwork` on `runtime/testing.ts`; and `releases/incident.docker.test.ts`.

**Four things from sitting 8 that touch Tasks 14 and 15** — the note at the top of Task 14 has all eight:

1. **Task 14's `authorizeStream` passes `'read'`, which is not a capability** — `'project:read'` is (finding 153). `tsc` catches it; the tests will not.
2. **The Incident has exactly one call site, and it now covers both failure routes**: the `if (!healthy)` branch of `deployRelease`. Task 15's publish for a failed deploy goes beside `captureIncident` and reuses the redactor built there.
3. **A failed deploy is a `200` whose state is `failed`** (finding 146). Nothing on the stream may equate "the call returned" with "the app is up".
4. **Exact-list tests will move again** when Task 15 adds call sites: `releases.test.ts`'s AI-key tests pin every `status()` read in order, and the Incident's read is now the last of them (finding 148).

**Still not proved end to end, and not sitting 9's to prove:** no app that declares models has been deployed and asked a question — Task 16's.

**Read these before writing a line:** the note at the top of Task 14; sitting 5's correction at the top of Task 15 — `rotateAppKey` no longer exists; and the plan's *What executing this plan found*, sittings 7 and 8.

```bash
make up && make doctor && make verify                   # expect 18/0 and 47/0
pnpm test                                               # expect 703 passed, 64 files
MANIFEST_TEST_DOCKER=1 pnpm exec vitest run --project docker src/releases/incident
```

**Tasks 14 and 15 touch `api/`, `observability/`, `releases/` and `ai/`, so the sitting ends with `pnpm test:docker` as well as the four gates** — then the record, the sittings table, and §6's sweep.
