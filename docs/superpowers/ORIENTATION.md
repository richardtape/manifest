...
(2026-09-16): 17 tasks in twelve agreed sittings, and the next job is EXECUTING ITS SITTING 1 — §7e says what that is.** This is the single
...
│                                                  SITTING 1 IS NEXT. Its six spec
...
**P5a IS WRITTEN (2026-09-16), AND YOUR TASK IS ITS NEXT SITTING — SITTING 1, TASK 1, THE MEASUREMENTS.**
...
**Read, in this order, before sitting 1:**

1. **The plan's** *How this plan is to be executed*, *Read this first*, *Decisions Rich made* and *Decisions this plan makes* — then **Task 1 in full**.
2. **[`plans/2026-09-16-p5-brief.md`](plans/2026-09-16-p5-brief.md)** §3 (the three measurements Task 1 repeats and extends) and §8 (the traps).
...
session**, with a check-in at each boundary. **Sitting 1 changes no platform code**: it measures — a `zod/v4` route, the
edge refusing an app network by source with its positive control, a CWL sign-in and a stream through a new origin,
`RelayState` through the real IdP, the reserved-label data — and writes a *Task 1 correction* at the top of every task
a result changes, re-cutting the sittings if a boundary moves. **Sitting 5 is the only one that needs the network.**
...
**The state you are handed, 2026-09-16.** `main`, clean. P4c's last code commit is `c51224e`; every
commit after it is documentation, the brief, the spec (P5a's six actions are `491f8be`) or the P5a plan itself —
so no platform code has changed since P4c. The four gate numbers are §2's box, all green. The proof app is deployed from `make demo-redeploy`'s last run — one app container, its
database, one live LiteLLM key — on a machine `make reset` during P4c sitting 8, so no other project's
data exists. The control plane is **not** running (port 7100 is free); README's *Running the control
plane* starts it. A redeploy is P4c's: `POST /environments/:environmentId/deploy` returns once the new
