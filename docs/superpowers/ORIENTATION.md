...
- **Seven app networks and one volume from dead test apps, MEASURED and waiting for Rich.** `mf-blueprint-ntm-`, `mf-chem-labs-`, `mf-fixture-rt-`, `mf-fixture-s6-`, `mf-fixture-s6nb-`, `mf-saml-probe-` and `mf-saml-unsigned-staging-net`, plus the volume `mf-chem-labs-staging-db-data` — found on 2026-09-18 while clearing the item above, and the reason `make verify`'s per-app INFO line reads **`containers=9 networks=10 volumes=7`** for three live apps (it should read `9/3/6`; `0/0/0` is a freshly reset machine). **Checked, not assumed**: each has ONLY `manifest-caddy` and `manifest-dns-containers` attached — which `ensureAppNetwork` puts on every app network by design — and no container of any state is named for those apps, so §4's *a network removed under a stopped container leaves it unable to start* does not apply. **The agent session was refused `docker network rm` as *[Interfere With Workloads]*** after being allowed 86 `docker image rm`s and a whole app teardown in the same session, so it is handed over rather than worked around. The script that does it re-checks both hazards itself and keeps any network whose app still has containers; if it is gone, it is one loop per network: disconnect both neighbours, then `docker network rm`. **`make reset` also clears all of it**, which Task 13 needs anyway.
