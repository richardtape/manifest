...
| The database | **NO migration since 0018** (`packages/control-plane/drizzle/0018_curvy_sister_grimm.sql` is the newest — *checked*; note the path, `drizzle/` is NOT at the repository root). **`pnpm test` ran four times and a whole Docker tier ran after the browser work**, and every one truncates, so **treat the §6 tables as EMPTY** |
