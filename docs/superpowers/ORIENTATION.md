...
(2026-09-16): 17 tasks in twelve agreed sittings. ITS SITTING 1 — Task 1, the measurements — IS DONE (2026-09-16, 9 findings), and the next job is EXECUTING SITTING 2 — §7e says what that is.** This is the single
...
**The four numbers you will check first, measured 2026-09-16 on this machine, at the end of P4c:**
...
│                                                  SITTING 1 DONE, 2 NEXT. Its six spec
...
**P5a IS WRITTEN (2026-09-16) AND ITS SITTING 1 IS DONE, THE SAME DAY — YOUR TASK IS SITTING 2: TASK 2 (every resource route under `/v1`), THEN TASK 3 (the API through the edge at `https://console.manifest.internal`, refused to app networks).**
...
**Read, in this order, before sitting 2:**

1. **The plan's** *How this plan is to be executed*, *Read this first*, *Decisions Rich made*, *Decisions this plan makes* and **sitting 1's entry in *What executing this plan found*** — then **Tasks 2 and 3 in full**, including the *Task 1 correction* at the top of Task 3.
2. **[`plans/2026-09-16-p5-brief.md`](plans/2026-09-16-p5-brief.md)** §2.4 (`make verify`'s console probes, which Task 3 moves) and §8 (the traps).
3. **This file's §4** (the machine) and **§6** (how to work, and the close-out sweep every sitting owes).

**How to execute it.** `superpowers:executing-plans` or `superpowers:subagent-driven-development`, **one sitting per
session**, with a check-in at each boundary. **Sitting 2 changes routes, every demo script, `infra/` and the Docker tier**, so it
ends with `pnpm test:docker` (~13 min) as well as the four gates. A Caddyfile edit reaches the edge through `make up`,
...
- **Every demo script, `scripts/lib/proof-app.sh`, `infra/lib/idp-login.sh`, README's *Running the
  control plane*, RUNBOOK and WALKTHROUGH speak `http://127.0.0.1:7100` with `curl`.** Moving the API
  to `https://console.manifest.internal/v1` moves all of them, and the control plane's own SP
  registration (`MANIFEST_CONTROL_PLANE_ORIGIN`, `sso/platform.ts`).
- **`make verify` probes the edge with `console.manifest.internal`** and expects the placeholder page
  (brief §2.4) — the task that serves the console there must move those probes.
...
**The state you are handed, 2026-09-16, after sitting 1.** `main`, clean. P4c's last code commit is `c51224e`; every
commit after it is documentation, the brief, the spec (P5a's six actions are `491f8be`), the P5a plan or sitting 1's
measurements in `spikes/p5a-baseline/` — so no platform code has changed since P4c. The four gate numbers are §2's box,
all green and unchanged by sitting 1. **The control plane's tables are EMPTY**: sitting 1's Vitest runs truncated them.
So the proof app's container, database, egress proxy, network, edge route and one live LiteLLM key live on with **no
project row behind them** (P4b findings 178 and 183) — `https://proof-app.staging.manifest.internal/` still serves the
app, but nothing can redeploy it through the API until a demo recreates the project, and because
`.manifest/repos/proof-app.git` survives, that first `POST /projects` answers `SOURCE_GIT_FAILED` and the demos
carry on (RUNBOOK's *Known gaps*). Tasks 2 and 3 re-run every demo. The platform SP row's ACS is
`http://127.0.0.1:7100/auth/saml/callback` — sitting 1 moved it to a probe origin and back, and checked both. The
control plane is **not** running (port 7100 is free); README's *Running the control plane* starts it. A redeploy is P4c's: `POST /environments/:environmentId/deploy` returns once the new
instance serves and the old one drains in the background.
...
- **`egress.allow` may not name a platform surface — ACCEPTED IN SUBSTANCE 2026-09-16, Rich's call; spec text his to apply, implementation is a later hardening slice, NOT P5a.** P5a Task 1's `[M2g]` measured that `renderAllowlist` (`runtime/docker/egress.ts`) accepts any syntactic hostname, and that the app's forced proxy — dual-homed onto `manifest-platform`, the app network's only route out — tunnels raw TCP to any port (the control opened `manifest-idp:80`) to any name it resolves there. So an app that declares a platform service (`manifest-postgres`) or the console/IdP zone in `egress.allow` gets a tunnel across the east-west boundary §12 exists to hold; the edge's own source allow-list (`10.89.0.1/32`, Task 3) already refuses the console leg, so this is defense in depth. The fix refuses the platform zone and `manifest-*` at validation as `EGRESS_ALLOW_INVALID`, derived from §23's reserved labels and the platform zones — the reserve-the-platform's-own-names pattern P5a builds for slugs. Scope: platform surfaces only, not app-to-app. **The write-up with proposed §12 wording is in P5a's *Spec actions* (raised while executing) and the roadmap's *Spec action raised by executing P5a*. Do not re-decide the substance; the open part is the spec edit and which slice builds it.**
