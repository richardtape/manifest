...
`qwen3.5:4b` the same afternoon; no number moved. `pnpm test` **1742 passed, 0 skipped, in 123 files**, twice after
the switch, identical. **`make doctor` 19 checks, 0 failed, 0 warnings** — the vulnerability database goes stale again
after 2026-10-01 (`make refresh-vulndb`). **`make verify` 55.** `pnpm test:docker` **200 in 31**, 1090.5 s — owed by
the switch (`infra/`) and run, green first time, `ai-path`'s thinking-model test included. `scripts/ci-acceptance.sh` reads `1742 / 123 / 19 / 55`. **This line states only the
...
**The four numbers you will check first, re-measured 2026-09-24 on this machine at the open AND the close of the D5 plan's sitting 1 (the measurements; its record is in the plan). `pnpm test` **1742** in **123** files, unmoved (twice at the open, twice at the close); `make doctor` **19** with **0 warnings**; `make verify` **55**; `pnpm test:docker` **200** in 31 — run after Rich's switch of the chat model to `qwen3.5:4b` the same afternoon:**
...
- **THE CHAT MODEL IS A THINKING MODEL, AND ONE LINE OF LITELLM CONFIG IS ALL THAT KEEPS IT ANSWERING** (Rich's
  switch to `qwen3.5:4b`, 2026-09-24). `reasoning_effort: none` on each chat mapping in `infra/litellm/config.yaml`
  is what LiteLLM 1.98.0 turns into Ollama's `think: false`; any value but `low`/`medium`/`high` does it. Remove
  it and `default-chat` streams 260 frames with **0 content** — every token is `reasoning_content`, which the app
  toolkit discards, so an app gets an empty answer and no error (S3 Evidence 9, re-measured). `make verify`'s
  *a streamed completion returns non-empty content* and `ai-path.docker.test.ts` both go red. **A REQUEST's own
  `reasoning_effort` overrides the mapping** — measured: `"low"` on one request brought back 231 reasoning frames and
  0 content — so an app that sends it gets an empty answer; the mapping protects only callers that say nothing. The config is a single-file bind mount:
  edit it IN PLACE (an editor that writes a new inode leaves the container on the old file), then `docker restart
  manifest-litellm`, and read it back with `docker exec manifest-litellm grep … /app/config.yaml`.
...
- **Rich switched the chat model to `qwen3.5:4b` after sitting 1** — a THINKING model, run with thinking off by
  `reasoning_effort: none` in `infra/litellm/config.yaml` (§4). If anything AI-shaped streams nothing, check that
  line first.
...
- After the model switch's Docker tier: **seven dead networks and one volume, and one LiteLLM orphan, came back and
  were cleared with `--apply`**; both scripts then read clean. `make verify`'s line: `mf- containers=6 networks=2
  volumes=4`.
- **Images: `docker images -q` 198, `sort -u` 190, `127.0.0.1:7107/local/*` 151** — up six, the app images the
  Docker tier builds, which no script sweeps. Ollama 0.34.4 with **no model resident** (both unloaded at close).
- The gates at the close: `pnpm test` 1742 in 123 (twice), lint, typecheck and format clean, `make doctor` 19/0/0,
  `make verify` 55/0, `pnpm test:docker` 200 in 31. The chat model is `qwen3.5:4b`; `ministral-3` is still on disk,
  unused.
...
- **The chat model is `qwen3.5:4b`, with thinking OFF — DECIDED 2026-09-24** (Rich). Both chat mappings in `infra/litellm/config.yaml`, with `reasoning_effort: none` (LiteLLM → Ollama `think: false`); without that line it streams no content at all (§4). `ministral-3` is still installed and unused — removing it (6.0 GB) is Rich's. A per-model "needs a more explicit prompt" attribute was discussed and NOT built: the D5 plan's Decision 22 says where it would go.
