...
| The apps | `token-app`, `journey-app` and the proof app are **up and healthy** — **the fixture app was TORN DOWN after the sitting** (§2's *Outstanding*), so `make demo` now starts from nothing — and `token-app` and `journey-app` **are reachable by name through the edge** — both demos re-registered their routes after the Docker tier truncated `routes`. **`proof-app` is NOT** — its route is gone and its name falls through to the wildcard, confirmed again after the cleanup (`manifest OK host=proof-app.staging…`). `fixture-app` does not exist at all any more. **Check the BODY, never the status** — a name nothing has ever deployed also answers `200`. Harmless: the next `make demo*` recreates each |
