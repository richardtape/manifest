...
(2026-09-16): 17 tasks in twelve agreed sittings. ITS SITTINGS 1, 2, 3 AND 4 ARE DONE (2026-09-16, 9, 16, 15 and 7 findings) — the measurements; the API under `/v1` and through the edge at `https://console.manifest.internal`, refused to every source but the host; then CSRF by `Origin`, a sign-in bound to its browser, and one registry of every error code; then `defineRoute` and the OpenAPI document generated from it, `packages/contract/openapi.json` — and the next job is EXECUTING SITTING 5, THE ONE THAT NEEDS THE NETWORK ON — §7e says what that is.** This is the single
...
**The four numbers you will check first, measured 2026-09-16 on this machine, at the end of P5a sitting 4:**

| | |
|---|---|
| `pnpm test` (from the **repo root**) | **872 passed, 79 files**, ~50 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **168 passed, 0 SKIPPED**, 27 files, ~755 s — measured at the end of P5a sitting 3 and not re-run in sitting 4, which the plan exempts (it changed `api/` and no Docker test); sitting 3 added no Docker test and changed `identity/saml.docker.test.ts`'s two; sitting 2's S6 probe 15 is the 168th (the edge refuses an app the control plane, with the host as its control). The eleven skips that used to be here were the driver contract's continuity block, gone since P4c Task 5 gave the Docker driver its fixtures. Needs `make up`, and **fails rather than skips** when asked to run |
...
│                                                  SITTINGS 1–4 DONE, 5 NEXT. Its six spec
...
**P5a IS WRITTEN (2026-09-16) AND ITS SITTINGS 1 TO 4 ARE DONE, THE SAME DAY — YOUR TASK IS SITTING 5: TASK 7 (the generated client and its first caller — `@manifest/contract` generated from `packages/contract/openapi.json` by `openapi-typescript` 7.13.0 and called through `openapi-fetch` 0.17.0, `packages/journey` with its import boundary, and `make demo-journey` at step 1). IT IS THE ONE SITTING THAT NEEDS THE NETWORK ON.**
...
**Read, in this order, before sitting 5:**

1. **The plan's** *How this plan is to be executed*, *Read this first*, *Decisions Rich made*, *Decisions this plan makes* (1–10 above all) and **sittings 1 to 4's entries in *What executing this plan found*** — then **Task 7 in full**, including the *Sitting 2 correction* at its top (the journey is a Node process and needs `NODE_EXTRA_CA_CERTS`), and *Read this first* 14 (whether `openapi-typescript`'s closure runs an install script is Task 7's first measurement).
2. **[`plans/2026-09-16-p5-brief.md`](plans/2026-09-16-p5-brief.md)** §8 (the traps).
3. **This file's §4** — its last five entries are sittings 3 and 4's — and **§6** (how to work, and the close-out sweep every sitting owes).
...
machine, `make up`, doctor, verify and the four gates — and compare every number with §2's box before changing anything. **Sitting 5 adds two packages — `packages/contract/`
(beside the `openapi.json` sitting 4 put there) and `packages/journey/` — plus `scripts/demo-journey.sh`, a `Makefile` target and
workspace config, and installs `openapi-fetch` and `openapi-typescript` pinned exactly** — the plan's rule exempts it from
`pnpm test:docker`, so it ends with the four gates, and **Decision 10 renames the typecheck gate to `pnpm typecheck` in
CLAUDE.md, ORIENTATION, README and RUNBOOK in that sitting's sweep**. A Caddyfile
...
**Sitting 5 is the only one that needs the network.**
...
**The state you are handed, 2026-09-16, after sitting 4.** `main`, clean. The code commit is Task 6's `204bf49`; everything
later is documentation. The four gate numbers are §2's box — all green, `pnpm test` 872, `make verify` 50/0 and
`pnpm test:docker` 168 (not re-run in sitting 4). **The proof app serves at `https://proof-app.staging.manifest.internal/`,
with a project row, a `Route` row and one LiteLLM key behind it** (`make demo-identity` ran last, after `pnpm test`), so
a demo reuses it by slug — but **the next `pnpm test` empties those tables again**, and after that the app keeps serving
with nothing behind it until a demo recreates the project. **The fixture app is not deployed** — sitting 2 removed what
...
proof-app project, whose rows sitting 4's `pnpm test` truncated and whose container the demo replaced. Sitting 4's attempt to
