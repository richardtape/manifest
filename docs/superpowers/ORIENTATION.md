...
- **`pnpm test:docker` is owed.** Task 6 changes `projects/authz.ts`, which is in the tier's
  blast radius. It has not moved from 177 in five runs across two sittings.
...
| The apps | The proof, journey and fixture app containers are **up and healthy**, and **`journey-app` IS reachable by name through the edge** — the demo re-registered its route after the closing `pnpm test:docker` truncated `routes`. `proof-app` and `fixture-app` are **not**: their routes are gone and their names fall through to the wildcard. **Check the BODY, never the status** — the wildcard answers `200` for any name at all. Harmless: the next `make demo*` recreates each |
