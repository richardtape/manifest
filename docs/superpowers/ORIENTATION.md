...
| `make ci-acceptance` | **NEW in P5c sitting 8, and RUN**: the headless half of 1c's acceptance — `make doctor`, `make verify`, the three fast gates, `pnpm test` with its COUNT asserted, the three package builds, then `make demo-journey`, `make demo-token` and — since P6a sitting 11 — `make demo-production`. Every step REPORTS rather than exits, so a red run is a measurement; a count that has moved reads `MOVED`, not `FAIL`. ~15 min. **Its four `EXPECT_` lines carry this box's numbers and move with it — and they did NOT, from P6a sitting 2 until sitting 3 found them reading 18 and 51 against a machine at 19 and 54.** They are now **1605 / 119 / 19 / 55**, moved by sitting 11 in the routing fix's own commit, and §6's sweep table has a row for this file so it cannot happen again — **and sitting 9's own post-sweep check found this line still reading 1565 / 117 after the box above it had moved**, which is the same defect one screen further down. **`EXPECT_TESTS` is the PASSED count**, which mattered from sitting 5 to sitting 8, when the suite had a skipped test; it has none now |
