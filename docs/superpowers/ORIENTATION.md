...
**The four numbers you will check first, re-measured 2026-09-20 on this machine at the close of P6a sitting 8. ONE OF THE FOUR MOVED — `pnpm test` 1544 → 1565 passed (plus the same one skipped), in 117 files, up 21 and three new files. `pnpm test:docker` was OWED, RUN and UNMOVED at 185 — the SIXTH consecutive run to read it (sittings 3 to 8) — a coverage statement, checked by grepping all thirty files. `make doctor` and `make verify` held at 19 and 54, because the sitting adds no platform check:**

| | |
|---|---|
| `pnpm test` (from the **repo root**) | **1565 passed + 1 SKIPPED, 117 files**, ~122 s — **up 21 and THREE new files in P6a sitting 8** (Tasks 12–13), counted per file off the run's own output: `launch/review.test.ts` **3 (NEW)** — `NullReviewer` names itself and reads nothing; `spec/registered-attributes.test.ts` **7 (NEW)** — §7's clause as a pure rule, empty registration included; `releases/build-attributes.test.ts` **4 (NEW)** — a BUILD fails on drift before the driver runs, and three positive controls; `launch/readiness.test.ts` **15 → 20** — `code-review` non-blocking in both directions, and `iam-registration` reading the candidate's attributes; `releases/approval.test.ts` **16 → 18** — the snapshot's review comes from the INJECTED reviewer. `spec/policy.test.ts` held at 28 with one test replaced. **The one skip is unchanged and STILL cannot run** — `api/delivery.test.ts`'s production-gate positive control, which **Task 14 un-skips** — so `scripts/ci-acceptance.sh`'s `EXPECT_TESTS` is the PASSED count, **1565**. **Each earlier sitting's movement is in its plan record**, dated; this row states only the latest. It is the `unit` project and `packages` (the client, the journey, the console and the mock, none of which need anything running). No Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/`, `releases/`, `tokens/` and `launch/` suites, plus `spec/injection-drift`, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` (§3) |
| `pnpm test:docker` | **185 passed, 0 SKIPPED**, 30 files, **821 s, re-measured by P6a sitting 8** — which OWED the tier (`releases/`, `launch/`, `spec/`), ran it, and **found it UNMOVED — the SIXTH CONSECUTIVE RUN to read 185** (sitting 3 moved it there from 180; sittings 4 to 8 each found it unchanged), **which is a COVERAGE STATEMENT rather than a formality — and this time it was CHECKED**: grepping all thirty `*.docker.test.ts` for `iamRegistration`, `registeredAttributes`, `computeLaunchReadiness`, `reviewer` and `approve` returns NOTHING, and the five files that build a CWL app all build with **no registration row**, which is the branch where the new check does nothing. **P6a Task 15's own Docker test is the first thing that should move it.** Each earlier sitting's figure is in its plan record. Not every sitting owes the tier — P5c's 4 to 7 and P6a's 1 neither ran nor owed it, so **do not spend 14 minutes on it out of habit**; the plan's own sittings rule says who owes it. `boot.docker.test.ts`'s *expires a question nobody answered* is still the ONLY test in the repository that fails if `src/index.ts` stops calling the expiry sweeper. The tier still drives SESSIONS: `grep` over every `*.docker.test.ts` finds no `Authorization: Bearer` that is a Manifest delegated token. **Running it costs the machine**: it restarts the edge (dropping every runtime route, so every app hostname then answers the WILDCARD), truncates the tables, re-registers the platform's SP row at a loopback ACS — **restart the control plane afterwards** — and regenerates exactly seven dead app networks and one volume. Needs `make up`, and **fails rather than skips** when asked to run |
| `make doctor` | **19 checks, 0 failed, 0 warnings** — **unmoved by P6a sitting 8** (re-run AFTER its Docker tier and after both cleanup scripts), and by sittings 3 to 7; up one in sitting 2, which REPLACED the single-alias check with a two-alias loop (net 0) and added *the production zone answers the public address, and the console does not*. That check asserts BOTH directions: one asserting only the production answer stays green while the console has moved to the public address. **It cannot tell you production is REACHABLE** — Task 2's control (a) measured two checks naming `127.0.0.3` staying green with the alias removed, because they assert what dnsmasq answers |
| `make verify` | **54 checks, 0 failed, 0 warnings** — **unmoved by P6a sitting 8**, which adds refusals and no platform check, and by sittings 3 to 6; **up three in sitting 2** for §12's second listener (*the edge serves two listeners*, *the public listener answers a production name on 127.0.0.3*, *neither listener answers for the other's zone*), and sitting 3 added an ASSERTION INSIDE an existing check rather than a check — *the public listener answers a production name* also holds `MANIFEST_EDGE_PUBLIC_PORT`'s default equal to the container-side half of `infra/compose.yaml`'s `127.0.0.3:443:8443`. Re-run after each sitting's Docker tier AND after the cleanup scripts. **It reads `containers=12 networks=4 volumes=8` today** (P6a sitting 8's close, after that sitting's Docker-tier run took it to `networks=11 volumes=9` and `dead-app-resources.sh --apply` took it back — the **TWELFTH** measurement of that cycle, the same seven networks and the same one volume again): **FOUR** apps are deployed — `journey-app`, `token-app`, `p5c-acceptance` and `proof-app` — three containers, one network and two volumes each, and **their project rows are truncated by every `pnpm test`, so the containers outlive their projects**, which is ordinary here and not a fault. **This is a state claim with a shelf life: query it, do not quote it.** The rule the number follows: **one app is `containers=3 networks=1 volumes=2`**, ANY `pnpm test:docker` run adds **seven dead networks and one dead volume**, and `bash scripts/dead-app-resources.sh --apply` takes them away again. More networks than apps means an app whose containers are gone and whose network is not; `0/0/0` is a freshly reset machine. **P6a's sittings 3 to 8 were each ALLOWED `--apply` for BOTH cleanup scripts and ran them themselves**, re-measuring with the scripts afterwards — the classifier refuses this in some sessions and not others, so **try the command rather than trusting any note about it** |
...
### 7e. Execute P6a's SITTING 9 — Tasks 14 and 15, the D21 rehearsal and the first production deploy ← **START HERE**

**P6a IS EXECUTING. Sitting 8 — Tasks 12 and 13, R4's `Reviewer` seam and §7's last production
clause — ran on 2026-09-20, and NO TASK BOUNDARY MOVED, so the eleven-sitting split still stands.**
...
**Your job is SITTING 9: TASKS 14 AND 15**, and they are COUPLED — the plan says so at Task 14's
head: the rehearsal deploys the candidate into PRODUCTION, so it cannot be measured until a
production deploy works. **Build Task 15 first if Task 14 blocks, and say so in the record.**
Task 14 is R2's rehearsal — a `rehearsals` table (migration **0020**; `rehearsal.completed` is
ALREADY an event type and already in 0019's CHECK, so check what drizzle generates rather than
appending), `runRehearsal`, the item met by a MEASUREMENT whose `why` says it proves the
registration's SHAPE and never UBC's acceptance, and `POST /v1/projects/{id}/rehearsal` — and it
**un-skips `api/delivery.test.ts`'s positive control**, the suite's one skip since sitting 5. Task
15 is **the first production deploy this platform has ever done**: the digest verified against the
approval before anything starts (`RELEASE_DIGEST_NOT_APPROVED`), five code paths running for the
first time, and `production.docker.test.ts`. Use `superpowers:executing-plans`.

**READ TWO CORRECTION BLOCKS BEFORE THE STEPS.** **Task 14's sitting 8 block** (four points —
summarised below) and **Task 15's sitting 6 block**: `release:promote` is in `STEP_UP_GUARDED`
and no route calls `assertStepUp` for it, and **Rich decided on 2026-09-20 that Task 15 adds the
call site** on the production branch of the deploy route only.

**WHAT SITTING 8 HANDS YOU.**
...
1. **`iam-registration` IS `met` ONLY WHEN THE REGISTRATION IS `active` AND THE CANDIDATE ASKS
   ...
   so **every registration you record — in tests, in the un-skipped delivery test, and later in
   Task 19's demo — must list every attribute the candidate asks for**, or the gate stays shut on
   an item nobody is looking at and it will read like the rehearsal failing.
2. **A BUILD FAILS on the same rule when a registration already exists**
   (`SPEC_ATTRIBUTE_NOT_REGISTERED`, before the driver runs, the reason as the log's last line).
   The rule is stated ONCE, `unregisteredAttributes` in `spec/registered-attributes.ts`; do not
   write a third copy.
3. **`code-review` is the checklist's LAST item and `blocking: false`.** Un-skipping the delivery
   test needs nothing for it — and **that test is the first place Decision 13 can be seen END TO
   END** (`ready: true` beside a `not_built` `code-review`). `readiness.test.ts` could only FORCE
   the other items met (F2). **Assert the `code-review` item there.**
4. **`ready` is `readyOf(items)`, exported from `launch/readiness.ts`, and is the only statement
   of it.** `rehearsalItem` must not compute its own.
5. **`ServerDeps.reviewer` is `NullReviewer`**, constructed in `src/index.ts` and `api/testing.ts`
   (not `server.ts`, which only types it). Every approval's `diff.review` is `not_performed`.
6. **The unit tier CANNOT build a CWL app through the build route** (F10): the only blueprint it
   has, `fixture-node`, declares `auth_providers: [none]`, and the route refuses a CWL manifest
   `BLUEPRINT_AUTH_UNSUPPORTED` before a build is recorded. The delivery test's app is therefore
   non-CWL, whose `rehearsal` item the plan says reads `met` with *"signs nobody in"*; a real CWL
   rehearsal belongs in the Docker tier with `node-ts-mongo`.
7. **`api/delivery.test.ts`'s skip goes when Task 14 un-skips it**, and `scripts/ci-acceptance.sh`'s
   `EXPECT_TESTS` is the PASSED count — so it moves by the new tests PLUS ONE, and the
   `| 1 skipped` disappears from `pnpm test`'s summary line. Its comment says so.

**THE THINGS MOST LIKELY TO COST YOU.**

- **TASK 15 IS A FIRST, AND THIS PROJECT'S WORST DISCOVERIES HAVE ALL ARRIVED AT A FIRST.** P3
  found that no build and then no deploy had ever succeeded, behind 74 green Docker tests. Five
  production code paths have never run. **Drive the real thing and write down each stage.**
- **GREP BEFORE YOU BUILD WHAT THE PLAN SAYS TO BUILD.** Sitting 8 found the plan's caller calling
  a helper that does not exist (F1), a test it could not run (F2), and §7's whole check already in
  the tree since P2 with no caller (F8) — the plan would have built it twice under two codes.
- **A PLAN'S PREDICTION ABOUT WHICH TEST GOES RED IS A HYPOTHESIS.** Sitting 8's own predictions
  all held, and one of the PLAN's did not (F7) — because an earlier task in the same sitting had
  changed what `tsc` sees. Write yours down before the run.
- **A HAND-OFF CAN OVER-PREDICT RED TOO** (F6): sitting 7 said `packages/journey/src/main.ts`
  would go red on `code-review`, and it cannot — it reads `items[0]`, `items[1]` and items by id.
  **It is still invisible to `pnpm test`**: grep the ids across the repository.
...
- **A ROUTE CHANGE IS THREE FILES IN ORDER**: the definition, `pnpm contract:write`,
  `pnpm contract:generate`. Task 14 adds a route; Task 15 adds an error code and a step-up call.
- **`@manifest/contract` IS STILL `1.0.0`**, by sitting 7's decision and sitting 8's.
- **`$SCRATCH` DOES NOT PERSIST BETWEEN TOOL CALLS.** On this machine the unset form fails loudly
  (`read-only file system: /x`, F14); use the literal scratchpad path.

**`pnpm test:docker` IS OWED BY THIS SITTING** — the rehearsal deploys, registers an SP and signs
in, and Task 15 CREATES `releases/production.docker.test.ts`. **It has read 185 in 30 for SIX
consecutive runs — sittings 3 to 8 — and this is the sitting that should finally move it** — predict by how much
before you run it.

**THE MACHINE, as sitting 8 left it** — — every number queried at the close, not recalled.
**`make doctor` 19/0 and `make verify` 54/0** — unmoved, re-run AFTER the Docker tier and after
both cleanup scripts. **`pnpm test` 1565 passed + 1 SKIPPED in 117 files**, run twice and
identical. **`pnpm test:docker` 185 in 30 files, 0 skipped, 821 s** — owed, run, unmoved. lint,
typecheck and format:check clean. **`lo0` carries `127.0.0.1`, `127.0.0.2` AND `127.0.0.3`.**
**TWENTY migrations are applied**; this sitting adds none. **NOTHING IS LISTENING ON 7100** — the
control plane WAS started this sitting, for `make demo-journey` and the live drive, and was
stopped again at the close; start it yourself with README's *Running the control plane* block,
and **every restart mints a new `MANIFEST_SESSION_SECRET`, so no cookie survives one.**
**THE DATABASE IS NOT EMPTY, and that is unusual here**: the live drive ran AFTER the final
`pnpm test`, so `projects=1` (`journey-app`, serving staging), `releases=1`, `users=2`
(`ins000001`, `opr000001` — who is a platform ADMIN), `approvals=0`, `iam_registrations=0` and
**one FAILED build**, which is the live drive's own refusal and is left as an honest record.
**The registration the drive recorded was DELETED at close on purpose**: it listed two of
`journey-app`'s five attributes, so leaving it would have failed the next rebuild of that app
for a reason that sitting did not cause. **Any `pnpm test` truncates all of it**, so run the
gates before you want rows. `.manifest/repos/` holds the same four bare repositories it held at
open. **`make verify`'s per-app INFO line reads `containers=12 networks=4 volumes=8`** and
**`runtime routes currently applied: 1`** — `journey-app`'s staging route, re-applied by the
demo after the Docker tier had dropped every route. **BOTH CLEANUP SCRIPTS WERE ALLOWED
`--apply` AND WERE RUN — THE SIXTH CONSECUTIVE SITTING** — clearing the tier's same seven
networks and one volume (**the TWELFTH measurement of that cycle**: `networks=11 volumes=9`
before, `4` and `8` after) and taking LiteLLM from 7 users to 4; `p4b-probe-user` came back
again, beside one staging user. **The app images are the only thing this sitting moved and left
moved, and neither script covers them. Name the metric**, because the obvious commands disagree
by design: `docker images -q | wc -l` reads **146**, `sort -u` **138**,
`127.0.0.1:7107/local/*` **96**, and `grep '^local/'` **0**, all on the same machine at the same
moment — **five more than sitting 7 left**, four from the Docker tier and one from the demo.
**`./scripts/snapshot-machine.sh` at open and at close, diffed — 52 lines, every one accounted
for**: timestamps and uptimes, free disk **50Gi → 48Gi**, those five images by name, the
`journey-app` app-files volume renamed by its redeploy, port 7100 (started and stopped), and
`HEAD` moving to this sitting's own commits. **The four protected containers all survive**
(`docker-simple-saml-saml-idp-1` present and `Exited (0) 2 weeks ago`, exactly as at open) and
**`caddy-data` is intact**; `docker-simple-saml`'s only dirty path is the untracked `cert.zip`
dated June. `POST …/members` still answers `400 MEMBER_USER_NOT_FOUND` for anybody who has not
signed in to Manifest itself, and a role reaches a person only at their next sign-in
(`scripts/admin-grant.sh`).

**ONE CONTROL IS STILL OWED — the offline acceptance below. TASK 2's CONTROL (a) IS DONE**,
run by Rich in his own terminal on 2026-09-20: `sudo ifconfig lo0 -alias 127.0.0.3` →
**`19 checks, 1 failed`**, the one being *both loopback aliases exist*; restored → 19/0. It also
measured that **`make doctor` alone cannot tell you production is reachable** — two checks naming
`127.0.0.3` stayed green with the alias gone, because `check_zone_split` asserts what dnsmasq
answers and never that anything listens. `make verify`'s *the public listener answers a
production name* would have gone red — **"would have" is a prediction**, and verify was not run
...
— **sitting 4 built them and sitting 5 made them BLOCK, so R1 is delivered end to end**; R2
...
checklist item, **which sitting 8 built (Task 12)**. **R4's spec action is ✅ APPROVED
...
2026-09-15 — and **P6a's R1 makes it MORE urgent, not less; since sitting 5 the platform
BLOCKS a production deploy on a real registration and a real PIA**, which
[`docs/external-track.md`](../external-track.md) now records. Plus the rest of §8.
...
- **COMMIT ON `main`, AND STAGE YOUR OWN PATHS BY NAME — §6 RULE 9 IS THE FULL RULE AND THIS
  ...
  not under sittings 5, 6, 7 or 8**, which makes it the exception rather than the rule either
...
- **P6a's THREE PROPOSED SPEC ACTIONS — RAISED 2026-09-19, none applied.** The plan's own
  *Spec actions* section has the exact wording for each; they are summarised here because §8 is
  ...
  mint a delegated token holding it** — **re-measured through the mint route by P6a sitting 1's `[M6]` on 2026-09-19, with `release:promote` refused `400 TOKEN_CAPABILITY_FORBIDDEN` beside it as the control, so this is a fact rather than a reading of the code.** P6a closes the hole with
