...
| `make verify` | **51 checks, 0 failed, 0 warnings** — and read its *per-app resources* INFO line, which is not a check: it should be **`containers=9 networks=3 volumes=6`** for the three deployed apps (three containers and two volumes each, one network each). More networks than apps means an app whose containers are gone and whose network is not; `0/0/0` is a freshly reset machine |
...
- **~~Seven app networks and one volume from dead test apps~~ — CLEARED 2026-09-18**, found while clearing the item above and removed by Rich the same day: `mf-blueprint-ntm-`, `mf-chem-labs-`, `mf-fixture-rt-`, `mf-fixture-s6-`, `mf-fixture-s6nb-`, `mf-saml-probe-` and `mf-saml-unsigned-staging-net`, plus `mf-chem-labs-staging-db-data`. **`make verify`'s per-app INFO line is the meter for this, and it now reads `containers=9 networks=3 volumes=6`** — three live apps, three networks, two volumes each — where it had read `10` and `7`. **That line is worth reading every sitting**: `0/0/0` is a freshly reset machine, and anything above three networks for three apps is an app whose containers are gone and whose network is not. Each removal disconnects `manifest-caddy` and `manifest-dns-containers` first (`ensureAppNetwork` attaches both to every app network by design) and is skipped unless those two are the ONLY things attached and no container of any state is named for that app — §4's *a network removed under a stopped container leaves it unable to start*. **The agent session was refused `docker network rm`** after being allowed 86 `docker image rm`s and a whole app teardown minutes earlier, so this went the way LiteLLM's deletes do: the agent measured and handed over, Rich ran it.
...
| **writing a plan** | None is waiting until P5b executes; then it is **P5c**. **Read [the P5 brief's §10, *What P5c INHERITS*](plans/2026-09-16-p5-brief.md) FIRST** — it is dated 2026-09-18 and written for exactly that job, and its first paragraph is the one that saves the plan: **§17's Phase 1c row is not P5c's scope**, because P5a and P5b already built four fifths of it. Then P5b's *What this plan does not build*. House style: [`plans/2026-08-30-p1-local-substrate.md`](plans/2026-08-30-p1-local-substrate.md), or any later plan. |
...
8. **Close out properly — AT THE END OF EVERY SITTING, not at the end of the plan.**
   ...
   **AFTER THE SWEEP, RE-READ YOUR OWN §7e AS A COLD AGENT AND CHECK ITS CLAIMS — THIS
   ...
   IT FOUND THREE AND IN SITTING 8 SIX.** *This list of sittings is the ONLY statement of that count; §7e points
   ...
   likely to cost them the sitting**. **Write the state table from `psql`, `docker` and `curl` at
...
2. **P5b's *What executing this plan found*, sitting 8's entry** — 16 findings. **F4 and F5 are the
...
| The database | **NO new migration this sitting; 0018 is still the newest.** **HOLDS `journey-app` AND NOTHING ELSE** — measured, not assumed: the gates' `pnpm test` ran AFTER the last `make demo-token` and truncated everything, and only the closing `make demo-journey` put rows back. So `journey-app` has its three environments, a build, a release and a healthy staging instance; **`token-app` has a running, serving container and NO project rows at all**, the shape the proof app has been in for sittings. `routes` holds one row, `journey-app`'s. Every `pnpm test` empties it again |
...
| The apps | `token-app`, `journey-app` and the proof app are **up and healthy** — **the fixture app was TORN DOWN after the sitting** (§2's *Outstanding*), so `make demo` now starts from nothing — and `token-app` and `journey-app` **are both reachable by name through the edge** — `make verify` reports 2 runtime routes. **But only `journey-app` has a `Route` ROW**: `token-app`'s was truncated by the gates' `pnpm test` after its demo ran, so its live Caddy route would NOT be restored if the edge were restarted. **`proof-app` is NOT** — its route is gone and its name falls through to the wildcard, confirmed again after the cleanup (`manifest OK host=proof-app.staging…`). `fixture-app` does not exist at all any more. **Check the BODY, never the status** — a name nothing has ever deployed also answers `200`. Harmless: the next `make demo*` recreates each |
| Identity | Three IdP test users — `student`, `instructor`, `operator`. **`operator` IS currently an administrator**, granted by this sitting's `make demo-journey`, which recreates and re-grants. **`student` (`stu000001`) HAS NOT SIGNED IN on this database** — `users` holds exactly `ins000001` (member) and `opr000001` (admin), measured at close. **So THE TRAP ABOVE IS ARMED.** `make demo-token` signs the student in on every run and so disarms it for itself; anything that hand-rolls a member add without that sign-in gets `400 MEMBER_USER_NOT_FOUND` |
| Docker images | **Far BELOW where the sitting started: `docker system df` Images 132 → 45** (`docker images -q | wc -l` says 53 — a different metric, not a disagreement; only three `local/*` app images remain, one per running app). The sitting's own five went first; then **P5a sitting 12's outstanding cleanup was discharged after it** — 86 unheld app images and the whole fixture app. §2's *Outstanding* has it, including that sitting 12's own KEEP list had gone stale and that **the classifier tightened MID-SESSION**. `base/*` and every held image were untouched |
| LiteLLM | **EMPTIED after the sitting — 19 rows to 4**, by Rich running `scripts/litellm-orphans.sh --apply` (§2's *Outstanding*). **Do not copy this row**: run `bash scripts/litellm-orphans.sh`, which changes nothing and re-derives the answer. It will have grown again, because every demo that replaces a project leaves one more |
