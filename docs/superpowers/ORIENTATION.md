...
**The next job is §7e — WRITE P5c, the clients. P5b executed on 2026-09-18 and nothing is part-finished. Nothing blocks it.** Each plan's own *What executing this plan found* is its record. The roadmap's ledger outranks this section on status.
...
distinction is sitting 8's F15, and it is the row below about `users` that it saved.*

| | |
|---|---|
| The four gate numbers | §2's box — `pnpm test` **1344** in 101 files (up 2, both in `tokens/token.test.ts`), `pnpm test:docker` **178** in 29 files (0 skipped, unchanged), `make doctor` **18/0**, `make verify` **51/0**. The last two were re-run AFTER the Docker tier, and after the control plane was restarted onto the restored tree |
| The machine was RESET | **`echo reset | make reset` ran in this sitting, with Rich's agreement**, for run 3 of the acceptance. So `journey-app`, the proof app and `fixture-app` **no longer exist** — their containers, networks and volumes are gone. `make demo`, `make demo-identity`, `make demo-ai` and `make demo-journey` each recreate their own app and were NOT re-run; all were green before the reset. **Only `token-app` is up** |
| The control plane | **LEFT RUNNING on 7100**, `driver: docker`, restarted many times this sitting — once per negative control — and last restarted onto the RESTORED tree, after `pnpm test:docker`. Check: `curl -sS --cacert infra/ca/manifest-root.crt https://console.manifest.internal/v1/me` → `401 UNAUTHENTICATED`. **Do not start a second one.** Kill it by PID from `lsof -nP -iTCP:7100 -sTCP:LISTEN` — `pkill -f 'control-plane/dist'` does NOT match it |
| The database | **NO new migration this sitting; 0018 is still the newest.** `token-app` and NOTHING else — but **the gates' `pnpm test` ran after the last demo and truncated everything**, so what is actually there is: `projects` **0**, `users` **0**, `routes` **0**, `delegated_tokens` **0**, `pending_actions` **0**. `token-app` has a running, serving container and NO project rows at all, which is the shape the proof app spent sittings in. Every `pnpm test` empties it again |
| Identity | **`users` IS EMPTY** — measured, not assumed. So **nobody has signed in on this database**, `student` included, and **§7e's long-standing trap is ARMED**: `POST /v1/projects/{id}/members` answers `400 MEMBER_USER_NOT_FOUND` for anybody who has never signed in. `scripts/demo-token.sh` signs the student in on every run and disarms it for itself; anything that hand-rolls a member add must do the same. There is also **no administrator** until a `make demo-journey` re-grants `operator` |
| The apps | **`token-app` only** — three containers, up and healthy. **It is NOT reachable as itself**: `https://token-app.staging.manifest.internal` answers `manifest OK host=token-app.staging…`, which is the EDGE'S WILDCARD — a name nothing ever deployed answers byte-identically, checked. `make verify` says *runtime routes currently applied: **0***. `pnpm test:docker` restarted the edge and dropped its runtime routes, and the control plane then booted with `routesRestored: 0` because the gates' `pnpm test` had already truncated `routes`. Harmless — the next `make demo-token` puts it back. **This row was WRONG until it was checked**: the body is the test, never the status |
| Owed to Rich | **Seven dead app networks and one volume**, all recreated by this sitting's `pnpm test:docker` — §2's *Outstanding* has the names and the reason. Run `bash scripts/dead-app-resources.sh` to re-derive and hand the output over; **an agent cannot apply it**. Also **LiteLLM's orphans**, which will have grown: `bash scripts/litellm-orphans.sh`, same split |
| Docker images | The Docker tier left about eleven more `local/*` app images. `docker system df` and `docker images -q | wc -l` count differently — **name the command beside any number** (sitting 8's F16) |
