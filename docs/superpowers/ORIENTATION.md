...
*Last verified 2026-09-24, at the close of the D5 plan's sitting 1 (the measurements), which changed no code and moved no
number. `pnpm test` **1742 passed, 0 skipped, in 123 files**, twice at the open and twice at the close, identical.
**`make doctor` 19 checks, 0 failed, 0 warnings** — the vulnerability database goes stale again after 2026-10-01
(`make refresh-vulndb`). **`make verify` 55.** `pnpm test:docker` **200 in 31** — not owed by sitting 1 and not run;
that is the F10 sitting's figure. `scripts/ci-acceptance.sh` reads `1742 / 123 / 19 / 55`. **This line states only the
...
**The four numbers you will check first, re-measured 2026-09-24 on this machine at the open AND the close of the D5 plan's sitting 1 (the measurements; its record is in the plan). `pnpm test` **1742** in **123** files, unmoved (twice at the open, twice at the close); `make doctor` **19** with **0 warnings**; `make verify` **55**; `pnpm test:docker` **200** in 31 — the F10 sitting's, since sitting 1 did not owe it:**
...
- **S6 PROBE 14 NEEDS OLLAMA'S CHAT MODEL RESIDENT, AND THE DOCKER TIER DOES NOT MAKE IT SO** (P6b sitting
  ...
  `default-chat` is `ministral-3:latest` — 9.7 GB — so a model that has not been loaded for a day, on a
  machine under memory pressure, answers `000`. Warm, the file is 18/18 and the probe takes 7.9 s. To warm
  it: `curl -s http://127.0.0.1:11434/api/generate -d '{"model":"ministral-3:latest","keep_alive":"30m"}'`.
...
- Nothing moved a sitting boundary. **F7 (Task 13's check) is Rich's in sitting 7**, not yours.
...
- **`pnpm test:docker` IS OWED THIS SITTING** (`releases/`, the build callers, `secrets/`), ~20 min. **It restarts
  the edge and truncates the tables — restart the control plane afterwards.** **S6's probe 14 needs `ministral-3`
  RESIDENT**, and sitting 1 unloaded it: warm it first (§2's `pnpm test:docker` row has the `curl`), or read a
  `000` there as that precondition, not a regression.
...
- `dead-app-resources.sh` reads **none dead**, and `litellm-orphans.sh` reads **nothing to delete**.
- **Images: `docker images -q` 192, `sort -u` 184, `127.0.0.1:7107/local/*` 145.** Ollama 0.34.4 with **no model
  resident**.
- The gates at close: `pnpm test` 1742 in 123 (twice), lint, typecheck and format clean, `make doctor` 19/0/0,
  `make verify` 55/0. The snapshot diff was uptimes, free disk (104 → 120 GiB, not the sitting's) and the working
  tree.
...
- **Task 13's summary check withholds 7 in 10 under Task 13's own prompt — none of them a verdict. RAISED 2026-09-24
  (the D5 plan's sitting 1, F7); ANSWER BEFORE SITTING 7 BUILDS TASK 13**, per that task's own rule (*"more than 5 of 10
  withheld → raise it with Rich"*). Twenty answers per prompt: the current prompt attributes a verdict to the coverage
  sentence or an administrator 6 times in 20 (P6b's F9, reproduced); Task 13's prompt 0 in 20 — but its `checkSummary`
  withholds 14 of 20, 11 for backticks round a hostname and 3 for *"previously blocked"* describing egress.
  **Recommended: (a) keep backticks (strip them for display) and drop "blocked" from the decision words** — 0 of 20
  withheld, all six inventions still caught. (b) Build it as written: an administrator sees the facts without a summary
  about 7 times in 10. (c) Keep the rules and lean harder on the prompt, which already says *"no backticks"* and is
  ignored 11 times in 20. The full question is at the top of the plan's Task 13; the answers are in
  [`spikes/d5-baseline/`](spikes/d5-baseline/README.md), `[M15]`.
