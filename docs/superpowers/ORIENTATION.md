...
**Manifest's design is finished and SEVEN implementation plans are executed — P1, P2, P3, P4a, P4b, P4c and P5a (the contract), whose acceptance passed on 2026-09-17. P5b is written and UNDER WAY: sittings 1 to 4 are done, so D24's privileged four are named, an agent holding a delegated token can authenticate with it, and **asking for one of the privileged four is now refused centrally and creates a `PendingAction` a human must confirm**. The next job is always §7e.** This is the single entry point: what Manifest is, where things stand, how the platform is built, what the machine will do to you, how to work here, and what to do next. It is written for someone with **no prior context** — a new agent with a fresh window, or a developer joining.

*Last verified 2026-09-18 (P5b sitting 4).* **Three places state current status — §2, §7e and §8 — and a sitting's sweep REPLACES what they say; it never appends a sitting's story here** (§6). **The roadmap's ledger outranks all three.** Everything else is durable.
...
**The next job is §7e — P5b's sitting 4, Task 6, alone. Nothing blocks it.** Each plan's own *What executing this plan found* is its record. The roadmap's ledger outranks this section on status.
...
### 7e. Execute P5b's sitting 5 — Task 7 ← **START HERE**

**P5b's sitting 4 is DONE** (2026-09-18, 10 findings, `11c47b9`, `f9f56f6`, migration 0016).
[`plans/2026-09-17-p5b-delegated-tokens.md`](plans/2026-09-17-p5b-delegated-tokens.md) — 13 tasks
in nine sittings. **Your job is sitting 5: Task 7, ALONE** — confirm, reject, and the one-shot
retry, which is the loop closing. The sittings table at the top of that plan is the maintained
copy of what is done.

**Read, in this order, before you start:**

1. **Task 7's correction block — THREE paragraphs, and the third says the task's own Step 1
   test is wrong.** The first is `[M5]`'s: **the retry reuses the SAME `Idempotency-Key`** as
   the refused request, because D23.6's own error hint tells a client to reuse a key across
   retries of an action. Sitting 4 made that possible by catching the refusal **outside**
   `app.idempotent`; F3 of its record is the measurement, and the test that sees it is
   `delegation.test.ts`'s *stores NO idempotency record for a refusal*. **The second is the one
   a reader skips and should not**: a confirmed retry's `2xx` **is** stored under that key, so
   replaying the key returns it for ever with no capability check — *"exactly once" describes
   the ACTION, not the ANSWER*. **The third, added by sitting 4, is the consequence**: Step 1's
   *lets the agent's retry through exactly once* calls `ask()` three times on one key and one
   body and expects the third to be `403`, which `replayOrStore` answers `201` from the store.
   Measured. Use a **fresh key for the second retry**, and assert the replay separately.
2. **P5b's *What executing this plan found*, sitting 4's entry** — 10 findings. **F1 is the one
   to read**: the plan's own ordering control could not fail against any of the 31 tests that
   existed, because every test writes a token holding the privileged capability and **no token
   the platform can mint can hold one**. Expect the same shape in Task 7 — a control whose
   fixture is not the state a real token is ever in.
3. **The correction block at the top of TASK 4 — the PLAN-WIDE one.** Every snippet still names
   test users that do not exist and a fixture under the wrong name. Counted at the end of
   sitting 4 by counting Tasks 7–13 rather than by subtracting, the tasks you have left
   carry **16 `withProject(async …)` that must be `withProjectServer`** and **13
   `ins000001` / `stu000001` that must be `bio_prof` / `bio_student`** — `ensureTestUser`
   throws by name and `tsc` refuses first.
...
**What sitting 4 built, which you now consume:**

- **`TokenCapabilityRefusedError`** (`projects/authz.ts`) — carries `capability`, `projectId`,
  `tokenId` and nothing about the request. Thrown by `assertCapability`'s token branch, whose
  three checks are ORDERED — scope, the privileged rule, then the token's own set — with the
  reasons written in the file.
- **`tokens/pending.ts`**: `recordPendingAction(db, bus, { error, fingerprint, now? })`,
  `pendingById`, `fingerprintOf({ method, url, body, summary })`, `bodySha256`,
  `PENDING_ACTION_TTL_MS` (24 h) and **`PendingActionRequiredError`**, which is what the
  wrapper throws once the row exists. It imports nothing from `api/`, deliberately.
- **`api/representations/pending-actions.ts`** — `PendingAction` and `toPendingAction(row)`.
  **There is no `PendingActionList`; it is yours or Task 8's**, and sitting 4's F4 says why it
  was removed.
- **`ErrorEnvelope.pendingAction`**, `$ref`'d, optional, parsed on the way out by `question()`
  in `api/errors.ts`, which **fails closed** exactly as `checklist()` does.
- **`TOKEN_ACTION_PENDING`** in `api/error-codes.ts`, listed on the two routes that can answer
  it (`addMember`, `deploy`). **Migration 0016** (`pending_action.created`). **0016 is the newest.**
- **`refusal()` is now in `api/testing.ts`**, not local to `credential.test.ts`. Use it: assert
  `{ status, code }` as ONE object.

**What will surprise you:**

- **Every delegation test mints its token STRAIGHT TO THE STORE holding a privileged
  capability, and that is not the state a real token is ever in.** Task 4's route refuses one,
  so the only realistic case is a token minted **without** it — which is the case sitting 4's
  F1 found no test covered. **Task 7's confirmed retry is this case**: a human confirms an
  action for a token that does not hold the capability, so the grant must be what lets it
  through, not the set. Write the test with a token minted the way a real one is.
- **The catch in `api/contract/route.ts` is OUTSIDE `app.idempotent` and must stay there.**
  The comment above it is long for a reason. Task 7's whole loop depends on it.
- **A `403` for the wrong reason is this plan's recurring defect — four sittings running.**
  Assert the CODE, never the status alone.
- **`api/stream-contract.test.ts` asserts its lifecycle reaches EVERY `EVENT_TYPE`.** Task 7's
  new event types must be driven through that lifecycle, not added to `PUBLISHED_ELSEWHERE`.
  Sitting 4 drove `pending_action.created` by injecting a refused request into it.
- **`api/authz-contract.ts` goes red the moment you register a route and do not list it**, even
  though the plan assigns that file to Task 11. Task 7 registers confirm and reject.
- **A route change is three files, in order**: the definition, `pnpm contract:write`,
  `pnpm contract:generate`. **A representation registered and answered by no route is published
  anyway** — sitting 4's F4.
...
- **`pnpm test:docker` is owed.** Task 7 changes `projects/authz.ts` and `api/contract/route.ts`.
  It has not moved from 177 in five runs across three sittings.
...
**The state you are handed, 2026-09-18, after P5b's sitting 4.** `main`, clean.

| | |
|---|---|
| The four gate numbers | §2's box — `pnpm test` **1108** in 99 files (up 20), `pnpm test:docker` **177** in 29 files (0 skipped), `make doctor` **18/0**, `make verify` **51/0**. The last two were re-run AFTER the Docker tier |
...
| The database | Migration **0016** applied. **NOT empty**: the closing `make demo-journey` left `journey-app`, its three environments, its build, release and instance, and the `operator` administrator grant. Every `pnpm test` empties it again |
| The demos | **`make demo-journey` was run at the end and is GREEN, all eight steps** — not asked for by the task, but Task 6 changes `api/contract/route.ts`, the wrapper EVERY `/v1` route runs through. The other four were not run; all were green at P5a sitting 12 |
...
| Docker images | **Back to where the sitting started.** The five it created were removed by digest — the four its Docker-tier run built, and `journey-app@a767606893e3`, which sitting 3 left because it backed a running instance, a reason this sitting's demo ended. The one image it is left up on is `journey-app@34f59b854bb4`, which backs the instance the closing demo deployed. **The `docker rmi` refusal did not recur, for the third sitting running.** Sitting 12's own list is still outstanding |
| LiteLLM | **TWELVE users, two held by a running container — re-measured independently this sitting, and the previous list's held entry had gone stale for the FOURTH sitting running.** §2's *Outstanding* has all twelve with the command that checks a key without printing it. **Re-measure before acting; do not copy this row** |
