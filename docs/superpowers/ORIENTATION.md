...
unable to fail, and this plan has now found one in every sitting but the first two.**
...
2. **P5b's *What executing this plan found*, sitting 8's entry** — 11 findings. **F4 and F5 are the
...
- **`scripts/demo-token.sh` + `packages/journey/src/token.ts`** — a SIBLING entry point to
  `main.ts`, not a phase of it, because this story has two credentials in it. Nine steps; the
  RUNBOOK's *`make demo-token`* section lists them and says what it leaves behind.
