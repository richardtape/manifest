...
redeploys — IS WRITTEN (2026-09-15) AND IS BEING EXECUTED: sitting 1 is DONE (2026-09-15, 13
findings), and SITTING 2 IS NEXT (§7d-3).** This is the single
...
2026-09-09, with 10 findings, sitting 2 (Task 3) on 2026-09-14, with 6, sitting 3 (Tasks 4–5) the same day, with 11, sitting 4 (Tasks 6–7) the same day, with 12, sitting 5 (Tasks 8–9) the same day, with 11, sitting 6 (Task 10) the same day, with 14, sitting 7 (Tasks 11–12) the same day, with 16, sitting 8 (Task 13) on 2026-09-15, with 20, sitting 9 (Tasks 14–15) the same day, with 18, and sitting 10 (Task 16) the same day, with 22 — 140 in all. P4c IS WRITTEN AND BEING EXECUTED — SITTING 1 IS DONE, 2026-09-15, WITH 13 FINDINGS**
...
| | |
|---|---|
| `pnpm test` (from the **repo root**) | **736 passed, 66 files**, ~45 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **133 tests, 25 files, all passing**, ~478 s — P4b sitting 10's run, unchanged in count: its proofs are `make demo-ai` and one unit test. Needs `make up`, and **fails rather than skips** when asked to run |
...
The immediate work is **executing P4c — zero-downtime redeploys — SITTING 2, WHICH IS TASK 2 ALONE** (§7d-3). **Sitting 1 is done (2026-09-15, 13 findings):** `make demo-redeploy` exists, was run three times and exited 1 each time with eleven assertions red, and the baseline it measured is in the plan's record. **Not the whole plan in one session:** eight agreed sittings, one per session, with a check-in at each boundary. P4b is finished; its sitting-10 record, the last entry in its *What executing this plan found*, is the one to read first. P4a's fifteen tasks are done: the IdP
...
│                                                  SITTING 1 DONE 2026-09-15,
│                                                  13 findings. Its brief,
...
## 7. What to do next — execute P4c, ONE SITTING AT A TIME (sitting 1 is done; sitting 2 is next)
...
pin on 2026-09-09, sitting 2 ran Task 3, sitting 3 ran Tasks 4–5, sitting 4 ran Tasks 6–7, sitting 5 ran Tasks 8–9, sitting 6 ran Task 10 and sitting 7 ran Tasks 11–12, all on 2026-09-14, sitting 8 ran Task 13, sitting 9 ran Tasks 14–15 and sitting 10 ran Task 16, all on 2026-09-15**, so P4b is finished and P4c — §7d-3 — is WRITTEN (2026-09-15) and being executed: **sitting 1, Task 1 alone, is done (2026-09-15, 13 findings), sitting 2 is next, and the plan runs one sitting per session** exactly as P4a's seven and P4b's ten did. **P4b ran in ten agreed sittings**,
...
**WRITTEN 2026-09-15 AND BEING EXECUTED: [`plans/2026-09-15-p4c-zero-downtime-redeploys.md`](plans/2026-09-15-p4c-zero-downtime-redeploys.md) — 11 tasks in EIGHT AGREED SITTINGS, ONE PER SESSION. SITTING 1 IS DONE (2026-09-15, 13 findings). SITTING 2 IS NEXT, AND IT IS TASK 2 ALONE.**
...
back 502**. Two of the thirteen findings were defects in the harness that made the measurement
meaningless — the acceptance rate-limited itself, and a fixed warm-up let a whole run answer zero
questions — and both were fixed before the baseline was taken. The five measurements settled every
constant the design left open: they are in the plan's record and the four durable ones are in §4.
**Sitting 2 is Task 2 alone**: §11's contract in code — `InstanceSpec.instanceId`/`hostname`, the
four new methods, the fake driver's routes and in-flight counters, and the contract suite's
continuity block. From sitting 2 on, **`pnpm test:docker` is part of every sitting's close-out**;
sitting 1 did not run it, because it touched no TypeScript and it restarts the edge.
