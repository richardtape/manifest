...
**The four numbers you will check first, re-measured 2026-09-23 on this machine at the open AND the close of P6b sitting 2 — Tasks 2 and 3. `pnpm test` MOVED, 1605 → **1635** and 119 → **121** files (twice after each commit); `make doctor` **19** — with ONE WARNING since `d0a5aad`, and it is true; `make verify` **55**; `pnpm test:docker` **194** in 31, owed and run, with one calendar red fixed the same sitting:**

| `pnpm test` (from the **repo root**) | **1635 passed, 0 skipped, 121 files**, ~140 s — **up 30 and two files in P6b sitting 2**: `projects/person-only.test.ts` (6, new) and `api/person-only.test.ts` (6, new) for Task 2; seven in `spec/diff.test.ts`, five in `releases/approval.test.ts` and five in `api/delivery.test.ts` for Task 3; one in `releases/releases.test.ts` (`88fd228`). Run twice alone and identical after each of the sitting's code commits and at the close. `scripts/ci-acceptance.sh`'s `EXPECT_TESTS` is **1635**. **Never run a second Vitest process beside it** — the shared global setup deletes the test repositories and truncates the tables in both (§4). **Each earlier sitting's movement is in its plan record**, dated; this row states only the latest. It is the `unit` project and `packages` (the client, the journey, the console and the mock, none of which need anything running). No Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/`, `releases/`, `tokens/` and `launch/` suites, plus `spec/injection-drift`, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` (§3) |
| `pnpm test:docker` | **194 tests, 0 SKIPPED, 31 files, 1206.6 s** on its last run — **P6b sitting 2's, owed (`projects/`, `spec/`, `releases/`) and run once after the code commits: 193 passed and ONE red on the CALENDAR** — `build/scan.docker.test.ts`'s *blocks the identical image when the base is unknown* asserted `blocked: true`, which is true only while the vulnerability database is under a week old, and it had just passed a week (F12). **Fixed in `d0a5aad`** — the test now asserts the fail-closed half that holds on any date — **and its file re-run alone 6/6**, so the tier stands at 194 in 31. Each earlier sitting's figure is in its plan record. Not every sitting owes the tier — P5c's 4 to 7 and P6a's 1 neither ran nor owed it, so **do not spend 14 minutes on it out of habit**; the plan's own sittings rule says who owes it. `boot.docker.test.ts`'s *expires a question nobody answered* is still the ONLY test in the repository that fails if `src/index.ts` stops calling the expiry sweeper. The tier still drives SESSIONS: `grep` over every `*.docker.test.ts` finds no `Authorization: Bearer` that is a Manifest delegated token. **Running it costs the machine**: it restarts the edge (dropping every runtime route, so every app hostname then answers the WILDCARD), truncates the tables, re-registers the platform's SP row at a loopback ACS — **restart the control plane afterwards** — and regenerates exactly seven dead app networks and one volume. Needs `make up`, and **fails rather than skips** when asked to run |
| `make doctor` | **19 checks, 0 failed, 1 warning — AND THE WARNING IS TRUE** (P6b sitting 2, F13): *the vulnerability database is fresh* now compares SECONDS with the scan gate's own seven-day threshold, where it used to floor to whole days and pass up to eight — so for a day every week it called a database "fresh" that every scan was already calling stale. It will read 0 warnings again once `make seed`, with the network on, refreshes the database (§2 *Outstanding*). The CHECK count is unmoved by P6a sittings 3 to 11 and P6b sittings 1 and 2; up one in P6a sitting 2, which REPLACED the single-alias check with a two-alias loop (net 0) and added *the production zone answers the public address, and the console does not*. That check asserts BOTH directions: one asserting only the production answer stays green while the console has moved to the public address. **It cannot tell you production is REACHABLE** — Task 2's control (a) measured two checks naming `127.0.0.3` staying green with the alias removed, because they assert what dnsmasq answers |
| `make verify` | **55 checks, 0 failed, 0 warnings** — **unmoved by P6a sitting 11 and P6b sittings 1 and 2**; up one in sitting 10 (*the registry's realm is the control plane's, for the service it mints*), three in sitting 2 for §12's second listener. **Straight after `make reset` it reads ONE red — the audit grant — until the control plane has migrated the empty database** (§4). **It reads `mf- containers=12 networks=4 volumes=8` today** (P6b sitting 2's close, after both cleanup scripts) and `runtime routes currently applied: 0` — **the Docker tier restarted the edge** and dropped `launch-app`'s routes, and with an empty database nothing re-applies them. **That meter counts `srv0` only**, so it never sees a production route on `srv1` (P6b sitting 1, F13); at sitting 1's close it read `1` with an app live in both environments |
| `make demo-production` | **NEW in P6a sitting 11 — P6a's acceptance, green FOUR times**: fresh (after `pnpm test`), on the re-use path, from an `echo reset \| make reset` machine, and on the re-use path again on the final code. ~3 min, three phases each ending `every check passed`. It is step 11 of `scripts/offline-acceptance.sh` and the last step of `make ci-acceptance`. It leaves `launch-app` in production. **Green twice more in P6b sitting 1** (fresh 62 s, re-use 61 s) — **and its re-use path's step 5 rehearses an app that has already launched, retiring the live approved instance for an unapproved one for about a second** (P6b sitting 1, F2), which P6b's Task 4 removes. **It also leaves its step-10 rebuild, unapproved, serving STAGING**, so after it the candidate is not the release production runs |
...
| `make ci-acceptance` | **NEW in P5c sitting 8, and RUN**: the headless half of 1c's acceptance — `make doctor`, `make verify`, the three fast gates, `pnpm test` with its COUNT asserted, the three package builds, then `make demo-journey`, `make demo-token` and — since P6a sitting 11 — `make demo-production`. Every step REPORTS rather than exits, so a red run is a measurement; a count that has moved reads `MOVED`, not `FAIL`. ~15 min. **Its four `EXPECT_` lines carry this box's numbers and move with it — and they did NOT, from P6a sitting 2 until sitting 3 found them reading 18 and 51 against a machine at 19 and 54.** They are now **1635 / 121 / 19 / 55**, moved by P6b sitting 2 in its close-out commit (and that sitting's post-sweep check found this very sentence still reading 1605 / 119 — P6b sitting 2, F18), and §6's sweep table has a row for this file so it cannot happen again — **and sitting 9's own post-sweep check found this line still reading 1565 / 117 after the box above it had moved**, which is the same defect one screen further down. **`EXPECT_TESTS` is the PASSED count**, which mattered from sitting 5 to sitting 8, when the suite had a skipped test; it has none now |
...
- **Refreshing the vulnerability database — `make seed` with the network on (NEW, P6b sitting 2).** It was built `2026-09-16T06:30:57Z` and passed §12's seven-day threshold on 2026-09-23, so every scan now WARNS rather than blocks at build time (by design — C1 lets an offline laptop deploy to staging) and `make doctor` says so with its one warning (F13). **AND IT BLOCKS EVERY PRODUCTION LAUNCH** (F14): §13's `scans` item is `unmet` for a candidate scanned against a stale database, so `make demo-production` — P6b sitting 3's Task 4 Step 5 — is red until it is refreshed. No test depends on the database's age since `d0a5aad`. It recurs weekly by construction, and it needs the network, which makes it Rich's call: `make seed`, or its one line, `docker run --rm -v manifest-grype-db:/db -e GRYPE_DB_CACHE_DIR=/db anchore/grype:v0.118.0 db update`.
...
### 7e. Execute P6b's sitting 3 — Tasks 4, 5 and 5a ← **START HERE**
...
naming it, and an added CWL attribute also waits for UBC IAM. **Sittings 1 and 2 are done**: the measurements
(2026-09-22), then Tasks 2 and 3 (2026-09-23) — the person-only class is one central rule answered `403
TOKEN_PERSON_ONLY`, the contract is `1.1.0`, `isSensitiveDiff` sees a production override, the re-escalation
baseline is each release's LATEST decision, and a release freezes its build's own spec. **Your job is sitting 3:
Task 4 (an app has launched — migration 0021), Task 5 (`deployRelease`'s half of D9.2) and Task 5a (the egress
proxy follows the release it serves — sitting 1's F1).** Use `superpowers:executing-plans` and **commit on
`main`, one commit per task** (§6 rule 9). **`pnpm test:docker` IS OWED** (`releases/`, `launch/`, `runtime/`,
`*.docker.test.ts`) — budget ~23 minutes for it after the last commit, and restart the control plane afterwards
only if it was running when you opened. **The lean split's rule: if the sitting runs long, stop after Task 5 and
sweep; Task 5a then opens sitting 4, ahead of Task 6, with which it shares nothing** (Rich confirmed on
2026-09-23 that 5a in sitting 3 is fine).

**READ, IN THIS ORDER:** the plan's sittings table; **sitting 2's record** at the end of *What executing this
plan found* (its findings are mostly about the plan's CONTROLS — read F6 to F9 before you trust a prediction);
then **Task 4, Task 5 and Task 5a in full, each starting with the correction block at its top** — Task 4 and Task
5 both gained one from sitting 2. Sitting 1's raw measurements are in
[`spikes/p6b-baseline/`](spikes/p6b-baseline/README.md) — Task 5a's commands are in its results file.

**WHAT SITTING 2 LEFT IN YOUR THREE TASKS — READ THE BLOCKS, THIS IS ONLY THE LIST:**

- **Task 4** — `commitManifest` already exists in `api/testing.ts`, with the signature the plan's fixtures
  section now gives (`ctx` is `{ app, deps, cookies, project: { id, slug } }`, and it throws). **The helper move
  must take `projectFor` too**, because `builtProject` calls it, and Task 3 added a `describe` at the end of
  `delivery.test.ts` that calls both. **The contract is already `1.1.0`**: `Project.launchedAt` regenerates the
  document and does not bump it.
- **Task 5** — `sensitiveChangeOf` is there with the planned shape, and **with no baseline it answers `fields:
  []`** — `approvalRequirementFor` must read a missing baseline as *approval required*. **Its doc comment says its
  only caller is its test and names Task 5: rewrite that paragraph when you call it.**
- **Task 5a** — unchanged since sitting 1. Its Step 5 drives `launch-app` through the edge, so it needs
  `make demo-production` to have run first (Task 4's Step 5 runs it).

**THE THINGS MOST LIKELY TO COST YOU:**

- **FIRST: IS THE VULNERABILITY DATABASE FRESH? IF NOT, TASK 4's STEP 5 CANNOT GO GREEN** (sitting 2, F14).
  `make doctor`'s *the vulnerability database is fresh* WARNED at sitting 2's close (7.4 days). §13's `scans`
  item is `unmet` for a candidate scanned stale, so `make demo-production` goes red at its step 3 with `scans`
  in the unmet set. **If it still warns when you open, ask Rich to refresh it before you reach Step 5** — it needs
  the network (`make seed`, or the one `grype db update` line in §2 *Outstanding*). Task 4's Steps 1–4, Task 5
  and most of 5a do not need it.
- **Your first test run is the baseline, so run the gates before touching code**: `pnpm test` twice (**1635 in
  121**), `pnpm lint`, `pnpm typecheck`, `pnpm format:check`, `make doctor` (**19 checks, 0 failed, and the one
  warning above** — none once the database is refreshed) and `make verify` (55). A different number is signal
  before it is your fault.
- **A migration is `pnpm --filter @manifest/control-plane db:generate`, then READ it** — drizzle writes the
  `audit.events` CHECK's `DROP`/`ADD` pair itself, and appending it by hand makes the migration fail to apply
  (P6a `[M10]`). Task 4's is **0021**.
- **Task 4's Step 5 runs `make demo-production` fresh AFTER a `pnpm test`, then on re-use** — and a `pnpm test`
  between a staging deploy and a production one truncates the project (the plan's first trap). Order the sitting
  so no Vitest run sits between them.
- **A missing export is `undefined` in Vitest, not an import error** (sitting 2, F1): a test that only uses a
  not-yet-written export in a way that tolerates `undefined` is green before the feature exists. Watch every new
  test go red for the reason you named.
- **Predict every control's reds by COUNT and by CODE before running it.** Sitting 2 found two predictions that
  under-counted and one that named the wrong code for one actor in four (F6, F7) — a wrong prediction is a
  finding, and an unpredicted green is a question (F8, F9).
- **Two Vitest processes cannot run at once** (§4). The Docker tier after all three commits, alone.
- **If a snippet defines `local path=…`, it breaks in zsh** (§4, P6b sitting 1's F9). Run the plan's bash under
  `bash`.

**THE MACHINE, AS SITTING 2 LEFT IT — query every one of these again; do not work from this line.**
**`make doctor` 19/0 WITH ONE WARNING, AND THE WARNING IS TRUE** (sitting 2's F13): the vulnerability
database was 7.4 days old at sitting 2's close and is older when you open; `make seed` with the network on
refreshes it, which is Rich's — until then every scan warns rather than blocks at build time (§12), and **§13's
`scans` item refuses every production launch** (F14, above). **`make verify` 55/0**, **`pnpm test` 1635 in 121** (twice), `pnpm test:docker` **194 in 31**
(193 passed and one calendar red, F12, fixed in `d0a5aad` and its file re-run 6/6), **21 migrations**, **the
database EMPTY**, **nothing listening on 7100, 7102 or 7104**, all three loopback aliases on `lo0`. `launch-app`
stands in production and staging on R2 — containers only: no rows hold it, and **no routes, because the Docker
tier restarted the edge** (`runtime routes currently applied: 0`), so its hostnames answer the wildcard —
with `click-launch` beside it; `make verify` reads `mf- containers=12 networks=4 volumes=8`. **Task 4's Step 5
runs `make demo-production` fresh, which rebuilds what it needs.** The control plane was stopped at sitting 2's
open and close, and it re-registers the platform's SP row at every boot. **Docker Desktop may not be running
when you open** (`open -a Docker`, §4).
