...
(2026-09-16): 17 tasks in twelve agreed sittings. ITS SITTINGS 1, 2 AND 3 ARE DONE (2026-09-16, 9, 16 and 15 findings) — the measurements; the API under `/v1` and through the edge at `https://console.manifest.internal`, refused to every source but the host; then CSRF by `Origin`, a sign-in bound to its browser, and one registry of every error code — and the next job is EXECUTING SITTING 4 — §7e says what that is.** This is the single
...
**The four numbers you will check first, measured 2026-09-16 on this machine, at the end of P5a sitting 3:**

| | |
|---|---|
| `pnpm test` (from the **repo root**) | **859 passed, 76 files**, ~50 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **168 passed, 0 SKIPPED**, 27 files, ~755 s — measured at the end of P5a sitting 3, which added no Docker test and changed `identity/saml.docker.test.ts`'s two; sitting 2's S6 probe 15 is the 168th (the edge refuses an app the control plane, with the host as its control). The eleven skips that used to be here were the driver contract's continuity block, gone since P4c Task 5 gave the Docker driver its fixtures. Needs `make up`, and **fails rather than skips** when asked to run |
...
│                                                  SITTINGS 1–3 DONE, 4 NEXT. Its six spec
...
**P5a IS WRITTEN (2026-09-16) AND ITS SITTINGS 1, 2 AND 3 ARE DONE, THE SAME DAY — YOUR TASK IS SITTING 4: TASK 6 (the contract's spine — `defineRoute`, the OpenAPI document generated from the route definitions and checked in, its drift test, and `GET /v1/me` as the first converted route).**
...
**Read, in this order, before sitting 4:**

1. **The plan's** *How this plan is to be executed*, *Read this first*, *Decisions Rich made*, *Decisions this plan makes* (1–10 above all) and **sittings 1, 2 and 3's entries in *What executing this plan found*** — then **Task 6 in full**, including the *Task 1 results* and the *Sitting 3 correction* at its top.
...
**The state you are handed, 2026-09-16, after sitting 3.** `main`, clean. The code commits are Task 4's `b59ef28` and `998557e`,
Task 5's `6b3ae28` and `cbd4ce5`; everything later is documentation. The four gate numbers are §2's box — all
green, `pnpm test` 859, `make verify` 50/0 and `pnpm test:docker` 168. **The proof app serves at `https://proof-app.staging.manifest.internal/`,
with a project row, a `Route` row and one LiteLLM key behind it** (`make demo-identity` ran last, after the Docker tier), so
