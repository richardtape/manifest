...
| | |
|---|---|
| `pnpm test` (from the **repo root**) | **748 passed, 66 files**, ~45 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **133 passed and 11 SKIPPED**, 25 files, ~480 s. The eleven are the driver contract's continuity block, which the Docker driver has no fixtures for until P4c Task 5 — it is skipped **with the reason in its name**, never silently. Needs `make up`, and **fails rather than skips** when asked to run |
...
The immediate work is **executing P4c — zero-downtime redeploys — SITTING 3, WHICH IS TASKS 3 AND 4** (§7d-3). **Sittings 1 and 2 are done (2026-09-15, 13 findings and 4):** `make demo-redeploy` exists and is red by design, the baseline it measured is in the plan's record, and §11's redeploy contract is in code — `InstanceSpec.instanceId`/`hostname`, the four new methods, and the contract suite's eleven-test continuity block, which the fake driver passes and the Docker driver skips with its reason until Task 5. **Not the whole plan in one session:** eight agreed sittings, one per session, with a check-in at each boundary. P4b is finished; its sitting-10 record, the last entry in its *What executing this plan found*, is the one to read first. P4a's fifteen tasks are done: the IdP
...
## 7. What to do next — execute P4c, ONE SITTING AT A TIME (sittings 1–2 are done; sitting 3 is next)
...
pin on 2026-09-09, sitting 2 ran Task 3, sitting 3 ran Tasks 4–5, sitting 4 ran Tasks 6–7, sitting 5 ran Tasks 8–9, sitting 6 ran Task 10 and sitting 7 ran Tasks 11–12, all on 2026-09-14, sitting 8 ran Task 13, sitting 9 ran Tasks 14–15 and sitting 10 ran Task 16, all on 2026-09-15**, so P4b is finished and P4c — §7d-3 — is WRITTEN (2026-09-15) and being executed: **sittings 1 and 2 are done (2026-09-15, 13 findings and 4), sitting 3 is next, and the plan runs one sitting per session** exactly as P4a's seven and P4b's ten did. **P4b ran in ten agreed sittings**,
...
**WRITTEN 2026-09-15 AND BEING EXECUTED: [`plans/2026-09-15-p4c-zero-downtime-redeploys.md`](plans/2026-09-15-p4c-zero-downtime-redeploys.md) — 11 tasks in EIGHT AGREED SITTINGS, ONE PER SESSION. SITTINGS 1 AND 2 ARE DONE (2026-09-15; 13 findings and 4). SITTING 3 IS NEXT, AND IT IS TASKS 3 AND 4.**
...
**What sittings 1 and 2 did, and what sitting 3 inherits.**
...
**Sitting 3 is Tasks 3 and 4**: the edge — `applyRoute` upserting in place, the deferred
`X-Manifest-Instance` header, `servingRoute`, `inFlightTo` — and then the Docker
`ensureInstance` that uses it: beside, privately ready, moved, verified, rolled back.
Sitting 1's M1, M2 and M3 measured the three facts it rests on, and **`pnpm test:docker`
is part of every sitting's close-out from here on**.
