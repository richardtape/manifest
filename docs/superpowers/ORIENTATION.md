...
│   └── 2026-09-18-p5c-the-clients.md              P5c ← CURRENT, written and not yet executed
└── spikes/
...
**P5c IS EXECUTING. Sitting 1 is DONE (2026-09-18, 18 findings); sittings 2 to 9 have not run.**
...
2. **The `[M4][M5][M9]` correction block at the top of Task 2** — four paragraphs, and the
   before-measurement its own control is watched against. **Then Tasks 2 and 3 in full.**
...
| The control plane | **RUNNING on 7100, pid 53829** — *checked*. **It was REBUILT and RESTARTED at this sitting's close**, because `routing/caddy.ts` changed and the process serves from `dist/`, so a source change does not reach it. Through the edge, `curl -sS https://console.manifest.internal/v1/me` answers **`401 UNAUTHENTICATED`** — *checked*. **Do not start a second one**; kill it by PID, because `pkill -f 'control-plane/dist'` does NOT match it |
...
| Owed to Rich | **THREE things, all re-derived at this sitting's close and none worked from a list.** (1) **Seven dead networks and one dead volume**, regenerated by this sitting's `pnpm test:docker` exactly as §2 predicts — `bash scripts/dead-app-resources.sh` read *"7 network(s) and 1 volume(s) are dead"*, **the same seven and one P5b sitting 9 measured**, which is now a second independent confirmation that this is a FIXED SET the tier regenerates rather than a growing backlog. (2) **One LiteLLM orphan**, `p4b-probe-user` — one, not the 19 of P5b sitting 8, because the script re-derives every run. (3) **Ten new `local/*` app image layers**, which **NEITHER script covers and nothing tracks**: `boot-recover` ×2, `chem-labs` ×2, `fixture-rd` ×3, `redeploy-cp` ×2, `token-app` ×1. **Name the metric** — `docker images -q \| wc -l` says **73** and `docker system df` says **65**, and a bare number reads as a contradiction. Rich runs each script with `--apply`; an agent's classifier refuses `docker network rm`. `make verify`'s per-app line is the meter and reads **`containers=3 networks=8 volumes=3`** |
