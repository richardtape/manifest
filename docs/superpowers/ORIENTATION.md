...
**15 → 16**, an existing route on the wrong listener is moved. `pnpm test:docker` was OWED ONCE and RUN:
**194 in 31, unmoved**. **`make verify` 55** and **`make doctor` 19**, both unmoved.
...
**The four numbers you will check first, re-measured 2026-09-22 on this machine at the close of P6a sitting 11 — P6a's last. ONE OF THE FOUR MOVED — `pnpm test` 1604 → **1605**, in 119 files, for the routing fix its acceptance forced; `pnpm test:docker` **194** in 31, `make verify` **55** and `make doctor` **19** held:**
...
| `make doctor` | **19 checks, 0 failed, 0 warnings** — **unmoved by P6a sittings 3 to 11** (sitting 11's re-run AFTER its Docker tier, both cleanup scripts and a control-plane restart); up one in sitting 2, which REPLACED the single-alias check with a two-alias loop (net 0) and added *the production zone answers the public address, and the console does not*. That check asserts BOTH directions: one asserting only the production answer stays green while the console has moved to the public address. **It cannot tell you production is REACHABLE** — Task 2's control (a) measured two checks naming `127.0.0.3` staying green with the alias removed, because they assert what dnsmasq answers |
| `make verify` | **55 checks, 0 failed, 0 warnings** — **unmoved by P6a sitting 11**; up one in sitting 10 (*the registry's realm is the control plane's, for the service it mints*), three in sitting 2 for §12's second listener. **Straight after `make reset` it reads ONE red — the audit grant — until the control plane has migrated the empty database** (§4). **It reads `mf- containers=12 networks=4 volumes=8` today** (sitting 11's close, after the Docker tier and `dead-app-resources.sh --apply`) and `runtime routes currently applied: 0`, because the tier restarted the edge |
| `make demo-production` | **NEW in P6a sitting 11 — P6a's acceptance, green FOUR times**: fresh (after `pnpm test`), on the re-use path, from an `echo reset \| make reset` machine, and on the re-use path again on the final code. ~3 min, three phases each ending `every check passed`. It is step 11 of `scripts/offline-acceptance.sh` and the last step of `make ci-acceptance`. It leaves `launch-app` in production |
...
**The next job is §7e — WRITE P6b, *subsequent releases*, from P6a's record** (P6a — [`plans/2026-09-19-p6a-first-production-launch.md`](plans/2026-09-19-p6a-first-production-launch.md) — IS EXECUTED: all 19 tasks in the eleven agreed sittings, 2026-09-19 → 2026-09-22, and its acceptance `make demo-production` passes). **P6b is written after P6a executes, the pattern P5a, P5b and P5c used**, and its cut is [the P6 brief's §10](plans/2026-09-19-p6-brief.md). *This preamble states the job because §7e is the section a cold agent is sent to and the two must agree — it said "EXECUTE P6a's SITTING 7" through the whole of sitting 8 (P6a sitting 8, F13). Change BOTH, every sitting.* Each plan's own *What executing this plan found* is its record. The roadmap's ledger outranks this section on status.
...
### 7e. Write P6b — subsequent releases ← **START HERE**

**P6a IS EXECUTED** (2026-09-22). An application reaches production with every one of §13's
blocking items honestly met, `make demo-production` proves it headlessly, and a person has clicked
it. **Your job is to WRITE the next plan, P6b, not to execute anything.** Use
`superpowers:writing-plans`; the house style is [`plans/2026-08-30-p1-local-substrate.md`](plans/2026-08-30-p1-local-substrate.md),
and P6a is the nearest example of the shape this project now uses (a measurements Task 1 alone and
first, the acceptance alone and last, correction blocks, a sittings table Rich agrees).

**P6b's scope is fixed** by [the P6 brief's §10](plans/2026-09-19-p6-brief.md) — *self-serve
redeploys to production; `isSensitiveDiff` given its caller at last; the `diff_snapshot` with its
AI summary, security-aware and carrying the reviewer's verdict (R4d); re-escalation; and the
`auth.attributes` → IAM change request path.* **Demo:** a self-serve production redeploy, then a
sensitive change that re-escalates and is refused until an administrator approves it.

**READ, IN THIS ORDER:**

1. P6a's *What this plan does not build* — **the list is P6b's input**, and sitting 11 added three
   items to it.
2. P6a's sitting 11 record (the end of *What executing this plan found*): F3, F6, F7 and Rich's
   decision.
3. The P6 brief's §5 (R4's four sub-decisions; (d) is P6b's), §8 (traps) and §10.
4. §8 below — **three of P6a's spec actions are still Rich's**, and two of them are about P6b's
   subject.

**WHAT P6b INHERITS THAT IS NOT IN THE BRIEF:**

- **RICH'S DECISION, 2026-09-22 — the administrator sees the diff BEFORE deciding, as a STORED
  preview that the approval binds.** The approve request names the preview; the platform refuses if
  what it would record now differs. A preview READ alone was rejected, because the model-written
  summary can differ between preview and record. This is P6b's to build and it changes the approve
  contract (§22's API is versioned: an addition, not a break, if designed as one).
- **An approval is found by RELEASE, and §13 says it binds a DIGEST** (sitting 11, F3). The builder
  is reproducible — the same commit rebuilds to the same digest — so re-escalation must decide
...
- **`make demo-production` leaves `launch-app` launched**, and re-uses it on a second run — the
  starting state for P6b's own demo. There is still **no route that deletes a project**.

**THE THINGS MOST LIKELY TO COST YOU:**

- **GREP BEFORE YOU PLAN TO BUILD.** Four of P6a's eleven sittings found the plan naming something
  that already existed or resting on a false premise — sitting 11's was Decision 11's *"a new build
  is a new digest"*.
- **A CONTROL THAT STAYS GREEN IS A QUESTION, NOT A RESULT.** P6a's worst defect (F6) was found only
  because control (e)'s green was chased to its cause. Write every control's predicted result, and
  plan the time to chase the ones that do not fire.
- **THE CONSOLE IS WHERE THE DEFECTS A GATE CANNOT SEE LIVE** — for the third plan running. Plan a
  clicked half, and ask Rich before the acceptance sitting whether he will click it.
- **Do not execute anything while writing a plan.** Measurements belong in the plan's Task 1.

**THE MACHINE, AS P6a LEFT IT — every number queried at the close of sitting 11, not recalled.**
**`make doctor` 19/0 and `make verify` 55/0**, re-run at the close after the Docker tier, both
cleanup scripts and a control-plane restart. **`pnpm test` 1605 passed, 0 skipped, in 119 files**,
twice alone and identical; **`pnpm test:docker` 194 in 31**; `scripts/ci-acceptance.sh` reads
`1605 / 119 / 19 / 55`. **21 migrations.** **THE DATABASE IS EMPTY** — every table 0, the Docker tier
truncated it — while **the containers outlive the rows**: `make verify` reads `mf-
