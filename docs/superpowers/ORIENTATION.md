...
**The four numbers you will check first, re-measured 2026-09-22 on this machine at the open AND the close of P6b sitting 1 — the measurements. NONE OF THE FOUR MOVED: `pnpm test` **1605** in 119 files (twice each time), `make doctor` **19**, `make verify` **55**; `pnpm test:docker` **194** in 31 was not owed and not run, and stands from P6a sitting 11 — the last sitting that moved anything (`pnpm test` 1604 → 1605, for the routing fix its acceptance forced):**

| `pnpm test` (from the **repo root**) | **1605 passed, 0 skipped, 119 files**, ~130 s — **up ONE in P6a sitting 11**: `routing/routes.test.ts` **15 → 16**, *moves an existing route to the RIGHT listener when it is on the wrong one* (F6). Run twice alone and identical after each of the sitting's code commits. `scripts/ci-acceptance.sh`'s `EXPECT_TESTS` is **1605**. **Never run a second Vitest process beside it** — the shared global setup deletes the test repositories and truncates the tables in both (§4). **Each earlier sitting's movement is in its plan record**, dated; this row states only the latest. It is the `unit` project and `packages` (the client, the journey, the console and the mock, none of which need anything running). No Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/`, `releases/`, `tokens/` and `launch/` suites, plus `spec/injection-drift`, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` (§3) |
| `pnpm test:docker` | **194 passed, 0 SKIPPED**, 31 files, **1394 s** on its last run (sitting 10's were ~870 s) — **OWED ONCE by P6a sitting 11** (`routing/` and `launch/` changed), run once after all four commits, and **unmoved, as predicted**. Each earlier sitting's figure is in its plan record. Not every sitting owes the tier — P5c's 4 to 7 and P6a's 1 neither ran nor owed it, so **do not spend 14 minutes on it out of habit**; the plan's own sittings rule says who owes it. `boot.docker.test.ts`'s *expires a question nobody answered* is still the ONLY test in the repository that fails if `src/index.ts` stops calling the expiry sweeper. The tier still drives SESSIONS: `grep` over every `*.docker.test.ts` finds no `Authorization: Bearer` that is a Manifest delegated token. **Running it costs the machine**: it restarts the edge (dropping every runtime route, so every app hostname then answers the WILDCARD), truncates the tables, re-registers the platform's SP row at a loopback ACS — **restart the control plane afterwards** — and regenerates exactly seven dead app networks and one volume. Needs `make up`, and **fails rather than skips** when asked to run |
| `make doctor` | **19 checks, 0 failed, 0 warnings** — **unmoved by P6a sittings 3 to 11 and P6b sitting 1** (sitting 11's re-run AFTER its Docker tier, both cleanup scripts and a control-plane restart); up one in sitting 2, which REPLACED the single-alias check with a two-alias loop (net 0) and added *the production zone answers the public address, and the console does not*. That check asserts BOTH directions: one asserting only the production answer stays green while the console has moved to the public address. **It cannot tell you production is REACHABLE** — Task 2's control (a) measured two checks naming `127.0.0.3` staying green with the alias removed, because they assert what dnsmasq answers |
| `make verify` | **55 checks, 0 failed, 0 warnings** — **unmoved by P6a sitting 11 and P6b sitting 1**; up one in sitting 10 (*the registry's realm is the control plane's, for the service it mints*), three in sitting 2 for §12's second listener. **Straight after `make reset` it reads ONE red — the audit grant — until the control plane has migrated the empty database** (§4). **It reads `mf- containers=12 networks=4 volumes=8` today** (P6b sitting 1's close) and `runtime routes currently applied: 1` — `launch-app`'s staging route; **that meter counts `srv0` only**, so it never sees the production route `srv1` holds beside it (P6b sitting 1, F13). It read `0` at P6a sitting 11's close because the Docker tier had restarted the edge |
...
### 7e. Execute P6b's sitting 2 — Tasks 2 and 3 ← **START HERE**
...
naming it, and an added CWL attribute also waits for UBC IAM. **Sitting 1 — the measurements — ran on
2026-09-22**: every premise the plan was written against measured true, all five controls fired, and it found
a defect nobody knew — **the egress proxy never re-renders its allowlist** (§4) — which added **Task 5a** to
sitting 3. **Your job is sitting 2: Task 2 (the person-only class) and Task 3 (the sensitive diff over frozen
releases).** Use `superpowers:executing-plans` and **commit on `main`, one commit per task** (§6 rule 9).
**`pnpm test:docker` IS OWED** (`projects/`, `spec/`, `releases/`) — budget ~23 minutes for it after both
commits, and restart the control plane afterwards.

**READ, IN THIS ORDER:** the plan's sittings table (the *Status* column says what sitting 1 made true);
**sitting 1's record** at the end of *What executing this plan found* — three of its findings land in
your two tasks (F7, F8, F12); then **Task 2 and Task 3 in full, each starting with the correction block at its
top**. The raw measurements are in [`spikes/p6b-baseline/`](spikes/p6b-baseline/README.md).

**WHAT SITTING 1 CHANGED IN YOUR TWO TASKS — READ THE BLOCKS, THIS IS ONLY THE LIST:**

- **Task 2** — its premises measured true (`[M13]`: both person-only capabilities mint `201` today and only
  `requireSession`'s `TOKEN_CREDENTIAL_REFUSED` stops their use). **One addition:** the comment above
  `CONTRACT_VERSION` says *"every one of its 34 operations"* — take the number out when you add the bump's
  sentence (F12).
- **Task 3** — **it now CREATES `commitManifest` in `api/testing.ts`** (its route test calls it, and the
  plan had it arriving in Task 4 — F8); `api/testing.ts` joins its *Files* and its commit. **And it gains
  one fix and one test**: the validate route answers `{sensitive: false}` whenever the previous spec row is
  invalid, without comparing anything (F7) — compare with the newest VALID row. **`[M15]` predicts Step 5
  turns NO existing test red**; a red one is a finding.

**THE THINGS MOST LIKELY TO COST YOU:**

- **Your first test run is the baseline, so run the gates before touching code**: `pnpm test` twice (1605 in
  119), `pnpm lint`, `pnpm typecheck`, `pnpm format:check`, `make doctor` (19) and `make verify` (55). A
  different number is signal before it is your fault.
- **A route change is three files, in order** — the definition, `pnpm contract:write`, `pnpm contract:generate`
  — and Task 2's bump to `1.1.0` touches `document.ts` AND `packages/contract/package.json`, which a test holds
  equal. **`pnpm contract:write` truncates the tables too.**
- **EVERY REFUSAL ASSERTS ITS CODE** — Task 2 exists to give the person-only rule a code no other layer answers
  with, and its control (c) is the one that proves it: status alone is `403` either way.
- **Two Vitest processes cannot run at once** (§4). The Docker tier after both commits, alone.
- **If a snippet defines `local path=…`, it breaks in zsh** (§4, F9). Run the plan's bash under `bash`.

**THE MACHINE, AS SITTING 1 LEFT IT — query every one of these again; do not work from this line.**
**`make doctor` 19/0**, **`make verify` 55/0**, **`pnpm test` 1605 in 119** (twice), `pnpm test:docker` **194 in 31**
(P6a sitting 11's; not owed in sitting 1), **21 migrations**, **the database EMPTY**, **nothing listening on 7100,
7102 or 7104**, all three loopback aliases on `lo0`. `launch-app` stands in production and staging on R2 (containers
only — no rows hold it), `click-launch` beside it; `make verify` reads `mf- containers=12 networks=4 volumes=8` and
`runtime routes currently applied: 1` (§4: that meter counts `srv0` only). **Docker Desktop may not be running when
