...
- **Seven app networks from dead test apps are still there, and nobody has owned them** — `mf-blueprint-ntm-staging-net`, `mf-chem-labs-staging-net`, `mf-fixture-rt-staging-net`, `mf-fixture-s6-staging-net`, `mf-fixture-s6nb-staging-net`, `mf-saml-probe-staging-net` and `mf-saml-unsigned-staging-net`, found while clearing the above. Their images are gone and no app container is on them, but `manifest-caddy` and `manifest-dns-containers` are attached to each by design (`ensureAppNetwork`), so each must have both disconnected before `docker network rm`. **Check each for a STOPPED container first**: a network removed under one leaves that container unable to start (§4). Not removed, because they are outside what sitting 12 listed and nothing has measured them.
