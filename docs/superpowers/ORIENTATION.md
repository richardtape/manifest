...
| **writing a plan** | **THE NEXT JOB: D5's GitHub source driver** (Rich's order, 2026-09-22: P6b → GitHub → the authoring API) — **P6b IS EXECUTED, so it is written now; §7e says how.** House style: [`plans/2026-08-30-p1-local-substrate.md`](plans/2026-08-30-p1-local-substrate.md), or the newest, [`plans/2026-09-22-p6b-subsequent-releases.md`](plans/2026-09-22-p6b-subsequent-releases.md). |
...
### 7e. Write the plan for D5's GitHub source driver ← **START HERE**

**P6b IS EXECUTED** (2026-09-23, [`plans/2026-09-22-p6b-subsequent-releases.md`](plans/2026-09-22-p6b-subsequent-releases.md)):
a launched app's release goes to production self-serve unless it changes a sensitive field, and then it waits for an
...
driver*** (GitHub App custody, per-repository short-lived installation tokens, HMAC-verified webhooks, repositories
private by default, push-time secret scanning on both drivers, a sandbox credential that can push to one branch and never
`main`); **P6b's *What this plan does not build*** — the inheritance list, including its sitting-7 findings; the
[authoring API brief](plans/2026-09-19-authoring-api-brief.md) §6, because the plan after this one writes through
...
- **Plan-writing sessions have left the machine exactly as found, every time** — snapshot at the start and the end, and
  do not run the Docker tier or a demo unless a measurement needs one.
- **A plan is a hypothesis** — every executed plan's own control predictions were wrong somewhere; P6b's acceptance
  alone found three (sitting 7, F6–F8). Put the measurements in a Task 1, alone and first.
- **Findings the next plan inherits** are in P6b's *What this plan does not build* — above all **F9, the model's
  summary inventing an administrator's verdict**, because a GitHub push is exactly the path that will put an unreviewed
  `manifest.yaml` change in front of that summary. **F10 — the console's *Sign out* leaving the IdP session alive — is
  NO LONGER INHERITED: it was fixed on 2026-09-24 in its own sitting** (`0aa1824`; P6b's record, *After the plan*),
  together with the defect fixing it found — Manifest's SLO route accepted UNSIGNED logout messages.
- **P6b's one spec action is APPLIED (2026-09-24)** — §13 D9.2 says an attribute *addition* waits on IAM and a removal does not, which is what P6b built (§8, *Decided*).

**THE MACHINE, AS THE F10 SITTING LEFT IT (2026-09-24) — query every one of these again; do not work from this line.**
**`make doctor` 19/0 with 0 warnings** (database built 2026-09-24 by the new `make refresh-vulndb`, stale after 2026-10-01), **`make verify` 55/0**,
**`pnpm test` 1742 in 123** (twice), **`pnpm test:docker` 200 in 31** (probe 14 on a warm re-run), **24 migrations**.
The tier truncated the tables, and **`make demo-releases` re-launched `launch-app` from nothing** — on its leg C release in
