...
**The next job is §7e — EXECUTE P5c's SITTING 1, the measurements. P5c was written on 2026-09-18, none of its 14 tasks has run, and nothing is part-finished. Nothing blocks it.** Each plan's own *What executing this plan found* is its record. The roadmap's ledger outranks this section on status.
...
### 7e. Execute P5c's sitting 1 — the measurements ← **START HERE**

**P5c IS WRITTEN AND NOT EXECUTED.** It was written on 2026-09-18
([`plans/2026-09-18-p5c-the-clients.md`](plans/2026-09-18-p5c-the-clients.md)): **14 tasks in
nine agreed sittings**, the last plan of Phase 1c. **No task has run. No code changed. No gate
moved.** Your job is **sitting 1 — Task 1 alone**, the measurements, using
...
1. **P5c's header, its sittings table, *Read this first*, *Decisions Rich made* and *Decisions
   this plan makes*** — in that order, and all five. *Read this first* has **twenty** items
   measured from the code on 2026-09-18, and **nine of them carry a `(T1: M<n>)` marker naming
   the measurement that re-checks them** — 1, 2, 3, 4, 9, 11, 12, 13 and 14; counted at close,
   not remembered. **Item 14 is the one to read twice**: a new package's tests are run by NO
   vitest project and are therefore silently green, which is why Task 2 exists at all and why
   its control is a deliberately failing test.
2. **Task 1 in full.** It has twelve steps and **ten measurements, M1 to M9 plus the baseline**.
   Do not start at Step 2.
3. **§4 of this file — searched, not read** — and **§6 IN FULL**, including the sweep table and
   the post-sweep check.
4. **The P5 brief's §10** only if something in P5c disagrees with itself; P5c was written from
   it and carries what it needed. **§2 and §4 of that brief are STALE and say so in their own
   headings.**

**What sitting 1 must produce**, and it is not code: `docs/superpowers/spikes/p5c-baseline/`
with one section per measurement — the command, the raw answer, and what it means for which
task — plus **a `[M<n>]` correction block at the top of any task a measurement contradicts**.
That is how P5b's sitting 1 moved five of its own tasks, and **a finding recorded only in the
spike file is a finding the next agent will not read**.

**THE ONE MEASUREMENT THAT IS RICH'S, NOT THE PLAN'S — M8.** §8's `stream_close_delay` question
has been open since 2026-09-16 and has never been measured. **Rich decided on 2026-09-18 (P5c's
R2) that the measurement settles it**: Task 1 Step 9 puts a runtime route in front of a
...
1. **Baseline first** (§6's *Your first ten minutes*): `./scripts/snapshot-machine.sh` to a
   ...
   `pnpm typecheck`, `pnpm format:check`. **Expect §2's box exactly** — it was last measured at
   the end of P5b sitting 9 and this sitting did not re-measure it, so a disagreement is the
   first finding of the plan and not noise.
2. **The control plane must be RUNNING for M1, M2, M6 and M8**, through the edge. README's
   *Running the control plane* is the export block; `pnpm test` truncates its tables, so run the
   gates **before** you need any rows.
3. **Read the four things above, in their order.**
4. **Run Task 1's twelve steps in order**, recording each raw answer as you go. **Steps 2, 3, 4
   and 9 temporarily change a running edge** — the Caddyfile's console line, and two admin-API
   routes — and **Step 11 puts every one of them back and proves it by re-reading**, not by
   having edited it back.
5. **Close out** (§6's sweep): the roadmap ledger first; then this §7e, which becomes *execute
...
- **`make reset` is not part of this sitting and should not be run.** If the machine was reset
  for some other reason, run `make verify` BEFORE any demo: the host can lose the edge while a
  container still has it, intermittently, and the remedy is `docker restart manifest-caddy`
  (P5b sitting 9, F6).
- **Step 9 writes two routes into the RUNNING edge by hand.** That is what the platform does on
  every deploy, but a route left behind is a change to a running edge: **read the route list
  back** rather than trusting the `DELETE`'s exit code.
- **`ws` resolves only from inside `packages/control-plane`**, so Steps 4 and 9's scratch scripts
  live there and Step 11 deletes them. `git status --short` at the end must be empty apart from
  the findings file.
...
**The state you are handed, 2026-09-18, after the sitting that WROTE P5c.** `main`, clean.
*This sitting ran no gates, no tests and no demos — it read code and wrote documents — so the
rows below are what P5b sitting 9 left, and only the four marked **checked** were re-read at
close.*

| | |
|---|---|
| The four gate numbers | §2's box, **measured at the end of P5b sitting 9 and NOT re-measured here**: `pnpm test` **1344** in 101 files, `pnpm test:docker` **178** in 29 files, `make doctor` **18/0**, `make verify` **51/0**. Task 1's Step 1 is where they are established again |
| The control plane | **RUNNING on 7100** — *checked* (`lsof -nP -iTCP:7100 -sTCP:LISTEN` → one `node`, pid as of close). Through the edge, `curl -sS https://console.manifest.internal/v1/me` answers **`401 UNAUTHENTICATED`** — *checked*. **Do not start a second one**; kill it by PID, because `pkill -f 'control-plane/dist'` does NOT match it |
| The console's origin | **Still the placeholder** — *checked*: `curl -sS https://console.manifest.internal/` answers `manifest console: not built yet — P5c serves it here. The API is under /v1/.` **P5c Task 4 is what replaces that line**, and Task 1 Step 2 changes it temporarily and puts it back |
| The database | **NO migration since 0018.** Whatever P5b sitting 9's gates left, which was `projects` **0**, `users` **0**, `routes` **0**, `delegated_tokens` **0**, `pending_actions` **0** — `pnpm test` truncates on every run, so **assume empty and measure if you need otherwise** |
...
| Owed to Rich | **Seven dead app networks and one volume**, and **LiteLLM's orphans**. `bash scripts/dead-app-resources.sh` and `bash scripts/litellm-orphans.sh` re-derive both; **an agent cannot apply either** — run them bare and hand the output over. **Neither stays cleared**: one `pnpm test:docker` puts the whole set back |
...
- **Should an APP's route carry `stream_close_delay`, as the console's does? — RAISED 2026-09-16 (P5a sitting 2).**
  ...
  and whether it belongs in a hardening slice or Phase 4. *Not measured:* an app WebSocket actually cut by another
  app's deploy. **RICH DECIDED ON 2026-09-18 HOW THIS IS SETTLED, AND THE ANSWER IS A MEASUREMENT** (P5c's R2):
  **P5c's Task 1, Step 9 (M8)** holds a socket open through a runtime route and causes an unrelated config reload,
  **with the field and without it**. If it is cut, P5c adds the field to `routing/caddy.ts`'s `buildRoute` and this
  item closes with the value; if it survives, it closes as *measured, and not a problem in Phase 1*. **It was NOT
  P5c that forced the question** — the console's own stream is already safe behind the console site's `stream_close_delay 1h`,
  and what the absence would break is an app holding its own socket, which no Phase 1 app does. **Still open until
