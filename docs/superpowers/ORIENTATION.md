...
### 7e. Execute P5c's sitting 3 — Task 4, alone ← **START HERE**

**P5c IS EXECUTING. Sittings 1 and 2 are DONE (2026-09-18, 19 and 10 findings); sittings 3 to 9
have not run.** Your job is **sitting 3 — Task 4, on its own**, using
`superpowers:executing-plans` or `superpowers:subagent-driven-development`. It replaces the
Caddyfile's placeholder with a `reverse_proxy` to 7104, writes the console's shell — the router,
the data layer `api.ts`, the refusal surface — and **ends with a person signing in with CWL in a
browser and seeing their own name** (§22 step 1).

**THE NETWORK-ON SITTING IS OVER.** Task 2 installed everything this plan gets. **If Task 4
believes it needs a package, that is a finding to record and raise, not a step to take** — the
plan says so in its Global Constraints. What you have: `react` 19.3.0, `react-dom` 19.3.0,
`vite` 8.3.0, `@vitejs/plugin-react` 6.1.1 and the two `@types` in `packages/console`; `ws`
8.21.3, `ajv` 8.20.0, `ajv-formats` 3.0.1 and `@types/ws` 8.18.1 in `packages/mock`. All exact.

**THIS IS THE SITTING THAT MAKES THE FOUR SHARED HTML PAGES LIE, AND NOTHING IN THE BUILD CHECKS
THEM.** `manifest-schematic.html` says *"no user interface has been built yet"* in two places and
`manifest-phases.html` once. All three are still true today, because nothing serves the console
yet — sittings 1 and 2 both checked and said so. **The moment you serve a real console at
`console.manifest.internal`, all three are false.** Sweep them in *your own* close-out. Rich
shares these pages outside the team.
...
1. **The plan — [`plans/2026-09-18-p5c-the-clients.md`](plans/2026-09-18-p5c-the-clients.md)** —
   ...
   `subscribe` in a browser, is sitting 4's — Task 6 — not yours.)
2. **The `[M1][M2][M6]` correction block at the top of Task 4 — FIVE paragraphs: four numbered
   points AND a final unnumbered one on editing the Caddyfile safely.** **Do not stop at the
   fourth**; the last is the bind-mount procedure, and it is the one a summariser drops (P5b
   sitting 4's defect, and P5c sitting 1's F17). **Then Task 4 in full.**
3. **Sitting 2's entry in the plan's *What executing this plan found*** — ten findings, and
   **F7 leaves you a job**: the import boundary's *"the scanner read no imports at all"* control
   currently asserts `react`, because nothing imported `@manifest/contract` yet. **Your `api.ts`
   is what changes that — tighten the assertion back to `@manifest/contract`**, which is the
   import D22 is actually about. There is a comment in the test saying so.
4. **[`spikes/p5c-baseline/README.md`](spikes/p5c-baseline/README.md)** — sitting 1's record.
   Read **M1**, **M2** and **M6** properly; they are the three that land on this sitting.
5. **§4 of this file — searched, not read — and §6 IN FULL**, including the sweep table and the
   post-sweep check.

**What sittings 1 and 2 measured that changes what you do.**

- **Everything this task assumes about the edge hop is MEASURED.** A host process on
  `127.0.0.1:7104` answers through the edge; `/v1/*` still reaches the control plane; a real
  browser's `Origin` arrives as `https://console.manifest.internal` unchanged; a WebSocket
  upgrade reaches 7104 with `Host` and path intact. **Deep paths arrive unchanged, so Decision
  3's real-path router is correct — but the edge does NO SPA fallback**, so whatever serves the
  console must do it (`vite preview` does).
- **`vite.config.ts` is already written and already carries what M1 and M3 found**:
  `allowedHosts: ['console.manifest.internal']` (the edge preserves the `Host` header and Vite
  rejects unknown hosts), and a real `hmr` block rather than the plan's `hmr: false` fallback,
  because an upgrade *does* survive the hop.
- **Run `pnpm --filter @manifest/contract build` before anything builds or previews the
  console.** The contract's `exports` map is conditional and the two conditions point at
  different trees — `"types": "./src/index.ts"` and `"default": "./dist/index.js"` — so the
  console **typechecks against source and bundles `dist/`**, and a stale `dist/` ships with every
  gate green.
- **`manifest_login` carries `Max-Age=600`.** A sign-in left sitting at the IdP for more than ten
  minutes loses its `returnTo` and lands on `/`. Invisible when an agent drives a form in two
  seconds; very visible in Task 14's shared run where Rich types the password.
- **An agent driving Chrome cannot type a password** (§4). This task ends in a browser: you
  drive and read every page, Rich types `student` / `student`. Say so when you get there.
...
1. **Baseline first** (§6's *Your first ten minutes*): `./scripts/snapshot-machine.sh` to a
   ...
   `pnpm lint`, `pnpm typecheck`, `pnpm format:check`. **Expect §2's box exactly.** A
   disagreement is your first finding, not noise.
2. **Start the control plane** — *this* sitting needs it, where sitting 2 did not. README's
   *Running the control plane* is the whole export block; `set -a; . ./.env; set +a` alone is
   NOT enough.
3. **Read the five things above, in their order.**
4. **Run Task 4.** The Caddyfile edit is the risky step: **edit it inode-preserving** (open
   `r+`, truncate, write — the agent's edit tool renames a new file over the old one and strands
   the mount), then `make up` and **re-read the bytes the edge actually serves** rather than
   trusting the edit. Commit.
5. **`pnpm test:docker` IS OWED by this sitting** — it changes `infra/caddy/Caddyfile`. ~13
   minutes, needs `make up`, fails rather than skips. **Restart the control plane afterwards**,
   and expect the per-app meter to go to `networks=8 volumes=3`: the tier regenerates exactly
   seven dead networks and one volume every run.
6. **Close out** (§6's sweep): the roadmap ledger first — **including its defect-rate table** —
   ...
   found*, **and the four HTML pages, which this sitting is the one to falsify.** Then **re-read
   your own §7e as a cold agent and CHECK its claims** by opening what they point at and counting.

**WHAT WILL SURPRISE YOU IN THIS SITTING, specifically.**

- **A `200` from `console.manifest.internal` may be the edge's wildcard, not your console.**
  Its body is `manifest OK host=… scheme=https`. **Read the body, never the status.**
- **A source swap does not reach a running control plane** — it serves from `dist/`. Any control
  that must be watched through a browser or a demo needs the process killed, rebuilt and
  restarted on the swap, then restored the same way.
...
- **`pgrep -f "vitest.*docker"` does not match a running Docker tier** — its argv is
  `node …/vitest 1`. Use `ps aux | grep [v]itest`, or wait for the harness's completion
  notification.
- **A negative control keyed on a whole suite's exit code borrows every flake in that suite**
  (sitting 2's F8). `src/api/delegation.test.ts > lets an owner go once somebody else owns the
  project` timed out at its 5 s bound once on a loaded machine, in a run that took 192 s against
  the usual ~110 s, and it passed on every other run that day. **Assert the shape of the answer**
  — grep the output for the thing that should or should not be there — rather than reading the
  exit code.
- **`${PIPESTATUS[0]}` is empty here**: the Bash tool's shell is zsh, which spells it
  `$pipestatus[1]` and indexes from 1. Take a status from the command, not from a pipeline.
...
**The state you are handed, 2026-09-18, at the close of P5c sitting 2.** *Written from `lsof`,
`docker`, `curl`, `git` and the scripts' own output at close, not from memory of the session.*

| | |
|---|---|
| The four gate numbers | §2's box. `pnpm test` **1348** in **102** files (up 3 and one file — the console's boundary test), `make doctor` **18/0**, `make verify` **51/0**, all re-measured at this sitting's close. **`pnpm test:docker` 178 in 29 files is CARRIED FORWARD from sitting 1 and was NOT re-run**, because sitting 2 touched no `routing/`, `infra/` or `*.docker.test.ts`. **Your sitting owes it**, because Task 4 edits the Caddyfile |
| The control plane | **NOTHING LISTENS ON 7100** — *checked by `lsof` at this sitting's close*. A sitting is one session and a control plane is a host process, so **check rather than believe this row**: `lsof -nP -iTCP:7100 -sTCP:LISTEN`. **Your sitting DOES need it** (Task 4 signs a person in through the edge), unlike sitting 2. README's *Running the control plane* is the whole export block — `set -a; . ./.env; set +a` alone is NOT enough, because `MANIFEST_ADMIN_DATABASE_URL` is DERIVED in that block and `db:migrate` then fails `[x] url: undefined`, naming neither the variable nor the file (P5b sitting 9, F7). **Never start a second one**, and kill the first BY PID — `pkill -f 'control-plane/dist'` does NOT match it |
| The console's origin | **Still the placeholder** — *checked at this sitting's baseline and unchanged since*: `curl -sS https://console.manifest.internal/` answers `manifest console: not built yet — P5c serves it here. The API is under /v1/.`, and `git diff infra/caddy/Caddyfile` is empty. **Task 4 — your sitting — is what replaces that line for good** |
| The two new packages | **`packages/console` and `packages/mock` exist and are committed** (`2f3731c`, `67275b4`). **Nine tracked files in the console and four in the mock** — *counted with `git ls-files`, not recalled*. The console has `package.json`, `index.html`, `vite.config.ts`, `tsconfig.json` and `src/{main.tsx,app.tsx,auth.ts,styles.css,boundary.test.ts}`; `app.tsx` is a placeholder with two sign-in buttons that Task 4 replaces with the shell, and `styles.css` is an 18-line placeholder Task 4 fills in — it exists only because `main.tsx` imports it and `vite build` will not resolve an import of a file that is not there. The mock has `src/{main.ts,server.ts}` and answers a deliberate `501` envelope on every path until Task 12. **`vitest.workspace.ts`'s `packages` project now names four packages**, and `pnpm typecheck` runs `tsc` for **five**, not three |
| The database | **NO migration since 0018** (`packages/control-plane/drizzle/0018_curvy_sister_grimm.sql` is the newest — *checked*; note the path, `drizzle/` is NOT at the repository root). **`pnpm test` ran FOURTEEN times in this sitting — counted from the captured output files — and it truncates on every run, as does a single-file `vitest run`**, so treat the §6 tables as EMPTY and re-read them if you need otherwise |
| Identity | **`users` is almost certainly EMPTY** for the reason above, so **the long-standing trap is ARMED**: `POST /v1/projects/{id}/members` answers `400 MEMBER_USER_NOT_FOUND` for anybody who has never signed in. There is also **no administrator** until a `make demo-journey` re-grants `operator`. **Task 4's own deliverable is a first sign-in**, which is what disarms it |
| The apps | **`token-app` only, exactly three containers** — *counted at close*. It is **NOT reachable as itself**: that hostname answers the edge's wildcard, which a status-only check cannot tell from the app. **Read the body, never the status** |
| Owed to Rich | **NOTHING.** Both scripts were run bare at this sitting's close and both read clear: `dead-app-resources.sh` **`none dead`** (0 networks, 0 volumes, token-app's one network and two volumes correctly KEPT) and `litellm-orphans.sh` **0 orphaned**, the one held user surviving. `make verify`'s per-app meter reads **`containers=3 networks=1 volumes=2`**. **The reason there is nothing owed is that this sitting ran no Docker tier** — the dead set is regenerated by `pnpm test:docker`, exactly the same seven networks and one volume every time, and **YOUR sitting runs one, so expect to hand a list over.** Run both scripts BARE at your close and give the output to Rich, who applies; an agent's classifier refuses `docker network rm`. **Never work from a list — the scripts re-derive.** **Neither covers app images**, of which **23** stand, only token-app's held by a container. **Name the metric**: they are tagged **`127.0.0.1:7107/local/*`**, so `docker images | grep '^local/'` answers **0** and reads as *none* — this sitting's post-sweep check made exactly that mistake before counting properly with `grep '/local/'` |
| This sitting's own footprint | **Two new packages, three config files, one plan and four status documents** — *counted from `git diff --stat`, not recalled*: `packages/console/` (9 files), `packages/mock/` (4), `vitest.workspace.ts`, `eslint.config.js`, `pnpm-lock.yaml`, the plan, and `ORIENTATION.md`, the roadmap, `README.md` and `RUNBOOK.md`. **No migration, no route, no spec change, no Caddyfile change, and no change to the control plane at all** |
