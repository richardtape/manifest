...
**Also waiting on Rich, and not Task 16's to decide:** P4b's three remaining spec actions (§8) — every task they waited for has now run.
...
- **P4b's three remaining spec actions — READY FOR RICH as of 2026-09-15, not applied.** Every task
  they waited for has now run, Task 14 last. Two are product calls: **§10's per-user AI budget**,
  which LiteLLM 1.98.0 cannot enforce and Phase 1 validates without applying; and **what streams
  over WebSocket** — §14 says *"build and deploy logs"*, D23.2 lists build logs, instance state
  transitions, incidents and approval decisions, and what sitting 9 built is D23.2's list less
  approvals (P6's), plus §9's SSO registrations and §10's key rotations, with no application
  stdout. The third is §10's agent-key row, which binds from Phase 3. The wording is in P4b's
