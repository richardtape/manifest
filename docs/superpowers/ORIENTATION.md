...
**Manifest's design is finished and EIGHT implementation plans are executed — P1, P2, P3, P4a, P4b, P4c, P5a (the contract) and P5b (delegated tokens), whose acceptance passed on 2026-09-18.** D24's whole loop runs end to end through the edge as `make demo-token` — a person mints a delegated token, an agent holding it builds and deploys a real application on its own authority, is refused the things it may not do, and gets past one of them only because a person confirmed that exact request, once. **It ran green three times, the third from a `make reset` machine, and is step 9 of the offline acceptance.** **P5c — the clients — IS WRITTEN AND NOW EXECUTING (2026-09-18): 14 tasks in nine agreed sittings, of which SITTINGS 1 TO 5 ARE DONE — the measurements (19 findings), the two new packages and the console's import boundary (10), the console SERVED and signing a person in (9), my projects, creating one and the project screen with its live stream (9), and both streaming screens (13). §22 STEPS 1 TO 6 ARE CLICKED: a person signs in with CWL, creates a project by typing a name checked as they type, watches its events arrive on a live socket, **builds it and reads the log lines as they are written, releases it, deploys it to staging watching the instance states arrive, and opens the running application and signs in to it with CWL**. The next job is its sitting 6 — Tasks 9 and 10, request-production and delegated tokens — and the next job is always §7e.** **The network-on sitting is over: nothing from here to the end of the plan may install a package.** Sitting 1 closed §8's `stream_close_delay` question, open since 2026-09-16: **an app's WebSocket IS cut by any other app's deploy**, so `buildRoute` now carries the field. This is the single entry point: what Manifest is, where things stand, how the platform is built, what the machine will do to you, how to work here, and what to do next. It is written for someone with **no prior context** — a new agent with a fresh window, or a developer joining.

*Last verified 2026-09-19 (P5c sitting 5, which ran on 2026-09-18 and closed out past midnight; **all four gate numbers UNCHANGED** — `pnpm test` **1355 in 103 files**, run twice at baseline and twice at close, with no new test file because Tasks 7 and 8 add none by Decision 7; `make doctor` 18/0 and `make verify` 51/0 re-run at close, with the per-app meter and the runtime-route count both back where the sitting found them; and **`pnpm test:docker` NOT run and NOT owed**, since both commits touch only `packages/console/src/` — though the sitting DID run real builds and deploys, which is Docker all the same, and removed every trace of them).* **Three places state current status — §2, §7e and §8 — and a sitting's sweep REPLACES what they say; it never appends a sitting's story here** (§6). **The roadmap's ledger outranks all three.** Everything else is durable.
...
**The four numbers you will check first, re-measured 2026-09-19 on this machine at the close of P5c sitting 5. NOTHING MOVED THIS SITTING — Tasks 7 and 8 add no test file, by Decision 7 (the console has no DOM test tier), so all four are the figures sitting 4 left:**
...
### 7e. Execute P5c's sitting 6 — Tasks 9 and 10 ← **START HERE**

**P5c IS EXECUTING. Sittings 1 to 5 are DONE (2026-09-18/19, with 19, 10, 9, 9 and 13
findings); sittings 6 to 9 have not run.** Your job is **sitting 6 — Tasks 9 AND 10, in that
order**, using `superpowers:executing-plans` or `superpowers:subagent-driven-development`.
Task 9 is *request production, and the fleet* — §13's `LaunchReadiness` with its blocked items
and why (§22 step 7), and §26's fleet, which is administrators only. Task 10 is *delegated
tokens*: minted once, listed, revoked (D24).

**THE HEAVY SITTING IS BEHIND YOU.** Sitting 5 carried both streaming screens, which is what
R1's lean split cost; yours is two smaller screens over routes that already exist. **Neither
...
1. **The plan — [`plans/2026-09-18-p5c-the-clients.md`](plans/2026-09-18-p5c-the-clients.md)** —
   ...
   this plan makes***. Of *Read this first*'s twenty numbered items, **1, 2 and 8 land hardest
   here** — *checked by opening them and counting, not by remembering*. Item 1 is the 34
   operations and the claim that a session can call every one of them, which **Task 9's
   `listFleet` is the first test of** (it is administrators only — a ROLE, not a second client).
   Item 2 is the `Idempotency-Key` header, required on `mintToken` and `revokeToken`, which are
...
3. **Sitting 5's entry in the plan's *What executing this plan found*** — thirteen findings.
   **F6, F7 and F11 change what you do.** F6: **`<Refusal>` now renders `launchReadiness`**, so
   ...
   second renderer, and note that `pendingAction` is still deliberately NOT rendered, because
   Task 11 is its first caller. F7: `createRelease` publishes no event, one of TWO routes now
   known to publish nothing — **check whether YOUR routes publish before assuming a screen can
   learn from the stream**; `mintToken` does publish `token.minted`, which sitting 4 proved live.
   F11: a control can be un-fireable because of the STATE you run it against, not because it is
   wrong.
4. **Sitting 4's entry**, for F1 (an array of objects rendered with `.join` is `[object Object]`,
...
- **`useProjectStream` EXISTS and is proved live** (`packages/console/src/stream.ts`), and the
  project screen already holds ONE socket that four panels consume. **Consume it; do not open a
  second.** Task 10's tokens screen is a project tab, so it has one available.
- **`api.ts` now has 24 of the contract's 34 operations** — *counted at close with
  `grep -c 'async '`*. Task 9 adds `getLaunchReadiness` and `listFleet`; Task 10 adds
  `mintToken`, `listTokens` and `revokeToken`. That leaves five for Task 11 and after.
- **`<Refusal>` renders `code`, `message`, `hint`, `details` AND `launchReadiness`** (sitting 5,
  F6). Task 9's screen and the production refusal must show the SAME checklist; they now share
  one renderer, so make the screen reuse it rather than restating the item list.
- **The mint response is the one place this API returns a credential, and the read schema has no
  `secret` field at all.** Show it once, and say so on the screen — a person who loses it mints
  another.
- **`pnpm --filter @manifest/contract build` before anything builds or previews the console**
  (sitting 1, F3), and **`vite build` after every commit** (sitting 2, F4).
...
1. **Baseline first** (§6's *Your first ten minutes*): `./scripts/snapshot-machine.sh` to a
   ...
   `pnpm lint`, `pnpm typecheck`, `pnpm format:check`. **Expect §2's box exactly**, including
   **`make verify`'s per-app INFO line reading `containers=3 networks=1 volumes=2`** and its
   **runtime routes reading 0**. A disagreement is your first finding.
2. **Start the control plane** — this sitting needs it. README's *Running the control plane* is
   the whole export block; `set -a; . ./.env; set +a` alone is NOT enough. Read its boot line.
   **`{"driver":"docker"}` matters less to you than to sitting 5** — you run no build and no
   deploy — but read it anyway, because a `fake` driver means something else is wrong.
...
5. **Create a project to work on.** The tables are EMPTY (see the state table). Sign in and
   click one up at `/` — a free re-test of Task 5 — or script one `POST /v1/projects`. **The
   `proof-app` starter is NOT needed this sitting**: nothing here builds or deploys, and §24's
   audience question is what Task 9's `large_course`/`public` readiness item keys on, so pick an
   audience deliberately. **Do it AFTER the baseline gates**, because every Vitest run destroys it.
6. **Task 9: write it, then CLICK IT, then run its controls, then the gates, then commit. Then
   Task 10, the same way.** The clicking and the controls come before `pnpm test` — the gates
   truncate the tables and take your project with them (sitting 4, F8). **If a control must run
   before a commit**, copy the files to your scratchpad with hashes and restore from that, never
   `git checkout`, which destroys uncommitted work (sitting 5 did this for Task 8's three).
7. **Neither task owes `pnpm test:docker`**, and unlike sitting 5 neither runs real Docker work
   either. **`GET /v1/fleet` is administrators only and there is no administrator** until
   `scripts/admin-grant.sh` is run — see the state table. That is Task 9's first obstacle, not a
   fault.
8. **Close out** (§6's sweep): the roadmap ledger first — **including its defect-rate table** —
   ...
   found*. **The four shared HTML pages were swept by sitting 3, checked by sitting 4 and checked
   again by sitting 5**, which changed what a person can DO (build, deploy, open the running app)
   without changing what those pages CLAIM. **Check them again**: Task 9 puts §13's launch
   checklist in front of a person, which those pages describe. Then **re-read your own §7e as a
...
- **THERE IS NO ADMINISTRATOR, and `GET /v1/fleet` answers everyone else `403` — not `404`.**
  A platform role is granted OUT OF BAND (`scripts/admin-grant.sh grant <puid> "<reason>"`, which
  runs one transaction as the database owner inside the Postgres container) **and reaches a
  person only when they SIGN IN AGAIN**, because sessions are stateless and carry the role they
  were issued with. The IdP's third test user, `operator` / `operator` (`opr000001`), is the one
  `make demo-journey` promotes. **`pnpm test` empties `users`, so the grant has to come after a
  sign-in and the sign-in after the truncation** — plan the order or you will grant a row that
  does not exist.
...
- **A token's secret is returned EXACTLY ONCE**, and the read schema has no `secret` field at all.
- **Only the MINTER may revoke a token**; everyone else gets the `404` an unknown id gets. A
  project owner cannot revoke a collaborator's token — that is D24, not a defect.
- **`pnpm test` TRUNCATES the tables and takes your clicked project with it** (sitting 4, F8).
...
- **The Chrome extension prints a synthetic `503` for a 204 whose page navigates away** (§4).
- **An agent driving Chrome cannot type a password** (§4, R3) — **but sittings 4 and 5 both
  needed none**, because Chrome held a live IdP session across sittings. **That is a property of
  one browser profile at one moment, not a rule**: plan for Rich to type it, and be pleased if
  the session is still there.
- **The permission classifier varies within a session** (§4) — and sitting 5 was ALLOWED both
  cleanup `--apply`s that §2 says an agent cannot run. **Do the step, then verify.**
...
**The state you are handed, 2026-09-19, at the close of P5c sitting 5.** *Written from `lsof`,
`docker`, `curl`, `git` and the scripts' own output at close, not from memory of the session.*

| | |
|---|---|
| The four gate numbers | §2's box. `pnpm test` **1355** in **103** files (UNCHANGED — Tasks 7 and 8 add no test file, by Decision 7; run twice at baseline and twice at close, 1355 every time), `pnpm test:docker` **178** in **29** files (**NOT re-run and NOT owed** — both commits touch only `packages/console/src/`), `make doctor` **18/0**, `make verify` **51/0**. **Your sitting owes no Docker tier either, and unlike sitting 5 runs no real Docker work** |
| The control plane | **RUNNING on 7100 when this was written — DO NOT BELIEVE THAT ROW, CHECK IT** with `lsof -nP -iTCP:7100 -sTCP:LISTEN`. A sitting is one session and this is a host process. **Your sitting DOES need it.** README's *Running the control plane* is the whole export block — `set -a; . ./.env; set +a` alone is NOT enough, because `MANIFEST_ADMIN_DATABASE_URL` is DERIVED in that block and `db:migrate` then fails `[x] url: undefined`, naming neither the variable nor the file. **Never start a second one**, and kill the first BY PID — `pkill -f 'control-plane/dist'` does NOT match it. It was restarted ONCE, at the start of this sitting, so its boot line could be read |
| The console | **`infra/caddy/Caddyfile` is untouched this sitting** — `git status` clean throughout. **Nothing was left listening on 7104** — *checked by `lsof` at close* — so `https://console.manifest.internal/` answers **502** until you start `vite`, and that is expected, not a fault. 7102 and 7105 are free |
| The console's code | **19 tracked files in `packages/console`** — *counted with `git ls-files`* — of which this sitting added `screens/builds.tsx` and `screens/deploy.tsx` and changed `api.ts`, `ui.tsx`, `app.tsx`'s sibling `screens/project.tsx`, and `styles.css`. **`api.ts` now has 24 of the contract's 34 operations** — *counted with `grep -c 'async '`*. `<Refusal>` renders `launchReadiness` as of this sitting and still does NOT render `pendingAction`, deliberately (Task 11 is its first caller) |
| The database | **NO migration since 0018** (`packages/control-plane/drizzle/0018_curvy_sister_grimm.sql` is the newest — *checked*; note the path, `drizzle/` is NOT at the repository root). **`pnpm test` ran FIVE times — counted from the captured output files** (`test1`, `test2`, `t7a`, `t8a`, `t8b`) — and every one truncates, so **treat the §6 tables as EMPTY** |
| Identity | **`users` is EMPTY** and the long-standing trap is ARMED: `POST /v1/projects/{id}/members` answers `400 MEMBER_USER_NOT_FOUND` for anybody who has never signed in. **There is NO administrator**, so `GET /v1/fleet` answers `403` for everyone until a grant is made — which Task 9 needs. The instructor signed in three times during this sitting and `pnpm test` ran afterwards |
| The apps | **`token-app` only, three containers** — *counted at close*. This sitting deployed an app and **removed every trace of it itself**; `make verify` reads `containers=3 networks=1 volumes=2` and *runtime routes* **0**. token-app is **NOT reachable as itself**: that hostname answers the edge's wildcard, which a status-only check cannot tell from the app |
| **Owed to Rich** | **NOTHING.** This sitting created five app containers, two networks, three volumes, four app images, two LiteLLM users, a bare repository and a runtime route, and removed all of them — **the classifier allowed both cleanup `--apply`s, which §2 says it refuses** (sitting 5, F12: try the command rather than trusting the note). Both scripts were re-run bare afterwards and read **`none dead`** and **0 orphaned**. App images stand at **27 by DISTINCT IMAGE ID**, unchanged — **name the metric, because three of them disagree**: `docker images \| grep -c '/local/'` answers **29** (it counts LINES), `scripts/snapshot-machine.sh`'s list **28**, and `docker images --format '{{.ID}}' \| sort -u` **27**; `docker images \| grep '^local/'` answers **0**, because they are tagged `127.0.0.1:7107/local/*`. **Never work from a list — run the scripts, which re-derive.** **The dead-resource set still comes back the moment anyone runs `pnpm test:docker`** — a property of the tier, measured three times, not a backlog |
| This sitting's own footprint | **Two code commits and status documents after them.** `4b464d1` — 4 files: `api.ts`, `screens/builds.tsx` (new), `screens/project.tsx`, `styles.css`. `abe9e72` — 6 files: `api.ts`, `screens/builds.tsx`, `screens/deploy.tsx` (new), `screens/project.tsx`, `styles.css`, `ui.tsx`. *Both counted with `git show --name-only`, not recalled.* **No migration, no route, no spec change, and no change to the control plane's `src/` at all** |
