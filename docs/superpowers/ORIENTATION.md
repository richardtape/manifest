...
1. **Task 7's correction block** — it is `[M5]`'s, and it is one paragraph: **the retry reuses
   the SAME `Idempotency-Key`** as the refused request, because D23.6's own error hint tells a
   client to reuse a key across retries of an action. Sitting 4 made that possible by catching
   the refusal **outside** `app.idempotent`; F3 of its record is the measurement, and the test
   that sees it is `delegation.test.ts`'s *stores NO idempotency record for a refusal*.
2. **P5b's *What executing this plan found*, sitting 4's entry** — 9 findings. **F1 is the one
