...
`scripts/ci-acceptance.sh` reads `1605 / 119 / 19 / 55`. **The P6b plan-writing session that followed (2026-09-22) changed no code and re-ran no gate, so these are still P6a sitting 11's.** **This line states only the latest sitting** — each sitting's numbers are
...
└── spikes/
    ...
    ├── p4c-, p5a-, p5b-, p5c-, p6a-baseline/   THE MEASUREMENT SITTINGS, one per plan since P4c (P6b's lands in its sitting 1).
...
### 7e. Execute P6b's sitting 1 — Task 1, the measurements ← **START HERE**

**P6b IS WRITTEN** (2026-09-22): [`plans/2026-09-22-p6b-subsequent-releases.md`](plans/2026-09-22-p6b-subsequent-releases.md),
eleven tasks. After an app's first launch, a release goes to production **self-serve**, unless it changes one of §7's
sensitive fields. Then it **re-escalates**: an administrator reads a **stored preview**, with a security-aware summary that
carries the reviewer's verdict, and approves by naming it. An added CWL attribute also waits for UBC IAM's change request
...
(the end of its *What executing this plan found*) is the state you inherit.

**WHAT THE PLAN FOUND WHILE BEING WRITTEN, which your measurements confirm or overturn.** Six premises it would otherwise have
built on were false in the code:

- **`[M3]`** `isSensitiveDiff` cannot see a raised `environments.production.resources`;
- **`[M5]`** the baseline counts a release that was approved and then rejected;
- **`[M6]`** a release freezes the project's NEWEST spec, not its build's, and accepts another project's build;
- **`[M7]`** a rehearsal after launch replaces live production with an unapproved release;
- **`[M8]`** the gate evaluates the candidate and deploys whatever release the request names;
- **`[M9]`** recording an IAM change request overwrites what UBC registered.

**A premise the measurement overturns gets an `[M<n>]` correction block at the top of the task it moves.**

**THE THINGS MOST LIKELY TO COST YOU:**

- **TAKE `[M1]` BEFORE ANYTHING RUNS VITEST.** `pnpm test` truncates the tables, which was P6a sitting 1's first finding.
  **From Step 7, when `make demo-production` puts `launch-app` into production, until Step 16, run no Vitest process at all**:
  nine measurements read that state.
- **`[M7]` DISTURBS PRODUCTION ON PURPOSE**, and it is last for that reason. Restore it exactly as Step 15 says, and leave
  `launch-app`'s repository with the same tree it started with (`git diff` against the original prints nothing).
- **The two probe tests are created and DELETED in their own steps.** `git status` at the close must show only the
  spike directory and the plan.
- **A CONTROL THAT STAYS GREEN IS A QUESTION, NOT A RESULT** (P6a F6). Task 1's controls are the positive halves of its
  measurements; the plan's table names all five.

**THE MACHINE, AS P6a LEFT IT — nothing was changed by the plan-writing session, which ran no gate and no container.**
At P6a sitting 11's close: **`make doctor` 19/0**, **`make verify` 55/0**, **`pnpm test` 1605 in 119**, **`pnpm test:docker` 194 in 31**,
**21 migrations**, **the database EMPTY**, `launch-app` and `click-launch` containers standing in production and staging,
**nothing listening on 7100, 7102 or 7104**, and both loopback aliases on `lo0`. **Query every one of these again**; do not
work from this line. **Docker Desktop may not be running when you open** (`open -a Docker`, §4).
