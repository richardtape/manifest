...
│   └── 2026-09-18-p5c-the-clients.md              P5c ← CURRENT, executing: sitting 1 of 9 done
└── spikes/
...
**The next job is §7e — EXECUTE P5c's SITTING 2, Tasks 2 and 3. P5c's sitting 1 (the measurements) ran on 2026-09-18 with 19 findings; Task 1 of its 14 is done, nothing is part-finished, and nothing blocks the next sitting.** **Sitting 2 is the ONE sitting in the plan with the network on**, so what it fails to install, no later sitting may add. Each plan's own *What executing this plan found* is its record. The roadmap's ledger outranks this section on status.
...
### 7e. Execute P5c's sitting 2 — Tasks 2 and 3 ← **START HERE**

**P5c IS EXECUTING. Sitting 1 is DONE (2026-09-18, 19 findings); sittings 2 to 9 have not run.**
Your job is **sitting 2 — Tasks 2 and 3 together**, using `superpowers:executing-plans` or
`superpowers:subagent-driven-development`. Task 2 makes `packages/console` and `packages/mock`
exist and all four gates see them; Task 3 is the console's import boundary, watched failing
before a single screen exists.

**THIS IS THE ONE SITTING IN THE WHOLE PLAN WITH THE NETWORK ON.** Nothing after Task 2 installs
a package. If a later task believes it needs one, that is a finding to record and raise — so
**anything you fail to install here, no later sitting may add.** Read the next paragraph before
you run `pnpm add` even once.

**THE DEPENDENCY SITTING 1 FOUND MISSING FROM THE PLAN, AND THIS IS YOUR ONLY CHANCE AT IT.**
Decision 9 names `ajv`. It does **not** name **`ajv-formats`**, and the document carries **185 format
assertions** — `uuid` ×141, `date-time` ×43, `uri` ×1 (counted across the whole document; **158** of them sit inside `components.schemas`, where `uuid` is ×114 — the two scopes give different totals, so state which one you mean). **Ajv v8 implements none of them itself**:
under `strict: true` an unknown format *throws*, and otherwise **the assertion is silently
ignored**, which is the dangerous one, because Task 12's fixtures would then validate 185 fewer
things than they appear to. **Install both, and pin 8.x deliberately** — `ajv@6.15.0` is also on
this machine (ESLint's, draft-07) and is the exact failure Decision 9 warns reads like a
malformed document. Both are already in the pnpm store transitively but are declared by **no**
`package.json`, and pnpm's strict `node_modules` means `packages/mock` cannot resolve a
...
1. **P5c's header, its sittings table, *Read this first*, *Decisions Rich made* and *Decisions
   this plan makes*** — all five. *Read this first* has twenty items measured from the code;
   **nine carry a `(T1: M<n>)` marker and sitting 1 re-checked all nine — every one held**, so
   they are now measurements rather than readings.
2. **The `[M4][M5][M9]` correction block at the top of Task 2** — **FIVE paragraphs: four
   numbered points AND a final unnumbered one** carrying the before-measurement this sitting's
   own control is watched against. **Do not stop at the fourth**; the last one is the one a
   summariser drops (P5b sitting 4's defect, and P5c sitting 1's F17). **Then Tasks 2 and 3 in
   full.**
3. **[`spikes/p5c-baseline/README.md`](spikes/p5c-baseline/README.md)** — sitting 1's record.
   Read **M4** and **M5** properly; they are the two that land on this sitting.
4. **§4 of this file — searched, not read — and §6 IN FULL**, including the sweep table and the
   post-sweep check.

**What sitting 1 measured that changes what you do.** *All of it is in the Task 2 correction
block; this is the short list so you cannot miss it.*

- **`pnpm test` does NOT see a new package, and says nothing.** With a probe package containing
  `expect(1).toBe(2)`, `pnpm test` exited **0** at **1344 passed in 101 files — identical to the
  baseline**, and the file was never collected (`grep -c` of the output: **0**). `pnpm lint` and
  `pnpm format:check` see a new package for free; **`pnpm typecheck` discovers it and silently
  skips it** for want of a `typecheck` script (`Scope: 3 of 4` became `Scope: 4 of 5`, and the
  same three packages ran `tsc`). **So both new packages need a `typecheck` script AND a glob in
  `vitest.workspace.ts`** — that is Task 2's actual deliverable, and the numbers above are the
  before-measurement its deliberately-failing-test control gets watched against.
- **Vite needs `server.allowedHosts`, and does NOT need `server.hmr: false`.** The edge
  **preserves `Host: console.manifest.internal`** rather than rewriting it, and Vite rejects
  unknown hosts. The plan's HMR fallback branch is **not** taken: an upgrade reaches a host
  process on 7104 through the edge, measured from a Node client *and* a real browser, with the
  path intact.
- **Run `pnpm --filter @manifest/contract build` before anything builds the console.** The
  contract's `exports` map is conditional and the two conditions point at different trees —
  `"types": "./src/index.ts"` and `"default": "./dist/index.js"` — so the console **typechecks
  against source and bundles `dist/`**, and a stale `dist/` ships with every gate green.
- **`pnpm-lock.yaml` WILL change, and here that is legitimate** and belongs in the commit.
  (In sitting 1 the same edit was an accident of the probe and was reverted.)
- **Decision 2 says take React and Vite CURRENT at install, not a remembered pin.** Record the
  exact versions and the `pnpm audit` of the closure in the plan's record — §6 rule 4.
...
1. **Baseline first** (§6's *Your first ten minutes*): `./scripts/snapshot-machine.sh` to a
   scratch file, `make up`, `make doctor && make verify`, `pnpm test` **twice**, then
   `pnpm lint`, `pnpm typecheck`, `pnpm format:check`. **Expect §2's box exactly** — it was
   re-measured at the end of this sitting. A disagreement is your first finding, not noise.
2. **Then, and only then, turn to the network** for Task 2's installs.
3. **Read the four things above, in their order.**
4. **Run Task 2, commit, then Task 3, commit.** Task 3's boundary test is the one to copy
   carefully: **`packages/journey/src/boundary.test.ts`, NOT the one P5a Task 7 describes** — it
   strips comments with a scanner, asserts what it still FOUND so an over-eager stripper cannot
   pass with an empty violation list, and counts the files it read. The naive version **matched
   English prose in a doc comment and turned `pnpm test` red on a file with no forbidden import**
   (P5b sitting 8, F3).
5. **Close out** (§6's sweep): the roadmap ledger first — **including its defect-rate table,
   which now has a P5c row** — then this §7e, §2's numbers box **if any gate moved**, the plan's
   sittings table and its *What executing this plan found*. Then **re-read your own §7e as a cold
   agent and CHECK its claims** by opening what they point at and counting.

**WHAT WILL SURPRISE YOU IN THIS SITTING, specifically.**

- **`pnpm test:docker` is NOT owed by this sitting.** The console and the mock are new packages
  the control plane does not import, and this sitting touches no `routing/`, `infra/` or
  `*.docker.test.ts`. The plan says so in terms: only sittings 3 (the Caddyfile) and 1 owe it.
  **Do not spend 13 minutes on it out of habit.**
- **`pgrep -f "vitest.*docker"` does not match a running tier** — its argv is `node …/vitest 1`,
  with no "docker" in it. Sitting 1 believed a finished-looking `pgrep` and **started a second
  concurrent `pnpm test:docker`**, two tiers sharing one database and one edge; both runs were
  discarded. **Use `ps aux | grep [v]itest`, or just wait for the harness's completion
  notification.**
- **A background command whose output you redirect yourself writes NOTHING to the harness's task
  file.** Sitting 1 read an empty task file and concluded a run had died when it had not.
- **The permission classifier varies within a session** (§4) and has refused read-only `docker`
  commands minutes after allowing destructive ones. **Do the step, then verify** — and if a
  verification is refused, the command's own output is the record.
- **`make reset` is not part of this sitting and should not be run.** If the machine was reset
  for another reason, run `make verify` BEFORE anything: the host can lose the edge while a
  container still has it, intermittently, and the remedy is `docker restart manifest-caddy`
  (P5b sitting 9, F6).
...
**The state you are handed, 2026-09-18, at the close of P5c sitting 1.** *Written from `psql`,
`docker`, `curl` and `lsof` at close, not from memory of the session.*

| | |
|---|---|
| The four gate numbers | §2's box, **re-measured at this sitting's close**: `pnpm test` **1345** in 101 files (up 1 — the `stream_close_delay` assertion), `pnpm test:docker` **178** in 29 files, `make doctor` **18/0**, `make verify` **51/0** |
| The control plane | **DO NOT ASSUME IT IS RUNNING, AND SITTING 2 DOES NOT NEED IT.** It was rebuilt and restarted at sitting 1's close (pid 53829 then) because `routing/caddy.ts` changed and the process serves from `dist/` — but **a sitting is one session, and a process this session started is a child of it.** One control plane HAS outlived its session here (sitting 1 inherited pid 14881 from the sitting before), so the honest statement is that it may or may not be there. **CHECK, do not believe this row:** `lsof -nP -iTCP:7100 -sTCP:LISTEN`. **If it is absent, that is fine for sitting 2** — Tasks 2 and 3 create packages, wire the four gates and write the import-boundary test, and the only server involved is the MOCK on 7102 that the sitting starts itself. `pnpm test` needs **Postgres**, which is a container `make up` gives you, not this process. **If you do need it** (sitting 3 onwards, once Task 4 signs a person in): README's *Running the control plane* is the whole export block — `set -a; . ./.env; set +a` alone is NOT enough, because `MANIFEST_ADMIN_DATABASE_URL` is DERIVED in that block and `db:migrate` then fails `[x] url: undefined`, naming neither the variable nor the file (P5b sitting 9, F7). **Never start a second one**, and kill the first BY PID — `pkill -f 'control-plane/dist'` does NOT match it |
| The console's origin | **Still the placeholder** — *checked*: `curl -sS https://console.manifest.internal/` answers `manifest console: not built yet — P5c serves it here. The API is under /v1/.` Sitting 1 replaced it temporarily for M1/M2/M3 and **put it back, proving it by re-reading the served bytes**; `git diff infra/caddy/Caddyfile` is empty. **Task 4 — sitting 3 — is what replaces that line for good, not this sitting** |
| The edge's runtime routes | **0** — *checked at close* (`GET /config/apps/http/servers/srv0/routes` returns **count 3, ids `(none), (none), (none)`** — the Caddyfile's own sites). Sitting 1's two probe routes were deleted and **read back** as `unknown object ID` |
| The database | **NO migration since 0018** (`drizzle/0018_curvy_sister_grimm.sql` is the newest — *checked*). **EMPTY, by `psql` at close**: `projects=0 users=0 routes=0 delegated_tokens=0 pending_actions=0` |
| Identity | **`users` is EMPTY**, so **the long-standing trap is ARMED**: `POST /v1/projects/{id}/members` answers `400 MEMBER_USER_NOT_FOUND` for anybody who has never signed in. There is also **no administrator** until a `make demo-journey` re-grants `operator`. Nothing in sitting 2 needs either |
| The apps | **`token-app` only, exactly three containers** — *counted at close* (`-app`, `-db`, `-egress`; mid-tier-run there was briefly a fourth, `mf-chem-labs-staging-egress`, and the tier cleaned it up). It is **NOT reachable as itself** — that hostname answers the edge's wildcard, which a status-only check cannot tell from the app. **Read the body, never the status** |
| Owed to Rich | **NOTHING that a script can clear.** Rich applied both at this sitting's close, 2026-09-18, and they were **re-measured clear afterwards rather than assumed**: `dead-app-resources.sh` reads **`none dead`** — 0 networks, 0 volumes, with token-app's one network and two volumes correctly KEPT — and `litellm-orphans.sh` reads **0 orphaned**, the one held user `mf-b81e4617-…-staging` surviving. `make verify`'s per-app meter reads **`containers=3 networks=1 volumes=2`**, the honest figure for one app. **NEITHER STAYS CLEAR, and this sitting measured the whole cycle inside itself**: clear at its baseline, then its own `pnpm test:docker` regenerated exactly the seven dead networks and one volume P5b sitting 9 measured — a second confirmation that this is a FIXED SET the Docker tier regenerates rather than a growing backlog — then clear again. Every demo adds a LiteLLM user besides. **So run both scripts BARE at your own close and hand the output to Rich, who applies**; an agent's classifier refuses `docker network rm`. **Never work from a list — the scripts re-derive.** **One thing neither script covers and nothing tracks: app images**, of which 23 `local/*` layers stand today, only token-app's held by a container. **Name the metric**, because the two disagree and a bare number reads as a contradiction: `docker images -q` piped to `wc -l` says **73**, `docker system df` says **65**. Not urgent — P5b sitting 8 cleared 86 that way, and found the previous sitting's written list had gone stale |
| This sitting's own footprint | **Two code files, one plan, one new spike directory, and four status documents** — *checked* (`git diff --stat`, counted rather than recalled): `routing/caddy.ts` and `routing/caddy.test.ts` (the `stream_close_delay` field and the assertion of its VALUE), the plan (five `[M<n>]` correction blocks, the sittings table, the record), the new `spikes/p5c-baseline/`, and `ORIENTATION.md`, the roadmap, `README.md` and `RUNBOOK.md`. **No migration, no route, no dependency, no spec change** |
