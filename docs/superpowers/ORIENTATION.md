...
P4a — and whose fifth, P4b, is PART-EXECUTED: sittings 1 to 7 of its ten are done. §16's proof
app signs a real person in with CWL and keeps their data theirs. CONTINUE P4b AT TASK 13
(sitting 8: §14's Incident — Docker-tier work, no network needed).** This is the single
...
*Last verified 2026-09-14.* Two things in this file state current status and will go
...
2026-09-09, with 10 findings, sitting 2 (Task 3) on 2026-09-14, with 6, sitting 3 (Tasks 4–5) the same day, with 11, sitting 4 (Tasks 6–7) the same day, with 12, sitting 5 (Tasks 8–9) the same day, with 11, sitting 6 (Task 10) the same day, with 14, and sitting 7 (Tasks 11–12) the same day, with 16. CONTINUE AT TASK 13**
...
| | |
|---|---|
| `pnpm test` (from the **repo root**) | **671 passed, 63 files**, ~35 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **131 tests, 24 files — 130 passed at the end of sitting 7; the one failure is P4b finding 133, a Mongo readiness race that fails under load**, ~440 s when green — needs `make up`, and **fails rather than skips** when asked to run |
...
The immediate work is **P4b, sitting 8 — Task 13, §14's `Incident`: Docker-tier work, no network needed** — see §7d-2's *Sitting 8 — what you are walking into*, and read the note from sitting 7 at the top of Task 13 first. P4a's fifteen tasks are done: the IdP
...
│                                                  plan). SITTINGS 1-7 (Tasks 1-12)
│                                                  ARE DONE, 2026-09-14.
│                                                  ← CONTINUE HERE, AT TASK 13. Records ten more measured
...
- **A Mongo service is HEALTHY before it accepts its own credentials.** The catalogue's health test is a loopback `ping`, and `mongodb/mongodb-community-server:7.0.28-ubi8` first runs an init `mongod` on `127.0.0.1` with no auth while it creates the user, then restarts with `--auth --bind_ip_all`. Measured 2026-09-14: healthy at 9.6 s, an authenticated insert from another container refused until 33.8 s. It widens under load — and each idle Mongo service's health check starts `mongosh` every second at about a core a run — so `services.docker.test.ts` fails on a busy machine and passes on an idle one. `getent hosts "$(hostname)"` answers `::1` first inside that image; `hostname -i` gives the container's IPv4 address. P4b findings 133 and 134; RUNBOOK's *Known gaps*.
...
## 7. What to do next — continue P4b, at Task 13
...
pin on 2026-09-09, sitting 2 ran Task 3, sitting 3 ran Tasks 4–5, sitting 4 ran Tasks 6–7, sitting 5 ran Tasks 8–9, sitting 6 ran Task 10 and sitting 7 ran Tasks 11–12, all on 2026-09-14**, so continue at Task 13. **P4b runs in ten agreed sittings**,
...
— **16 tasks, written 2026-09-07. Tasks 1–2 executed 2026-09-09 and Tasks 3–12 on 2026-09-14; continue at Task 13.**
...
| **8 ← next** | **13** | §14's `Incident` — **Docker-tier work, no network; FIRST the Mongo readiness race (finding 133); a note from sitting 7 at the top of Task 13; hand-off below** |
...
is at the top of Task 7. **Sitting 3 — Tasks 4 and 5 — is done too (2026-09-14), with 11 findings.** The two to carry forward: **Task 7's `ensureAiUser` recognises an existing LiteLLM user by text Task 5's client exists to strip, so as written every redeploy of an AI app throws** — catch on status 409 — and **Task 9's `createLiteLlmClient(config.litellm)` cannot typecheck** against the shapes Task 5 fixed. Both corrections are at the top of those tasks. **Sitting 4 — Tasks 6 and 7 — is done too (2026-09-14), with 12 findings**, and it made the boot decision its hand-off named: AI is on unless `MANIFEST_AI_ENABLED=0`. **Sitting 5 — Tasks 8 and 9 — is done too (2026-09-14), with 11 findings**: the gateway joins only an AI app's network, `deployRelease` mints before the instance starts and commits after health, and an AI release deployed with AI switched off is refused with `RELEASE_AI_DISABLED`. **Sitting 6 — Task 10 — is done too (2026-09-14), with 14 findings**: `node-ts-mongo@1` provides AI and its component is proved on the wire, and seed's npm warm — inert on any machine whose npm cache already held a package — is fixed. **Sitting 7 — Tasks 11 and 12 — is done too (2026-09-14), with 16 findings**: build logs are captured line by line, redacted and stored, and redaction has its heuristic half — and its Docker tier ended 130 of 131 on a Mongo readiness race that is not its code (finding 133). **Sitting 8 — Task 13 — is next, and its hand-off is immediately below.**

#### Sitting 8 — Task 13 — what you are walking into

**The state it was handed over in, 2026-09-14.** `main`, tree clean; last code commit `db570e7` (Task 12; Task 11 is `dec743f`). `pnpm test` **671** (63 files), `pnpm test:docker` **131 tests in 24 files, 130 passing** — `services.docker.test.ts` fails under load, and that is finding 133, below — `make doctor` **18/0**, `make verify` **47/0**. Migration `0005_loud_carlie_cooper` is applied — `audit.build_logs`, with `manifest_app=ar` — so a database that has not run it fails global setup on `audit.build_logs`; `db:migrate` fixes that. LiteLLM 1.98.0 (`sha256:20b5044b`) running. If your first measurements differ, find out why before you start.

**FIRST, BEFORE TASK 13: the Mongo readiness race (finding 133).** `ensureServiceContainer` returns when Docker calls a Mongo service healthy, and the catalogue's loopback `ping` health test passes during the image's unauthenticated init phase — measured, healthy at 9.6 s with authenticated writes refused until 33.8 s. Task 13 deploys apps that fail on purpose, so an app that fails for THIS reason will look like a correct Incident. Settle it first, test-first: a deterministic Docker-tier test with a 20 s init script copied into `/docker-entrypoint-initdb.d` before start, asserting an authenticated insert works at the moment the service first reports healthy — it goes red today. The candidate fix is a `CMD-SHELL` ping of the container's own IPv4 address, `"$(hostname -i | cut -d ' ' -f1)"`; it worked once and once never went healthy in 200 s, unexplained, so measure it before trusting it. Finding 134 — the check's CPU cost — is the same line of code. It is timing-dependent — the same file failed alone 2 of 2 and passed 2 of 2 twenty minutes later with nothing changed — so a rerun proves nothing; only the deterministic test does.

**No network needed, but the Docker tier is the task**: an `Incident` can only be proved against a container that has really failed. **The sitting ends with `pnpm test:docker`.**

**What sitting 7 built** (Tasks 11–12): `audit.build_logs` and `observability/build-logs.ts` — `appendBuildLog`, `readBuildLog`, `createBuildLogWriter`; `Driver.buildImage(src, spec, { onLog })`, which the contract suite requires to report before it resolves; `runStreamed` in `runtime/docker/builder.ts`; `startBuild` writing the log on both paths, its failure reason as the last line; `GET /builds/:buildId/logs`; `expectSqlState` in `observability/testing.ts`; and `makeRedactor`'s heuristic half — five patterns and Rich's entropy rule, on every string at every depth, including for an empty secret set.

**Three things from sitting 7 that touch Task 13** — the note at the top of Task 13 has all six:

1. **Its migration is `0006`, named by drizzle-kit, and its GRANT is appended through a glob.** Sitting 7's grant never reached its migration through `$(ls …)`, and the migration applied without it (finding 119).
2. **The redactor now redacts a URL password with no secret set at all**, so Task 13's printed redaction test — `mongodb://app:hunter2@…` — no longer proves that the app's own secret set reaches the Incident. Use a canary no heuristic matches.
3. **A single-connection transaction hides an ordering race** (finding 127). If Task 13 has to prove something is written before a status moves, prove it on the pool.

**Still not proved end to end, and not Task 13's to prove:** no app that declares models has been deployed and asked a question — Task 16's; and no client has received a build's lines as they happen — Tasks 14 and 15's.

**Read these before writing a line:** the note at the top of Task 13; the plan's *What executing this plan found*, sitting 7; and sitting 1's reconciliation — `audit.incidents` is `ON DELETE RESTRICT`, never `CASCADE`.

```bash
make up && make doctor && make verify                   # expect 18/0 and 47/0
pnpm test                                               # expect 671 passed, 63 files
MANIFEST_TEST_DOCKER=1 pnpm exec vitest run --project docker src/releases/deploy-sso
```

**Task 13 touches `releases/release.ts`, `observability/` and a migration, so the sitting ends with `pnpm test:docker` as well as the four gates** — then the record, the sittings table, and §6's sweep.
