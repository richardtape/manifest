...
*Last verified 2026-09-23, at the close of P6b sitting 7 — P6b's acceptance. `pnpm test` **1723 passed, 0 skipped, in
123 files** — unmoved: the acceptance added a journey entry point and no test — twice at the open and twice at the close,
identical. **`make doctor` 19 checks, 0 failed, 0 warnings** — the vulnerability database goes stale again after
2026-09-30. **`make verify` 55.** `pnpm test:docker` **198 in 31, all passed** at sitting 6's close; not owed in sitting 7
(no code under an owing path changed). `scripts/ci-acceptance.sh` reads `1723 / 123 / 19 / 55`. **This
line states only the latest sitting** — each sitting's numbers are in its plan's *What executing this plan found*, dated,
...
| `make verify` | **55 checks, 0 failed, 0 warnings** — **unmoved by all seven P6b sittings**; up one in P6a sitting 10, three in P6a sitting 2 for §12's second listener. **Straight after `make reset` it reads ONE red — the audit grant — until the control plane has migrated the empty database** (§4). **It reads `mf- containers=6 networks=2 volumes=4` today** — only `launch-app`, because P6b sitting 7 ran `make reset` for its third green — and `runtime routes currently applied: 1`. **That meter counts `srv0` only**, so it never sees a production route on `srv1` (P6b sitting 1, F13) |
...
- **THE CONSOLE'S *Sign out* DOES NOT SIGN YOU OUT OF CWL — so a SECOND PERSON CANNOT SIGN IN in the same browser** (P6b
  sitting 7, F10; pre-existing, not fixed). It ends Manifest's session and leaves the IdP's: the next *Sign in with CWL*
  silently returns the previous person, and neither the console's `GET /auth/logout` nor the IdP's `initSLO` (a 404)
  ends it. **To switch person in Chrome, delete the site data for `manifest.internal` in `chrome://settings/content/all`**
  (the extension cannot open `chrome://` pages — Rich does it). `idp_login` in bash is immune: one jar per identity.
...
- **Findings the next plan inherits** are in P6b's *What this plan does not build* — above all **F9, the model's
  ...
  `manifest.yaml` change in front of that summary, and **F10, the console's *Sign out* leaving the IdP session alive**.
...
**THE MACHINE, AS P6b SITTING 7 LEFT IT — query every one of these again; do not work from this line.** **`make
doctor` 19/0 with 0 warnings** (database built 2026-09-23, stale after 2026-09-30), **`make verify` 55/0**, **`pnpm
test` 1723 in 123** (twice), `pnpm test:docker` **198 in 31** at P6b sitting 6's close, **24 migrations**. **`make
reset` ran** for the acceptance's third green, so the only app is `launch-app` — launched, on its leg C release in staging
and production, the registration `active` five-wide — and the database holds 1 project and 4 releases. `make verify`
reads `mf- containers=6 networks=2 volumes=4`. **Nothing listens on 7100, 7102 or 7104**; both loopback aliases are on
`lo0`. Both cleanup scripts read nothing to do. **Images: `docker images -q` 182, `sort -u` 173,
`127.0.0.1:7107/local/*` 135** — up from 68 app images at the sitting's open, about twenty demo runs' worth, and no
script sweeps them. **Docker Desktop may not be running when you open** (`open -a Docker`, §4).
