...
(2026-09-16): 17 tasks in twelve agreed sittings. ITS SITTINGS 1 AND 2 ARE DONE (2026-09-16, 9 and 16 findings) — the measurements, then the API under `/v1` and through the edge at `https://console.manifest.internal`, refused to every source but the host — and the next job is EXECUTING SITTING 3 — §7e says what that is.** This is the single
...
**The four numbers you will check first, measured 2026-09-16 on this machine, at the end of P5a sitting 2:**

| | |
|---|---|
| `pnpm test` (from the **repo root**) | **835 passed, 73 files**, ~50 s, no Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/` and `releases/` suites — plus **`spec/injection-drift`**, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` — see §7d |
| `pnpm test:docker` | **168 passed, 0 SKIPPED**, 27 files, ~762 s — measured at the end of P5a sitting 2, whose S6 probe 15 is the 168th (the edge refuses an app the control plane, with the host as its control). The eleven skips that used to be here were the driver contract's continuity block, gone since P4c Task 5 gave the Docker driver its fixtures. Needs `make up`, and **fails rather than skips** when asked to run |
...
│                                                  SITTINGS 1–2 DONE, 3 NEXT. Its six spec
...
**P5a IS WRITTEN (2026-09-16) AND ITS SITTINGS 1 AND 2 ARE DONE, THE SAME DAY — YOUR TASK IS SITTING 3: TASK 4 (CSRF by `Origin` on every cookie-authenticated mutation and stream upgrade, a sign-in bound to the browser that started it, a safe return path), THEN TASK 5 (one registry of every error code a client can receive).**
...
**Read, in this order, before sitting 3:**

1. **The plan's** *How this plan is to be executed*, *Read this first*, *Decisions Rich made*, *Decisions this plan makes* (15–18 above all) and **sittings 1 and 2's entries in *What executing this plan found*** — then **Tasks 4 and 5 in full**, including the *Task 1 result* at the top of Task 4.
2. **[`plans/2026-09-16-p5-brief.md`](plans/2026-09-16-p5-brief.md)** §8 (the traps).
3. **This file's §4** — its last seven entries are sitting 2's — and **§6** (how to work, and the close-out sweep every sitting owes).

**How to execute it.** `superpowers:executing-plans` or `superpowers:subagent-driven-development`, **one sitting per
session**, with a check-in at each boundary. **Sitting 3 changes `api/`, `identity/`, `infra/lib/idp-login.sh`, the demos' helpers and
`identity/saml.docker.test.ts`**, so it ends with `pnpm test:docker` (~13 min) as well as the four gates. A Caddyfile edit reaches the edge through `make up`,
whose reload drops every runtime route, and the Docker tier restarts the edge and re-registers the platform's SP row at
a loopback ACS — **restart the control plane after either**; its boot puts both back. **Sitting 5 is the only one that
needs the network.**
...
- **Every demo already speaks `$ORIGIN` — `https://console.manifest.internal` — through ONE helper,
  `scripts/lib/api.sh`** (sitting 2). Task 4's `Origin` header goes in its `api`, once, and in
  `scripts/lib/event-stream.mjs`'s upgrade; the demos' watchers already run under `NODE_EXTRA_CA_CERTS`.
...
**The state you are handed, 2026-09-16, after sitting 2.** `main`, clean. The code commits are Task 2's `fad5e31`, Task 3's
`8520e0a`, and `92b4ebb` and `1c83bd3` after it; everything later is documentation. The four gate numbers are §2's box — all
green, `make verify` 50/0 and `pnpm test:docker` 168. **The proof app serves at `https://proof-app.staging.manifest.internal/`,
