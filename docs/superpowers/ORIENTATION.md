...
**The four numbers you will check first, re-measured 2026-09-23 on this machine at the open AND the close of P6b sitting 3 — Tasks 4, 5 and 5a. `pnpm test` MOVED, 1635 → **1651** and 121 → **122** files (twice after each commit, on a quiet machine); `make doctor` **19** with **0 warnings** since the database refresh; `make verify` **55**; `pnpm test:docker` MOVED, 194 → **198** in 31, owed and run, with two reds investigated — one fixed, one recorded:**

| `pnpm test` (from the **repo root**) | **1651 passed, 0 skipped, 122 files**, ~140–160 s on a quiet machine — **up 16 and one file in P6b sitting 3**: six in `releases/releases.test.ts` and one in `launch/rehearsal.test.ts` for Task 4, nine in `api/subsequent-releases.test.ts` (new) for Task 5. Run twice alone and identical after each of the sitting's code commits and at the close. **Read `uptime` before believing a red**: under a Zoom screen share (load 50–90) four runs read 17, 0, 2 and 7 red, every one a 5000 ms timeout (§4). `scripts/ci-acceptance.sh`'s `EXPECT_TESTS` is **1651**. **Never run a second Vitest process beside it** — the shared global setup deletes the test repositories and truncates the tables in both (§4). **Each earlier sitting's movement is in its plan record**, dated; this row states only the latest. It is the `unit` project and `packages` (the client, the journey, the console and the mock, none of which need anything running). No Docker needed except Postgres for the `db/`, `api/`, `secrets/`, `services/`, `sso/`, `observability/`, `releases/`, `tokens/` and `launch/` suites, plus `spec/injection-drift`, which reads the pinned `passport-ubcshib` tarball out of the platform's own mirror. It connects as **`manifest_app`**, not as `manifest` (§3) |
| `pnpm test:docker` | **198 tests, 0 SKIPPED, 31 files, 1933.7 s** on its last run — **P6b sitting 3's, owed and run under heavy load: 196 passed and TWO red, both investigated.** `releases/redeploy.docker.test.ts`'s request loop was being **rate-limited by the edge it tests** — 600 requests a minute per client IP, and it asked every 25 ms — so any redeploy slower than ~15 s read *"the last 200 was the old instance"*; latent since P4c, **fixed in `a7948fd`** and its file re-run 3/3 (sitting 3, F16). `runtime/docker/s6.docker.test.ts`'s probe 14 answered `000` because Ollama's `default-chat` model (`ministral-3`, 9.7 GB) was not resident; **warm, the file is 18/18** — recorded, not fixed (F17). +4 tests: the launched suite's rehearsal refusal and Task 5a's three egress cases. Each earlier sitting's figure is in its plan record. Not every sitting owes the tier — the plan's own sittings rule says who does, so **do not spend 30 minutes on it out of habit**. `boot.docker.test.ts`'s *expires a question nobody answered* is still the ONLY test in the repository that fails if `src/index.ts` stops calling the expiry sweeper. **Running it costs the machine**: it restarts the edge (dropping every runtime route, so every app hostname then answers the WILDCARD), truncates the tables, re-registers the platform's SP row at a loopback ACS — **restart the control plane afterwards** — and regenerates exactly seven dead app networks and one volume. Needs `make up`, and **fails rather than skips** when asked to run |
...
### 7e. Execute P6b's sitting 4 — Task 6, alone ← **START HERE**
...
naming it, and an added CWL attribute also waits for UBC IAM. **Sittings 1 to 3 are done**: the measurements
(2026-09-22), then Tasks 2 and 3, then Tasks 4, 5 and 5a (both 2026-09-23) — an app's launch is recorded once
by the deploy that makes it true (`projects.launched_at`, migration 0021) and a launched app is never
rehearsed; `deployRelease` reads ONE rule, `approvalRequirementFor`; and the egress proxy follows the release
it serves. **Your job is sitting 4: Task 6 — the gate for a launched app, this plan's centre, ALONE.** The
checklist branches on `launched`, a launched app's non-sensitive release goes through the ROUTE with no
administrator, a sensitive one is refused `409 RELEASE_REESCALATED` carrying exactly what changed, and a
release that is not the one serving staging is refused `409 RELEASE_NOT_STAGED`. **Until you do, the route
still refuses a launched app's self-serve release while `deployRelease` alone would allow it** — the
one-sitting window the plan predicted; nothing a client can do reaches it. Use `superpowers:executing-plans`
and **commit on `main`, one commit per task** (§6 rule 9). **`pnpm test:docker` IS OWED** (`launch/`) —
budget ~25–35 minutes for it after the commit, and restart the control plane afterwards only if it was
running when you opened.

**READ, IN THIS ORDER:** the plan's sittings table; **sitting 3's record** at the end of *What executing this
plan found* (F8 to F12 are about the plan's own predictions — four were wrong); then **Task 6 in full,
starting with the correction block at its top — sitting 3 left FIVE things in it.**

**WHAT SITTING 3 LEFT IN TASK 6 — READ THE BLOCK, THIS IS ONLY THE LIST:**

- **Two launch helpers, not one**, in `api/testing.ts`: `approvedProject` (approved, NOT launched) and
  `launchedProject` (that plus the production deploy); `api/subsequent-releases.test.ts` already has
  `nextRelease`, `decide`, `manifest`, `rowOf` and `instancesOf` for your route cases.
- **The `covered` branch of the plan's `releaseApprovalItem` prints "This release changes , and an
  administrator approved it"** for the state `make demo-production` leaves: redeploying the only approved
  release reads `no-baseline` with `fields: []` and is covered by its own approval (sitting 3, F9). Word it.
- **`make demo-production`'s re-use path is already a launched app that stops** — your Step 5's *green on
  re-use* is measured true. **The fresh path still runs step 10**, which is yours to remove.
- **Task 4's control (b) has a demo half that could NOT fail until this task** (F8): re-run it once after
  the checklist branches on `launched`, and predict the exact check.

**THE THINGS MOST LIKELY TO COST YOU:**

- **FIRST: is the machine quiet? Read `uptime` before you believe a red.** Sitting 3 lost an hour to a Zoom
  screen share (load 50–90): four `pnpm test` runs read 17, 0, 2 and 7 red, every one a 5000 ms timeout
  (F15). A red under that load is not a measurement — wait, or ask.
- **Is the vulnerability database still fresh?** It is until **2026-09-30** (§2 *Outstanding*). Past that,
  `make doctor` warns and §13's `scans` item refuses every production launch, so `make demo-production`
  goes red at step 3 — ask Rich to refresh it (network) before any step that runs the demo.
- **Your first test run is the baseline, so run the gates before touching code**: `pnpm test` twice (**1651
  ...
  your fault.
- **A new representation field or error code has more exact-set witnesses than the plan names** (sitting 3,
  F1): `projects.test.ts`'s key lists, `stream-contract.test.ts`'s event accounting, the mock's
  `validate.test.ts` and `error-codes.test.ts`. Predict them before your first full run.
- **Task 6 regenerates the contract** (`pnpm contract:write`, then `pnpm contract:generate`) and changes the
  mock's checklist fixtures and two console screens — `packages/console`'s coverage test and the mock's
  tests read the document.
- **A production deploy needs the candidate SERVING STAGING**, and a `pnpm test` between a staging deploy and
  a production one truncates the project (the plan's first trap). Order the sitting so no Vitest run sits
  between them.
- **Predict every control's reds by COUNT and by CODE before running it**; sitting 3 under-counted twice
  (F10, F12) and the plan was wrong about a demo half (F8). **Every refusal asserts its CODE** — `409` now
  means `RELEASE_PRODUCTION_GATE_UNAVAILABLE`, `RELEASE_REESCALATED` or `RELEASE_NOT_STAGED` on one route.
- **Two Vitest processes cannot run at once** (§4). **If a snippet defines `local path=…`, it breaks in zsh**
  (§4). Run the plan's bash under `bash`.

**THE MACHINE, AS SITTING 3 LEFT IT — query every one of these again; do not work from this line.**
**`make doctor` 19/0 with 0 warnings** (database 0.6 days old at the close), **`make verify` 55/0**,
**`pnpm test` 1651 in 122** (twice, quiet), `pnpm test:docker` **198 in 31** (196 passed; the redeploy test
fixed in `a7948fd` and re-run 3/3; S6 probe 14 recorded, 18/18 with Ollama's chat model warm), **22
migrations**, **the database EMPTY**, **nothing listening on 7100, 7102 or 7104**, all three loopback aliases
on `lo0`. `launch-app` stands in production on its launch release and in staging on a rebuild of its original
manifest — containers only: no rows hold them — with `click-launch` beside it; `make verify` reads `mf-
containers=12 networks=4 volumes=8`. **Task 6's `make demo-production` runs fresh after a `pnpm test`, which
rebuilds what it needs.** The control plane was stopped at sitting 3's open and close, and it re-registers the
platform's SP row at every boot. **Docker Desktop may not be running when you open** (`open -a Docker`, §4).
