...
**Five spikes are done — S7, S2, S1, S3 and S6 — and all five answered yes (§5); S5 and S4 are deliberately later. TEN plans are executed, and each has an acceptance that passes:**
...
| **writing a plan** | **Nothing is waiting to be written. P6b was WRITTEN on 2026-09-22; the next plan, D5's GitHub source driver, is written after P6b executes** (Rich's order, 2026-09-22: P6b → GitHub → the authoring API). House style: [`plans/2026-08-30-p1-local-substrate.md`](plans/2026-08-30-p1-local-substrate.md), or the newest, [`plans/2026-09-22-p6b-subsequent-releases.md`](plans/2026-09-22-p6b-subsequent-releases.md). |
...
│   ├── 2026-09-22-p6b-subsequent-releases.md      P6b ← CURRENT; its own sittings table says how far
...
### 7e. Execute P6b's sitting 7 — Task 11, the acceptance, alone and last ← **START HERE**

**P6b IS EXECUTING** ([`plans/2026-09-22-p6b-subsequent-releases.md`](plans/2026-09-22-p6b-subsequent-releases.md)).
After an app's first launch, a release goes to production **self-serve** unless it changes one of §7's
sensitive fields; then it **re-escalates** to an administrator, who reads a **stored preview** and approves by
naming it, and an added CWL attribute also waits for UBC IAM. **Sittings 1 to 6 are done** — the measurements
(2026-09-22), then Tasks 2 to 10 (2026-09-23). **Since sitting 6** an administrator takes a stored preview (`POST
/v1/releases/{id}/approval-preview`, no step-up) and approve and reject must NAME it — the record copies the preview,
so the model is asked once — and the console's approvals screen shows the preview BEFORE the decision, its id in
`?preview=` across the step-up. **Your job is sitting 7: Task 11 — `make demo-releases`, the acceptance.** A
self-serve production redeploy of `launch-app`; a sensitive change refused until an administrator, having read the
stored preview, approves it; and the IAM change request path — through the edge, by nothing but
`@manifest/contract`. **Green three times, the third from an `echo reset | make reset` machine**, plus offline-acceptance
step 12, a `ci-acceptance` step, **and a clicked half by a person.** **ASK RICH, BEFORE THE SITTING STARTS, WHETHER HE
WILL CLICK IT** — it needs him to type the operator's and the instructor's passwords and to step up more than once (the
extension will not type a password, §4). Use `superpowers:executing-plans` and **commit on `main`** (§6 rule 9).
**`pnpm test:docker` is owed only if code under the owing paths changes** — budget ~26 minutes if it does.

**READ, IN THIS ORDER:** the plan's sittings table; **Task 11's two blocks at its top — sitting 6's, then sitting 5's**
(what the preview changes for every approval, the stale path nobody has clicked, and leg C's walk to `active`); then
**sitting 6's record** at the end of *What executing this plan found* (F1, F3, F13 matter here); then Task 11 in full,
and Decision 17 (its three legs, their order, and why the fresh path runs `make demo-production` first).

**THE THINGS MOST LIKELY TO COST YOU:**

- **FIRST: is the machine quiet? Read `uptime` before you believe a red** (§4). And a red that appears once and not
  again, at a time comparison, is the Docker VM's clock before it is a flake (§4, sitting 6's F1).
- **Is the vulnerability database still fresh?** It is until **2026-09-30** (§2 *Outstanding*). Past that, §13's
  `scans` item refuses every production launch and `make demo-production` — which `make demo-releases`' fresh path
  runs first — goes red at step 3. Ask Rich to refresh it (network) before any step that runs a demo.
- **Your first test run is the baseline, so run the gates before touching code** — §2's box has the numbers.
- **EVERY APPROVAL NAMES A PREVIEW NOW.** In the journey, `packages/journey/src/production.ts` step 7 is the pattern
  (preview, approve with `previewId`, and assert the record's diff equals the preview's); in the unit tier,
  `previewThenDecide` (`api/testing.ts`). A decision without one is `400 APPROVAL_PREVIEW_REQUIRED` — and the mock
  refuses it too, since sitting 6.
- **THE STALE PATH HAS NEVER BEEN CLICKED** (sitting 6, F13) and **control 10b — the screen ignoring `?preview=` — is
  invisible to every gate** (F9): the clicked half is the first witness of both. Task 11's block says how.
- **TWO TRAPS §7e NAMED FIRED ANYWAY IN SITTING 6** (F8): an unset `$SCRATCH` in a redirect, and a control runner
  invoked from zsh with an unquoted `$F` — *"No test files found"*, which measures NOTHING. **Spell the paths out, run
  runners under `bash`, and read the summary line before you record a control.**
- **Predict every control's reds by COUNT and by CODE before running it.** Sitting 6's plan predictions were wrong
  twice (F2, F3), and both were caught only because the prediction was written first.
- **A Chrome click by `ref` may not dispatch** (§4): read the network before calling a button dead.
- **Two Vitest processes cannot run at once**, and **`pnpm test` and the Docker tier TRUNCATE the tables** — run a
  demo's reads before any Vitest run, and restart the control plane after the Docker tier.

**THE MACHINE, AS SITTING 6 LEFT IT — query every one of these again; do not work from this line.**
**`make doctor` 19/0 with 0 warnings** (database built 2026-09-23, stale after 2026-09-30), **`make verify`
55/0**, **`pnpm test` 1723 in 123** (twice), `pnpm test:docker` **198 in 31, all passed** (1504 s), **24
migrations**, **the database EMPTY** (0 projects — the close's `pnpm test` truncated it after the demo), **nothing
listening on 7100, 7102 or 7104**, both loopback aliases on `lo0`. `launch-app` stands in production and staging on the
close's fresh-demo release — containers only: no rows hold them — with `click-launch` and journey-app's staging beside
it; `make verify` reads `mf- containers=15 networks=5 volumes=10`. **`make demo-releases`' fresh path runs `make
demo-production` first** (Decision 17), which rebuilds what it needs on an empty database. Both cleanup scripts were
applied and re-measured clean; images stand at `docker images -q` 115, `127.0.0.1:7107/local/*` 68, which no script
sweeps. The control plane was stopped at sitting 6's close, and it re-registers the platform's SP row at every boot.
