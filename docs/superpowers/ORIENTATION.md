...
- **The D5 plan's three spec actions — RAISED 2026-09-24, none applied** (the exact wording is in [the plan's *Spec actions*](plans/2026-09-24-d5-github-source-driver.md)).
  - **(1) §20 *Git driver* bullet 4 is Rich's decision.** *"A detected secret blocks the push"* cannot hold on GitHub.com, which runs no custom pre-receive hooks, while webhooks fire after a push lands.
    - **(a) Recommended**: reword it to what each path does. Manifest blocks every push it makes and every push to driver 1, and FINDS every commit pushed straight to GitHub, raising an Event and refusing to build it. Blocking a direct push becomes GitHub's push protection on UBC's organisation, an external-track item.
    - **(b)**: keep the sentence and require GitHub Secret Protection (paid, per active committer, Team or Enterprise plans) as a precondition of driver 2. That cannot be tested on a free organisation.
    - **Task 11 (sitting 6) builds (a)'s behaviour, which (b) keeps and adds a precondition to, so the answer is needed before sitting 6.**
  - **(2) §21's inventory** gains one row for the opt-in GitHub fake on 7110.
  - **(3) §20 bullet 1** says that the one installation token which cannot be scoped to a repository is the one that creates it: administration permission alone, used for one call, never kept. **It is withdrawn if Task 6's real conformance leg shows GitHub scopes creation otherwise.**
