...
**Manifest's design is finished and SEVEN implementation plans are executed — P1, P2, P3, P4a, P4b, P4c and P5a (the contract), whose acceptance passed on 2026-09-17. P5b is written and UNDER WAY: sittings 1 to 3 are done, so D24's privileged four are named, the two token tables exist, and an agent holding a delegated token can authenticate with it. The next job is always §7e.** This is the single entry point: what Manifest is, where things stand, how the platform is built, what the machine will do to you, how to work here, and what to do next. It is written for someone with **no prior context** — a new agent with a fresh window, or a developer joining.

*Last verified 2026-09-17 (P5b sitting 3).* **Three places state current status — §2, §7e and §8 — and a sitting's sweep REPLACES what they say; it never appends a sitting's story here** (§6). **The roadmap's ledger outranks all three.** Everything else is durable.
...
| **P5b** | [`plans/2026-09-17-p5b-delegated-tokens.md`](plans/2026-09-17-p5b-delegated-tokens.md) — **UNDER WAY**, sittings 1–3 done | `make demo-token`, Task 12's, not yet built | 40 findings in three sittings. **Sitting 2's F1**: the plan's token parser split on `_` while base64url's alphabet contains it, so it refused 47.5% of the tokens the same file minted — and the round-trip test minted ONE token, so it would have gone red about half the time and read as a flaky harness. **Sitting 2's F14**: the plan's own TRUNCATE negative control cannot fail, because the statement's CASCADE reaches both new tables unnamed. **Sitting 3's F1**: `DELETE /v1/tokens/{tokenId}` is the API's first bodyless mutation, and the contract layer could not carry one — the route answered `400` before its handler ran and the OpenAPI document could not be generated for it at all, because both kept on `method === 'GET'` rather than on the body schema. **Sitting 3's F13**: three of that sitting's own negative controls answer `403` for the WRONG REASON, so a status-only assertion is green through all of them. |

### 7e. Execute P5b's sitting 4 — Task 6 ← **START HERE**

**P5b's sitting 3 is DONE** (2026-09-17, 15 findings, `e407ae3`, `eab5466`, `d992534`).
[`plans/2026-09-17-p5b-delegated-tokens.md`](plans/2026-09-17-p5b-delegated-tokens.md) — 13 tasks
in nine sittings. **Your job is sitting 4: Task 6, ALONE** — the central refusal, and the
`PendingAction` it creates. It is this plan's one irreversible design commitment, which is why
the sittings table gives it a sitting of its own and says it may not be moved. The sittings
table at the top of that plan is the maintained copy of what is done.

**Read, in this order, before you start:**

1. **Task 6's correction block — it is long, it is `[M5]`'s, `[M1]`'s and `[M7]`'s, and it says
   the task AS DRAFTED DEADLOCKS D24's LOOP.** A refusal caught around `run` resolves rather
   than throws, so `replayOrStore` caches the 403; the confirmed retry reuses its
   `Idempotency-Key` exactly as D23.6's own hint instructs, replays the cached refusal for
   ever, and `consumed_at` is never stamped — **while every test using a fresh key per request
   stays green.** Only catch PLACEMENT fixes it; scoping the idempotency record does not.
...
- **`Actor` is a discriminated union on `credential`** (`projects/authz.ts`), with `SessionActor`
  carrying `puid` and `platformRole` and **`TokenActor` carrying neither** — it has `tokenId`,
  `projectId`, `capabilities: ReadonlySet<Capability>`, `rateLimit`, and the `grant?: Capability`
  field **Task 7 is the only thing that may ever set**.
- **`requireSession(request)`** (`api/actor.ts`), and `TokenCredentialRefusedError` →
  `403 TOKEN_CREDENTIAL_REFUSED`. `RouteContext.actor` is the union, so a handler that wants a
  session field calls `requireSession` and `tsc` enforces it.
- **`assertCapability` HAS A TOKEN BRANCH ALREADY** — scope first (`NOT_FOUND`), then
  `actor.capabilities.has(capability)` (`FORBIDDEN`). **Task 6 is what makes a PRIVILEGED
  capability stop being an ordinary `FORBIDDEN` there.**
- **`tokens/actor.ts`** — `tokenActor(db, plaintext, now?)` and `readBearer(header)`, called by
  the one `onRequest` hook in `api/server.ts`, plus `TOUCH_INTERVAL_MS`.
- **Three routes and their representations** (`api/routes/tokens.ts`,
  `api/representations/tokens.ts`), **migration 0015** (`token.minted`). **0015 is the newest.**
- **`withProjectServer` and `sessionFor`** in `api/testing.ts` — the fixture carries `app`,
  `deps`, `db`, `userId`, `ownerCookies`, `projectId`, `otherProjectId`, `commitSha` and the
  staging and production environment ids.
...
- **`api/authz-contract.ts` goes red the moment you register a route and do not list it**, even
  though the plan assigns that file to Task 11. Sitting 3 added three rows for the same reason.
- **`api/stream-contract.test.ts` asserts its lifecycle reaches EVERY `EVENT_TYPE`**, so a new
  event type makes it red unless you drive it through that lifecycle or add it to
  `PUBLISHED_ELSEWHERE`. Driving it is better: that test exists to prove a REAL frame parses as
  the contract's `StreamFrame`.
- **A bodyless mutation now works** — `readsBody(route)` in `api/contract/route.ts` — but that
  was sitting 3's F1 and it had never been exercised before. If you add another `DELETE`, it is
  the second one ever.
...
- **`pnpm test:docker` is owed.** Task 6 changes `projects/authz.ts`, which is in the tier's
  blast radius. It has not moved from 177 in four runs across two sittings — twice in sitting 2, twice in sitting 3.

**The state you are handed, 2026-09-17, after P5b's sitting 3.** `main`, clean.

| | |
|---|---|
| The four gate numbers | §2's box — `pnpm test` **1088** in 97 files (up 38), `pnpm test:docker` **177** in 29 files (0 skipped, measured twice this sitting), `make doctor` **18/0**, `make verify` **51/0**. The last two were re-run AFTER the Docker tier |
| The control plane | **LEFT RUNNING on 7100**, `driver: docker`, **killed, rebuilt and restarted at the end of this sitting** so it serves the sitting's own routes — it serves from `dist/`, and until the restart `POST /v1/projects/{id}/tokens` did not exist through the edge. Check: `curl -sS -o /dev/null -w '%{http_code}' --cacert infra/ca/manifest-root.crt https://console.manifest.internal/v1/me` → `401` when it is up. **Do not start a second one.** It carries a **new `MANIFEST_SESSION_SECRET`** — any cookie predating this sitting is void |
| The database | Migration **0015** applied. **NOT empty**: the closing `make demo-journey` left `journey-app`, its three environments, its build, release and instance, and the `operator` administrator grant. Every `pnpm test` empties it again |
| The demos | **`make demo-journey` was run at the end of this sitting and is GREEN, all eight steps** — not asked for by either task, but Task 5 changes `/v1/me`, `GET /v1/projects` and the fleet, which are §22's journey. The other four were not run; all were green at P5a sitting 12 |
| The apps | The proof, journey and fixture app containers are **up and healthy**, and **`journey-app` IS reachable by name through the edge** — the demo re-registered its route after the closing `pnpm test:docker` truncated `routes`. `proof-app` and `fixture-app` are **not**: their routes are gone and their names fall through to the wildcard. **Check the BODY, never the status** — measured at the end of this sitting: `journey-app` answers its real HTML, while `proof-app`, `fixture-app` and a name nothing has ever deployed all answer `200` with the identical `manifest OK host=… scheme=https remote=10.89.0.1`. Harmless: the next `make demo*` recreates each |
...
| Docker images | **129 local images, from 128 at the start.** This sitting removed the 8 its own two Docker-tier runs built, by digest — **the `docker rmi` refusal did not recur, for the second sitting running** — and the one image it is left up on is `journey-app@sha256:a767606893e3`, which backs the instance the closing demo deployed. Sitting 12's own list is still outstanding |
| LiteLLM | **ELEVEN users, two held by a running container — re-measured independently this sitting, and the previous list's held entry had gone stale AGAIN.** §2's *Outstanding* has all eleven with the command that checks a key without printing it. **Re-measure before acting; do not copy this row** |
