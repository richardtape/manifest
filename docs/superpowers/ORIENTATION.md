...
**Manifest's design is finished and SEVEN implementation plans are executed — P1, P2, P3, P4a, P4b, P4c and P5a (the contract), whose acceptance passed on 2026-09-17. P5b is written and UNDER WAY: EIGHT of its nine sittings are done, and **D24's whole loop now runs end to end through the edge as `make demo-token`** — a person mints a delegated token, an agent holding it builds and deploys a real application on its own authority, is refused the two things it may not do, and gets past one of them only because a person confirmed that exact request, once. **What is left is Task 13, the acceptance.** The next job is always §7e.** This is the single entry point: what Manifest is, where things stand, how the platform is built, what the machine will do to you, how to work here, and what to do next. It is written for someone with **no prior context** — a new agent with a fresh window, or a developer joining.

*Last verified 2026-09-18 (P5b sitting 8).* **Three places state current status — §2, §7e and §8 — and a sitting's sweep REPLACES what they say; it never appends a sitting's story here** (§6). **The roadmap's ledger outranks all three.** Everything else is durable.
...
- **LiteLLM's users, which only Rich can remove — and there is now a script for it, `scripts/litellm-orphans.sh`** (RUNBOOK, *Reaping LiteLLM's orphaned users*). Deleting one through LiteLLM's admin API is refused by the session's permission classifier as a secret-store write, so an agent **lists them (`bash scripts/litellm-orphans.sh`, which changes nothing) and hands them over; it never works around it**, and Rich runs the same script with `--apply`. **The script re-derives what is orphaned every run**, reads stopped containers as well as running ones, and refuses to delete anything if it read app containers and got no key hashes at all — so the list below is a record of what was measured, never an input to the delete. Every demo that replaces a project leaves one more user with no project — **and so does `pnpm test:docker`** — so the list grows most sittings. **RE-MEASURE IT; NEVER COPY THE PREVIOUS SITTING'S.** **RE-MEASURED INDEPENDENTLY at the end of P5b sitting 8, and the previous list's "held" entry HAD GONE STALE AGAIN — the EIGHTH sitting running.** Measured 2026-09-18 on **port 7106** (`/user/list?page_size=100`; a copy of this bullet once implied 7111, which is not LiteLLM's published port): **NINETEEN rows, or EIGHTEEN excluding LiteLLM's own `default_user_id`** — three more than sitting 7's sixteen/fifteen, and all three are this sitting's, because it added a fifth demo slug (`token-app`, whose project is recreated after every truncation) and redeployed `journey-app`. **THREE are now held by a running container, where there were two**: `mf-1db14646-5f52-4274-a107-54a002108e12-staging` (proof-app, still, seven sittings running), **`mf-3a7ed662-a5ab-478e-9efb-0f3a002487d9-staging` (token-app, NEW — the slug `make demo-token` owns)** and **`mf-4992be4b-f06d-4bee-a5bb-abb991312d4d-staging` (journey-app, NEW — this sitting's `make demo-journey` redeployed it)**. **The other fifteen are orphaned and safe**: `p4b-probe-user` (no keys at all), `mf-217d4549-…`, `mf-4192aef8-…`, `mf-49170f5e-…`, `mf-58fcaf86-…`, `mf-6adf7ff8-…`, `mf-9fa0019b-…`, `mf-a2e2cb0d-…`, `mf-af2629e6-…`, `mf-b27ebd54-…`, `mf-d10f278f-…`, `mf-e650609e-…`, `mf-f72bd023-…`, **`mf-c360cca6-8976-4a30-a144-8447c1f195e7-staging`, which the PREVIOUS list named as journey-app's and said was held**, and **`mf-c046a16f-c488-4bb4-bdcc-8e3036cf4f9f-staging`, this sitting's FIRST token-app, orphaned by its own `pnpm test`**. journey-app has moved in every one of the last eight sittings, which is why this bullet says re-measure and means it. **Which key a container holds is checkable without printing it**: `docker exec <app> printenv LLM_API_KEY | tr -d '\n' | shasum -a 256` equals that key's `token` in `GET /user/info?user_id=<user>` — matched this sitting for all three held rows (`24b0a4f2…` → proof-app, `740e28e6…` → journey-app, `2f9e41b4…` → token-app). An app with no AI hashes to `e3b0c442…`, the empty string — `fixture-app` did again, so a hash that looks like a match for "no key" is not one.
- **The Docker cleanup sitting 12 could not do (2026-09-17).** That session's auto-mode classifier refused **every** `docker rm`, `docker network rm`, `docker volume rm` and `docker rmi` as *[Interfere With Workloads]*, so the by-hand sweep sittings 10 and 11 performed was not possible. **THE REFUSAL HAS NOT RECURRED IN P5b SITTINGS 2, 3, 4, 5, 6, 7 OR 8 — and this list of sittings is the ONLY statement of that count in this file; §7e points here rather than restating it** (see the §9 lesson it is an instance of). Sitting 2 removed the 8 images its own two `pnpm test:docker` runs built, by digest, with no refusal (94 → 86); sitting 3 removed its own 8 the same way; **sittings 4 and 5 each removed five**, **sitting 7 removed nine** after two Docker-tier runs, and **sitting 8 removed five** after one (`boot-recover`, `chem-labs`, `fixture-rd`, `redeploy-cp` and one superseded `token-app`) — each keeping the images that BACK RUNNING INSTANCES, which is the reason each sitting's own closing demo leaves one, and each sitting's before/after snapshots differ only by those instances being replaced. So **try it rather than assuming it is refused** — but sitting 12's list below is still there, because that sitting's images are not this one's to judge. Nothing is broken by leaving it — `make demo` simply starts from a fixture app that already exists — but the machine carries more than it did. **`make reset` does all of it in one step**, or by hand: the fixture app `make demo` left (`mf-fixture-app-staging-{…-app,-db,-egress}` with `docker rm -f -v`, then its `-net`, its `-db-data` volume, its image `127.0.0.1:7107/local/fixture-app@sha256:90d104c5…`, and `.manifest/repos/fixture-app.git`), and **20 images the Docker tier and the demos built** that the sitting's before-snapshot did not have — `blueprint-ntm`, `boot-recover`, `chem-labs`, `fixture-rd`, `fixture-s6`, `incident-probe`, `redeploy-cp` (2), `saml-unsigned`, seven `journey-app` and three `proof-app` — **keeping `journey-app@sha256:e288f8b3…` and `proof-app@sha256:11d6174d…`, which the two running apps hold.** The exact list is in P5a's sitting 12 record.
...
| **writing a plan** | None is waiting: P5c is written only after P5b executes, from [`plans/2026-09-16-p5-brief.md`](plans/2026-09-16-p5-brief.md) and P5b's *What this plan does not build*. House style: [`plans/2026-08-30-p1-local-substrate.md`](plans/2026-08-30-p1-local-substrate.md), or any later plan. |
...
│   └── 2026-09-17-p5b-delegated-tokens.md         P5b ← CURRENT
└── spikes/
...
8. **Close out properly — AT THE END OF EVERY SITTING, not at the end of the plan.**
   ...
   **AFTER THE SWEEP, RE-READ YOUR OWN §7e AS A COLD AGENT AND CHECK ITS CLAIMS — THIS
   HAS NOW FOUND A DEFECT IN EVERY ONE OF P5b's SITTINGS 3, 4, 5, 6, 7 AND 8; IN SITTING 7
   IT FOUND THREE AND IN SITTING 8 THREE.** *This list of sittings is the ONLY statement of that count; §7e points
   ...
   because the stale copy carries a different number by definition.
...
**The next job is §7e — P5b's sitting 9, Task 13, the acceptance. Nothing blocks it.** Each plan's own *What executing this plan found* is its record. The roadmap's ledger outranks this section on status.
...
1. **Baseline first** (§6's *Your first ten minutes*): `./scripts/snapshot-machine.sh` to a
   ...
   format:check. **Expect §2's box exactly** — `pnpm test` moved by 3 this sitting and
   `pnpm test:docker` did not move at all.
2. **The plan's *Global Constraints*** — it is the plan that states the commit convention
   (**after every task, on `main`, conventional messages, no branch and no push**), not this file.
3. **REBUILD AND RESTART THE CONTROL PLANE, before you run any demo.** It serves from `dist/`, so
   it is still the PREVIOUS sitting's binary until `tsc` runs, and **step 1's own `pnpm test`
   truncates its tables**, so the database every demo reads is empty. README's ***Running the
   control plane*** is the export block — all three database URLs, the session secret and the
   LiteLLM master key — and `pnpm --filter @manifest/control-plane dev` does `tsc` and then
   `node dist/index.js`. **Kill the old one by PID from `lsof -nP -iTCP:7100 -sTCP:LISTEN`** and
   read the boot line back (`driver: docker`) before going further. **You will do this once per
   negative control as well**, because Step 4's breaks are under `src/`.
4. **Task 13 by its own Steps 1–6.** It is one task, so this sitting is one commit plus the
   close-out. **Step 3's third run needs Rich's agreement** (`make reset`); runs 1 and 2 do not,
   so do those first, and a refusal still leaves two-thirds of the acceptance.
5. **Close out** (§6's sweep) — and this is the sweep that **FINISHES A PLAN**, so `CLAUDE.md`'s
   *State* moves for the first time in eight sittings, and §7e becomes **P5c, TO BE WRITTEN**.
   Then **re-read your own §7e as a cold agent and CHECK its claims**. §6 names the sittings this
   has found a defect in and is the only place that count is stated — do not restate it here.

**THE ONE THING MOST LIKELY TO COST YOU THE SITTING.** `make demo-token` signs the **student** in
(`stu000001`) before the TypeScript runs, and it must: step 6 has the agent ask to add them, and
`POST /v1/projects/{id}/members` refuses `400 MEMBER_USER_NOT_FOUND` for anybody who has never
signed in — which `pnpm test` and `make reset` both arrange by emptying `users`. If that sign-in
is skipped or broken, the **confirmed retry** answers `400` and reads as a defect in D24's grant
rather than as a missing row. `scripts/demo-token.sh` already does it and fails loudly if it left
no session.

**What sitting 8 built, which you now prove:**

- **`scripts/demo-token.sh` + `packages/journey/src/token.ts`** — a SIBLING entry point to
  `main.ts`, not a phase of it, because this story has two credentials in it. **The run prints nine
  steps**; the RUNBOOK's *`make demo-token`* section describes them — grouped into seven, and it says so —
  and says what the demo leaves behind.
- **It owns its own project, `token-app`**, created from `node-ts-mongo@1`'s `proof-app` starter
  the first time and reused after — creating it is the instructor's job, because
  `POST /v1/projects` is interactive-only.
- **`subscribe` now takes a `token`** (F1). A token subscription sends the bearer and **no
  `Origin`**, and passing both credentials is refused in the client.
- **`boundary.test.ts` strips comments** and asserts what it still found, so it cannot pass by
  reading nothing.

**What will surprise you:**

- **The demo leaves ONE `pending` question behind on purpose** — the production promotion nobody
  answers — so §26's queue shows a `pending`, a `confirmed` and a `rejected` after a run. It is
  what Task 10's expiry exists for. Do not read it as a leak.
- **A `checks.must` failure stops every later step** (sitting 8's F7), so a control aimed at step 8
  can only be watched while step 6 passes. **Read which steps EXECUTED before reading which
  passed** — it is P5a sitting 12's *a phase-1 failure makes every phase-2 measurement MISSING*
  inside one process.
- **`&& false` does not disable a branch here** (F8): it destroys the discriminated-union
  narrowing the body depends on and `tsc` refuses the file, naming a property rather than the
  edit. Disable the CALL — `void theFunction; void narrowed.field`.
- **`pnpm test` — even one file — truncates the control plane's tables**, and `pnpm test:docker`
  ...
  `EADDRINUSE` while `/v1/me` still answers `401` from the old one.
- **`make reset` prompts**, so from a tool call it is `echo reset | make reset`, and after it the
  order is `db:migrate`, then the control plane, then the demos.
- **A route change is three files, in order**: the definition, `pnpm contract:write`,
  `pnpm contract:generate`. Task 13 should need none of them — if it does, that is a finding.

**The state you are handed, 2026-09-18, after P5b's sitting 8.** `main`, clean.

| | |
|---|---|
| The four gate numbers | §2's box — `pnpm test` **1342** in 101 files (up 3, all in `packages/`), `pnpm test:docker` **178** in 29 files (0 skipped, unchanged), `make doctor` **18/0**, `make verify` **51/0**. The last two were re-run AFTER the Docker tier |
| The control plane | **LEFT RUNNING on 7100**, `driver: docker`, killed, rebuilt and restarted many times this sitting — once per negative control — and last restarted onto the RESTORED tree. Check: `curl -sS -o /dev/null -w '%{http_code}' --cacert infra/ca/manifest-root.crt https://console.manifest.internal/v1/me` → `401` when it is up. **Do not start a second one.** It carries a session secret minted this sitting — any cookie predating it is void |
| The database | **NO new migration this sitting; 0018 is still the newest.** **NOT empty**: the closing runs left `token-app` and `journey-app`, each with three environments, a build, a release and a healthy staging instance, plus this sitting's pending actions and the `operator` administrator grant. Every `pnpm test` empties it again |
| The demos | **`make demo-token` (NEW) and `make demo-journey` were both run at the end and are GREEN.** The other four were not run; all were green at P5a sitting 12 |
| The apps | `token-app`, `journey-app`, the proof app and the fixture app are **up and healthy**, and `token-app` and `journey-app` **are reachable by name through the edge** — both demos re-registered their routes after the Docker tier truncated `routes`. `proof-app` and `fixture-app` are **not**: their routes are gone and their names fall through to the wildcard. **Check the BODY, never the status** — a name nothing has ever deployed also answers `200`. Harmless: the next `make demo*` recreates each |
| Identity | Three IdP test users — `student`, `instructor`, `operator`. **`operator` IS currently an administrator**, granted by this sitting's `make demo-journey`, which recreates and re-grants. **`student` (`stu000001`) HAS signed in on this database**, put there by `make demo-token`, which does it every run — so the trap above is armed only after a truncation |
| Docker images | **Back to where the sitting started**, re-diffed after the removals (§4): the snapshots differ only by the images backing the running `token-app` and `journey-app` instances, and networks differ only by `token-app`'s own. **The `docker rmi` refusal did not recur — §2's *Outstanding* names the sittings it has not recurred in, and is the only place that count is stated.** P5a sitting 12's own list is still outstanding |
| LiteLLM | **Re-measured independently this sitting — do not copy this row.** §2's *Outstanding* has the count, which entries are held by a running container, the command that checks a key without printing it, and **how many sittings the previous list's "held" entry has gone stale for: that number is stated THERE and nowhere else.** This sitting added a slug, `token-app`, so the platform now has two apps that hold a key rather than one |
