> **Post-P1 snapshot.** Taken 2026-09-05, immediately after P1's execution, with the
> platform **up** (`make up`, builder profile down). Compare against
> [`machine-baseline-2026-09-04.md`](machine-baseline-2026-09-04.md), which was taken
> before P1 ran and with nothing of ours running. The differences you should expect,
> and which are the whole of P1's footprint on this machine:
>
> - **Ten `manifest-*` containers**, two Docker networks and five volumes.
> - **`/etc/resolver/manifest.internal`** — scoped to our zone only, never all of
>   `.internal`.
> - **The `127.0.0.2` alias on `lo0`**, which does not survive a reboot.
> - **The Caddy root in the System keychain.**
> - **New images**: `manifest-caddy:local`, `manifest-dnsmasq:local`,
>   `manifest-idp:local`, `manifest-egress:local`, plus `caddy:2.11.4*`,
>   `alpine:3.22`, `php:8.3-apache` and `composer:2`.
>
> All three host changes reverse with `make host-undo`. **Laravel Valet is untouched**:
> its config files are unmodified (mtime 2026-07-03), its dnsmasq is the same process
> it has run since 1 September, and it keeps `.test`, port 53 and `127.0.0.1:80/443`.

Manifest machine snapshot
taken: 2026-09-05 17:39:12 PDT
...
codercom-coder-1	ghcr.io/coder/coder:latest	Exited (0) 7 days ago
codercom-database-1	postgres:17	Exited (0) 7 days ago
...
credit-card-helper-app-1	credit-card-helper-app	Exited (0) 8 days ago
credit-card-helper-frontend-1	node:20-alpine	Exited (0) 8 days ago
docker-simple-saml-saml-idp-1	docker-simple-saml-saml-idp	Exited (0) 25 hours ago
...
manifest-egress	manifest-egress:local	Up 7 minutes
...
mongo-express	mongo-express:latest	Up 26 hours
mongodb	mongodb/mongodb-community-server:7.0.28-ubi8	Up 26 hours
openwebui-openwebui-1	ghcr.io/open-webui/open-webui:main	Exited (137) 11 days ago
qdrant-local-dev	qdrant/qdrant:v1.17.1	Up 26 hours
ubc-document-parsing-api-clamav-1	clamav/clamav:latest	Exited (143) 11 days ago
ubc-document-parsing-api-gateway-1	ubc-document-parsing-api-gateway	Exited (0) 11 days ago
ubc-document-parsing-api-minio-1	minio/minio	Exited (0) 11 days ago
ubc-document-parsing-api-redis-1	redis:7-alpine	Exited (0) 11 days ago
ubc-document-parsing-api-vlmstub-1	python:3.12-slim	Exited (137) 11 days ago
ubc-document-parsing-api-worker-1	ubc-document-parsing-api-worker	Exited (0) 11 days ago
ubc-document-parsing-api-worker-2	ubc-document-parsing-api-worker	Exited (0) 11 days ago
...
scripts/snapshot-machine.sh: line 151: syntax error near unexpected token `('
scripts/snapshot-machine.sh: line 151: `printf 'dirty       %s file(s)\n' "$(git status --porcelain 2>/dev/null | wc -l | tr -d ' ')"'
