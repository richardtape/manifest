...
**What works today** *(status, as of P6b sitting 7 — 2026-09-23)*: through Manifest's API — under `/v1`, at `https://console.manifest.internal`, which only the host can reach, and which refuses a session-bearing change that does not come from that origin — create a project, push code, validate its
...
while sessions live in the app's own database. The API describes itself: `packages/contract/openapi.json` is generated from the routes declared through `defineRoute` and held to them by a test — **every `/v1` route is declared that way**, each answering a public representation rather than a database row, so a deploy answers an instance with no driver or handle and a release shows its build's digest and scan and its env var *names* without their values — and `make demo-journey` calls it through a TypeScript client generated from that document. `GET /v1/slugs/{slug}` says whether a project name will work before you create it: `chem`, `console` and every other §23 reserved label are refused, with what the label stands for. `GET /v1/blueprints` lists the blueprints and the *starters* each offers — §16's proof app is `node-ts-mongo@1`'s `proof-app` — with each blueprint's knowledge pack beside it, and `POST /v1/projects` creates a project from a blueprint's skeleton and, optionally, a starter, for a stated audience (§24), saying so on the project's event stream. That stream is in the contract too: every event the platform records has a payload schema it is refused without, `openapi.json` describes the stream's messages with those schemas, and the generated client's `subscribe` opens it — `make demo-journey` watches a new project's provisioning replayed. Asking for production is refused with §13's checklist COMPUTED from what the project has — `GET /v1/projects/{projectId}/launch-readiness` answers the same thing, read-only, saying which plan builds each item Manifest does not track yet — and a platform administrator, made out of band by `scripts/admin-grant.sh` and audited in `audit.role_changes`, reads every app on the platform at `GET /v1/fleet` (§26). **An agent can now hold its own credential.** A project owner mints a *delegated token* in the console, by clicking — scoped to that one project and to an explicit list of what it may do, with an expiry of at most a year (D24). Its secret is shown once, when it is minted, and the platform keeps only a hash of it: no route, no query and no support request can produce it again. An agent sends it as `Authorization: Bearer …` instead of a person's session, and it reads and acts on exactly the one project it was scoped to — any other answers as though it did not exist. Four things it may never do, however it was minted: manage members, promote a release to production, change a quota, or read a secret. **And two more it can never do at all, since P6b** — approve a release, or record what UBC IAM or the Privacy Office decided — because each is a record that a named person decided: a token cannot be minted holding either, and one that asks is refused outright with no question for anybody to confirm. **And when it asks for one of those four, Manifest now turns the refusal into a question.** The refusal happens in one place, at the authorization layer rather than in each route, so a route added later cannot forget it; the answer carries a *pending action* — what was asked for, in a sentence, and a fingerprint of the request, never the request's own contents. **And a person now answers it.** Signed in, and only if they hold that power themselves, they confirm it or refuse it in their own words. A confirmation does not do the thing on the agent's behalf: it lets **that one request** — that agent, that exact ask — through **once**, and the agent makes it again itself, checked as any request is. Asking a second time is a fresh question. A refusal is final for that request and the agent is told why, so it stops rather than asking for ever. **And the questions are now readable as a list, which is what a console is built on.** A person reading a project sees every question agents have put to it, newest first, with how long each has waited — §26's headline number, which the platform computes rather than leaving each client to subtract two timestamps. An agent reads only the questions it asked itself, which is how it waits for an answer instead of retrying to find out. Neither read ever carries the refused request's contents. **Removing somebody from a project** is the fourth of those privileged four to become possible at all, and it was written after the rule that guards it: it does nothing about agents, and an agent asking is turned into a question anyway. **An agent's requests are also limited now**, at a rate set on its own token — past it every route answers *too many requests*, and says when to try again — while a person in a browser is not limited, because the reason for the control is that an agent is code the platform did not write. **And a question nobody answers does not wait for ever.** Each one is asked with a life of a day, an agent cannot spend an answer given to a question that has since lapsed, and the platform marks the lapsed ones as such when it starts — so a list of what is waiting is a list of what somebody can still act on. Two agents asking the identical thing at the identical moment now produce one question rather than two, which is enforced by the database rather than by a check that could lose a race. **And the whole loop now runs end to end**, as `make demo-token`: an agent builds and deploys a real application on a token of its own, is stopped at the two things it may not do, and gets past one of them only because a person said so. **And there is now a console to open in a browser, and a faculty member can drive most of the journey in it by clicking.** `https://console.manifest.internal` serves it — a deliberately plain reference client, the working proof that everything above can be driven from outside. A person **signs in with their CWL** and sees who the platform thinks they are; **creates a project** by typing a name that is checked for them as they type, choosing a blueprint and a starter and saying who the application is for; **watches it being made** on a live feed that needs no refreshing; **builds it**, reading the builder's output line by line as it is written rather than waiting for a wall of text at the end, and sees how the build ended along with the image it produced and what the vulnerability scan found; **releases that build**, **deploys it to staging** watching each step arrive as it happens, and then **opens the running application and signs in to it with their CWL**. When a deploy fails it says so, keeps the previous version serving, and shows what the platform checked, what the application printed, and what changed since it last worked. **Asks for production** and reads §13's checklist of what a first launch still needs — every item with its state, the reason, who owns it, and, for the ones Manifest does not track yet, which plan builds them — the same checklist, to the byte, that the production deploy's refusal carries. **Mints a delegated token for an agent** and is shown its secret exactly once, with the four things D24 never lets a token hold greyed out and the reason beside them — and, since P6b, the two only a person may do (approving a release, recording UBC's decisions), greyed out with theirs; lists the project's tokens, seeing at a glance which are active, which a clock stopped and which a person did; and revokes one. And an administrator reads **the whole fleet** — every project on the platform, its owner, its environments and what each is running. It is a developer's host process on port 7104 and nothing starts it for you, so the address answers `502` until somebody does. **And a person now answers an agent's questions by clicking, too** — the queue is a screen: every question the project's agents have put to it, who asked, how long it has waited, and a confirm or a refusal typed in the person's own words, with a lapsed one shown as lapsed and offering no button at all. **And a front-end developer needs none of this running.** `manifest-mock` serves the whole published contract from fixtures in a single process — with a scripted event stream that plays a build's output line by line and then a deploy's states — so the console can be developed and driven with no Docker, no database, no control plane and no language model. It is held to the same published document the real API is generated from, so a client built against it cannot compile against a contract the platform does not serve. **And some things now ask you to prove who you are a second time.** Changing who is on a project, confirming an agent's question, and **now approving a release for production** are refused unless you have signed in again within the last ten minutes. The refusal says so and tells you where to go; `/auth/step-up` sends you back to CWL, which prompts you **even though you are already signed in to it**, and the same action then works. Signing in does not count: §20 asks for a *second* round trip, on purpose. An agent holding a delegated token cannot do this at all — what stands in its place is the question a person already answers for it. **And a release can now be approved for production** — the first thing in this platform's life that asserts `release:approve`. A platform administrator, having proved themselves a second time, approves or declines the release serving staging; the approval binds the *image digest* rather than a tag, so a later rebuild is not covered by it and the checklist says so in words; it stores the exact diff the administrator was shown, with an AI-written summary of what changed since the last approved release — and if the language model cannot be reached the summary is recorded as *absent* rather than blocking the approval, because a gate that fails closed on a model being down is an outage and not a control. A decline must carry the administrator's own words, and the project's owner can read them. **And since the same day it is a screen**: the checklist links to it, an administrator reads the release's digest, its scan and the production configuration first, and presses *Approve* — which is refused with a link, *Confirm it is you, then try again*, that takes them to CWL and back. **And since P6b the administrator reads it FIRST**: the screen takes a *stored preview* — the diff, a security note per sensitive field, the code reviewer's verdict, the summary — valid for thirty minutes, and the approval names that preview and records exactly it, so the words read are the words kept. **And an application has now REACHED production.** Manifest runs a *rehearsal* first, which is D21's: it deploys the release serving staging to the application's real production hostname, registers its Service Provider with production values, and **completes a real CWL sign-in against it** — then records what it measured, including which attributes the assertion actually released, so the checklist item is met by evidence rather than by a tick. It proves the *shape* of the registration and says in those words that it proves nothing about UBC's acceptance of it. Then the administrator approves, and the deploy verifies the approved image digest **before it starts anything at all**. The application answers on its production address, on the public listener, and on nothing else. **And all of it is now clicked in the console** (P6a sitting 10): an administrator reaches any project from the fleet, records what UBC IAM and the Privacy Office said on the project's *Launch records* tab — along §9's steps, the platform refusing a jump in its own words — runs the rehearsal from its checklist item and reads what it measured, approves, and deploys to production. **And the same launch runs headlessly as `make demo-production`** (P6a's acceptance, 2026-09-22): an instructor is refused production, an administrator records the two external records, Manifest rehearses, the administrator is refused the approval until CWL has asked them again, approves, and the owner deploys — and the app answers on the public address as the instance the deploy started, and on the internal address not at all. **And once an application has launched, its next release goes to production without an administrator** (P6b, `make demo-releases`) — unless it changes one of §7's sensitive fields, when it is refused until an administrator approves it from a preview; and a CWL attribute UBC IAM has not registered cannot even be built until an administrator records that UBC registered it. **What does not exist yet:** an application in production on this laptop points at UBC's real single sign-on, which is not reachable from here, so it serves and signs nobody in; and nothing marks a question lapsed *while* the platform is running, so one that ran out an hour ago is still stored as waiting until the next restart — the queue screen shows it as lapsed from its own deadline, and asking to act on it is refused either way.
...
| 9 | back in the console, **Request production** | `LaunchReadiness`: `ready: false`, every item with its `why`. **Since P6a Task 7 the deploy is refused BY this checklist rather than unconditionally**, and the items are in three states: `met` for what the platform computed, `unmet` for the IAM registration and the PIA — real rows an administrator records, so **no `builtBy`** — and — **since P6a Task 14 — `rehearsal` is `unmet` with no `builtBy` too**, because Manifest runs one now; the only `not_built` item left is `code-review`, which does not block. **Since P6a Task 10 `admin-approval` is `unmet` with no `builtBy` too** — it reads a real approval row, and on a project nobody has approved it says the release has not been reviewed yet |
| 10 | **Tokens** → mint one | the secret is shown **once**; reload and it is gone; the privileged four and the person-only two cannot be ticked |
...
### The first production launch, clicked

The same `make demo-console`, signed in as the **administrator** — `operator` / `operator`,
made one by `scripts/admin-grant.sh grant opr000001 "<reason>"` and a second sign-in (`make
demo-production` does both). **You type the password twice**: once to sign in, and once when CWL
asks again before the approval. An administrator holds everything an owner does, so the whole
launch can be clicked as one person. What you will actually see, from P6a sitting 11
(2026-09-22):
...
and **row 10 is the one only a person can prove**: that CWL really asks again.

### What a launched app's next release does, clicked

*Added by P6b sitting 7, 2026-09-23 — the rows as a person saw them.* After `make demo-releases` (or
`make demo-production`), `launch-app` has launched. Stage a release to click with — a code-only commit,
...
- **What a person can DO there today** (P5c Tasks 5 to 8): sign in with CWL and see who the
  ...
  **Deploy to production is there and is refused**, with §13's checklist rendered from the
  ...
  shortens the list of what is still missing. **SINCE P6a TASK 15 THE CONSOLE'S PRODUCTION
  DEPLOY IS REFUSED `403 STEP_UP_REQUIRED` FIRST**, before the checklist is consulted — §20
  guards promoting a release — and **since P6a sitting 10 that refusal carries the link that
  does the step-up**, *Confirm it is you, then try again*, which sends the browser to CWL (it
  asks for the password again even though you are signed in) and back to the same page. **§26's QUEUE AND D24's TOKENS ARE BOTH SCREENS NOW.** On a project's *Tokens* tab a person
...
| `make down` | Stops the platform, the GitHub fake included. Data, the mirror and the CA survive |
| `make reset` | **Destroys every project's data** — databases, registry contents, the GitHub fake's repositories, every `mf-` container, network and volume. Asks you to type `reset`. Keeps the CA, the master key (and the fake GitHub App's credentials beside it) and the npm mirror. Afterwards: `make up`, then `db:migrate` before the control plane |
