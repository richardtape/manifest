...
**What works today** *(status, as of P5c sitting 8 — 2026-09-19)*: through Manifest's API — under `/v1`, at `https://console.manifest.internal`, which only the host can reach, and which refuses a session-bearing change that does not come from that origin — create a project, push code, validate its
...
| `make demo-production` | **P6a's acceptance** — the first production launch through the edge, on `launch-app`: refused production, both external records recorded along §9's steps, the rehearsal, the approval refused until step-up, approved against the digest, deployed — and the app answers on `127.0.0.3` as the instance the deploy started and not on `127.0.0.2`. A second run re-uses `launch-app` (nothing deletes a project) and says so. **Green from a `make reset` machine and step 11 of the offline acceptance** (2026-09-22). **It leaves its last rebuild, unapproved, serving STAGING**, so a production deploy clicked afterwards is refused until an approved release is staged again | `make up`, `127.0.0.3` on `lo0`, and the control plane | ~3 min |
