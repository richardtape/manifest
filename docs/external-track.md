...
| 10 | **The production GitHub organisation's own controls** (D5's driver 2): members may not create public repositories or make one public; **GitHub push protection** on (GitHub Secret Protection, a paid Team or Enterprise add-on), with Manifest's secret rules as custom patterns | Driver 2 serving UBC — never needed on a laptop | §20, as applied on 2026-09-24 (the D5 plan's Spec action 1, option (a)): **on GitHub.com nothing Manifest runs can block a push made straight to GitHub** — Manifest finds it afterwards, raises an Event and refuses to build it, but the secret is already on GitHub. Push protection is the only control that blocks it there, and the organisation's policy is the only PREVENTIVE control for visibility (Manifest makes a public repository private again, but §20 calls that leak immediate and irreversible). Both are UBC's to turn on |
...
| 2026-09-22 | — | **P6a IS EXECUTED, AND THE WHOLE LOCAL LAUNCH NOW RUNS BY ITSELF — so everything this page lists is now the only thing between the platform and a real first launch.** `make demo-production` takes an application to production with every blocking item met, repeatably and from a wiped machine, and a person has clicked the same launch in the console. Its IAM registration and PIA are rows an administrator records with a ticket reference — today `IAM-ACCEPTANCE` and `PIA-ACCEPTANCE`, invented. **Nothing on this page has been started**; starting it is Rich's (ORIENTATION §2). Items 3, 4 and 5 are what would replace the invented tickets with real ones. |
| 2026-09-24 | **new, item 10** | **The production GitHub organisation's controls join the track.** Rich chose option (a) for the D5 plan's Spec action 1 and it was applied to §20 the same day: Manifest blocks every push it makes and every push to its local repositories, and FINDS a push made straight to GitHub, because GitHub.com runs no custom pre-receive hooks. **Blocking a direct push, and preventing a repository from being made public, are therefore settings on UBC's GitHub organisation**, not code — push protection (GitHub Secret Protection) and the organisation's visibility policy. Not needed until driver 2 serves UBC. |
