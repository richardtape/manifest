...
(its trigger fired on 2026-09-15), and the rest of ORIENTATION §8. **THE MACHINE CLEANUPS ARE OWED AGAIN as of the close of P5c sitting 3** — seven
dead networks, one dead volume and one LiteLLM orphan (`p4b-probe-user`), all regenerated by that
sitting's own `pnpm test:docker`. They were ALL CLEAR at the close of sitting 1, applied by Rich
and re-measured by the scripts; sitting 2 ran no Docker tier and they stayed clear; sitting 3 ran
one and they came back. **The cycle is the thing to understand, not the status**: a tier run takes
